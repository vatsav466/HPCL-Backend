...
async def update_vendor_authentication_headers():
    ...
    await redis_client.hset(
        "vendor_auth",
        f"hpcl_{vendor}_access_key",
        "Mauad4ysWfCzb1eAOSLKYM9yp8DyxLtio0H7QbXl7kkkzaTePw7dYJui3KWCccSp",
    )
    await redis_client.hset(
        "vendor_auth",
        f"hpcl_{vendor}_allowed_apis",
        json.dumps(
            [
                f"/api/{vendor}/ingest_data",
                f"/api/{vendor}",
                f"/api/{vendor}/ingest_data_score",
                f"/api/{vendor}/ingest_data_close",
            ]
        ),
    )
    ...
    await redis_client.hset(
        ...
        f"hpcl_{vendor}_allowed_apis",
        json.dumps(
            [f"/api/{vendor}/ingest_data", f"/api/{vendor}", f"/api/{vendor}/api_ack"]
        ),
    )

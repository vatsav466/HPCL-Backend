    ...
    try:
        ...
        df = df.rename(
            columns={
                ...
                "ccoe tankage  (tmt)": "ccoe_tankage",
                "time of commissioning": "time_of_commissioning",
                "mode": "mode",
                "supply": "supply",
                "LOCATION": "location_name",
                "latitude": "latitude",
        ...
        df["installed_bottling_capacity"] = pd.to_numeric(
            df["installed_bottling_capacity"], errors="coerce"
        ).fillna(0.0)
        df["operating_bottling_capacity"] = pd.to_numeric(
            df["operating_bottling_capacity"], errors="coerce"
        ).fillna(0.0)
        df["ccoe_tankage"] = pd.to_numeric(df["ccoe_tankage"], errors="coerce").fillna(
            0.0
        )
        ...
        current_data = await LPGInfra.get_all(
        ...
        if existing_records:
            print(
                "----Moving current LPGInfra to Historic before inserting new data----"
            )
...
async def lpginfra_update_lpg_data(data: Lpginfra_Update_Lpg_DataParams):
    try:
        ...
        existing = await LPGInfra.get_all(
        ...
        if existing["data"]:
            ...
            return {
                "status": False,
                "message": f"No record found for ID {lpg_data.unique_id}",
            }
        ...
        return {
            "status": False,
            "message": "An error occurred while updating LPG data",
            "error": str(e),
        }
...
async def lpginfra_add_lpg_data(data: Lpginfra_Add_Lpg_DataParams):
    try:
        ...
        return {
            "status": False,
            "message": "An error occurred while creating LPG data",
            "error": str(e),
        }
...
async def lpginfra_delete_lpg_data(data: Lpginfra_Delete_Lpg_DataParams):
    try:
        ...
        return {
            "status": True,
            "message": f"LPG Data Deleted Successfully. Deleted: {len(deleted_ids)}, Not Found: {len(not_found_ids)}",
        ...
        return {
            "status": False,
            "message": "An error occurred while deleting LPG data",
            "error": str(e),
        }

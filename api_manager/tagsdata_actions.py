...
async def tagsdata_things_board_device_data(
        ...
        for _, row in location_df.iterrows():
            ...
            for dev_type, device_names in location_counts.items():
                ...
                if count > 0:
                    ...
                    if upper_dev_type in system_classification:
                        ...
                        record["system"] = (
                            "Unknown"  # Default value for unmapped device types
                        )
                    ...
                    for maintenance_item in Maintenance:
                        ...
                        if isinstance(equipment, list):
                            if (
                                dev_type in equipment
                                and maintenance_item["alert_category"] == system
                            ):
                                # interlocks.append(maintenance_item["interlock_name"])
                                if (
                                    maintenance_item["interlock_name"]
                                    not in global_interlock
                                ):
                                    interlocks.append(
                                        maintenance_item["interlock_name"]
                                    )
                                    global_interlock.append(
                                        maintenance_item["interlock_name"]
                                    )
                        # Handle the case where equipment_name is a string
                        elif (
                            dev_type.upper() == equipment.upper()
                            and maintenance_item["alert_category"] == system
                        ):
                            # interlocks.append(maintenance_item["interlock_name"])
                            if (
                    ...
                    for fault_item in Fault:
                        ...
                        if isinstance(equipment, list):
                            if (
                                dev_type in equipment
                                and fault_item["alert_category"] == system
                            ):
            ...
            if sap_id in mfm_map:
                ...
                for maintenance_item in Maintenance:
                    ...
                    if (
                        isinstance(equipment, str)
                        and equipment.upper() == "MFM"
                        and maintenance_item["alert_category"] == "Gantry"
                    ):
                        interlocks.append(maintenance_item["interlock_name"])
                    # Check if equipment_name is a list containing "MFM"
                    elif (
                        isinstance(equipment, list)
                        and "MFM" in equipment
                        and maintenance_item["alert_category"] == "Gantry"
                    ):
                        interlocks.append(maintenance_item["interlock_name"])
                ...
                for fault_item in Fault:
                    ...
                    if (
                        isinstance(equipment, str)
                        and equipment.upper() == "MFM"
                        and fault_item["alert_category"] == "Gantry"
                    ):
                        interlocks.append(fault_item["interlock_name"])
                    # Check if equipment_name is a list containing "MFM"
                    elif (
...
async def tagsdata_get_tags_data(data: Tagsdata_Get_Tags_DataParams):
    try:
        ...
        device_data_result = await tagsdata_things_board_device_data(
            Tagsdata_Things_Board_Device_DataParams()
        )
        ...
        if res:
            ...
            df["mf_count"] = (
                pd.to_numeric(df["mf_count"], errors="coerce").fillna(0).astype(int)
            )

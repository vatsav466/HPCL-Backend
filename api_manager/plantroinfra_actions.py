...
async def plantroinfra_upload_plant_ro_file(file: fastapi.UploadFile):
    ...
    start_date = (
        pd.to_datetime(start_date, errors="coerce").strftime("%d/%m/%Y")
        if not pd.isna(start_date)
        else ""
    )
    to_date = (
        pd.to_datetime(to_date, errors="coerce").strftime("%d/%m/%Y")
    ...
    while col < len(company_row):
        ...
        if comp_name:
            companies.append({"name": comp_name, "col": col})
        col += 4
    ...
    for i, val in df[0].items():
        ...
        elif current_status and sval not in [
            "STATES",
            "STATE",
            "T O T A L",
            "TOTAL",
...
async def plantroinfra_get_all_plant_ro_infra(
    data: Plantroinfra_Get_All_Plant_Ro_InfraParams,
):
    return await plant_retail_functions.get_all_plant_ro_info(
...
async def plantroinfra_get_plant_ro_count_infra(
    data: Plantroinfra_Get_Plant_Ro_Count_InfraParams,
):
    return await plant_retail_functions.get_plant_ro_count_info(
        filters=data.filters,
        cross_filters=data.cross_filters,
        drill_state=data.drill_state,
        limit=data.limit,
...
async def plantroinfra_get_retail_company_infra(
    data: Plantroinfra_Get_Retail_Company_InfraParams,
):
    return await plant_retail_functions.get_retail_company_info(
        filters=data.filters,
        cross_filters=data.cross_filters,
        drill_state=data.drill_state,
        limit=data.limit,
...
async def plantroinfra_get_distinct_ro_retail_infra(
    data: Plantroinfra_Get_Distinct_Ro_Retail_InfraParams,
):
    return await plant_retail_functions.get_distinct_ro_retail_info(
        data.sbu, data.company, data.zone, data.state, data.status, data.ro_status
    )


# Action get_top_five_ro_infra
@router.post("/get_top_five_ro_infra", tags=["PlantRoInfra"])
async def plantroinfra_get_top_five_ro_infra(
    data: Plantroinfra_Get_Top_Five_Ro_InfraParams,
):
    return await plant_retail_functions.get_top_five_ro_info(
        filters=data.filters,
        cross_filters=data.cross_filters,
        drill_state=data.drill_state,
        limit=data.limit,
...
async def plantroinfra_get_zone_wise_ro_infra(
    data: Plantroinfra_Get_Zone_Wise_Ro_InfraParams,
):
    return await plant_retail_functions.get_zone_wise_ro_info(
        filters=data.filters,
        cross_filters=data.cross_filters,
        drill_state=data.drill_state,
        limit=data.limit,
...
async def plantroinfra_get_ro_status_ro_infra(
    data: Plantroinfra_Get_Ro_Status_Ro_InfraParams,
):
    return await plant_retail_functions.get_ro_status_ro_info(
        filters=data.filters,
        cross_filters=data.cross_filters,
        drill_state=data.drill_state,
        limit=data.limit,
...
async def plantroinfra_get_zone_table_ro_infra(
    data: Plantroinfra_Get_Zone_Table_Ro_InfraParams,
):
    return await plant_retail_functions.get_zone_table_ro_info(
        filters=data.filters,
        cross_filters=data.cross_filters,
        drill_state=data.drill_state,
        limit=data.limit,

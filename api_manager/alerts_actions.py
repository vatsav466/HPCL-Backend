...
async def get_truck_location_data(truck_number):
    ...
    if res.get("data"):
        ...
        res = await VtsAlertHistory.get_all(
            urdhva_base.queryparams.QueryParams(
                q=f"tl_number='{truck_number}'", limit=1
            ),
            resp_type="plain",
        )
        if not res.get("data"):
    ...
    location = (
        loc["data"][0] if loc.get("data") else {"name": "", "zone": "", "region": ""}
    )
...
async def alerts_get_performance_index(data: Alerts_Get_Performance_IndexParams):
    ...
    resp = await urdhva_base.BasePostgresModel.get_aggr_data(
        query, limit=data.limit, skip=data.skip
    )
...
async def alerts_upload_document(
    bu: str, upload_file: fastapi.UploadFile = fastapi.File(None)
):
    try:
        UPLOAD_DIR = os.path.join(
            urdhva_base.settings.uploads, bu
...
async def alerts_intitiate_vts_exception(data: Alerts_Intitiate_Vts_ExceptionParams):
    ...
    user = ""
    altcount = await check_violation_count.CheckViolationCount().check_violation_count(
        alert_data["sap_id"],
        alert_data["bu"],
    ...
    if altcount > max_limit:
        altcount = max_limit
    future_time = (
        details["alerting_rules"][str(altcount)]["block_duration"] * 24 * 60 * 60
    )
    # Add future_time to the original timestamp
    unblock_time = (
        alert_data["created_at"] + datetime.timedelta(seconds=future_time)
    ).strftime("%d-%m-%Y")
...
async def alerts_get_frequent_dryout_terminals(
    data: Alerts_Get_Frequent_Dryout_TerminalsParams,
):
...
async def alerts_get_closed_alerts_details(
    data: Alerts_Get_Closed_Alerts_DetailsParams,
):
    ...
    alert_role = (
        alert_data["assigned_user_roles"] if alert_data["assigned_user_roles"] else ""
    )
    ...
    close_alert_details["category"] = list(
        action_data.get("category", {"Others": "Others"}).keys()
    )
    if data.alert_section in ["VTS"]:
        if not data.category:
            for key in close_alert_details["category"]:
                close_alert_details["rca_reason"].extend(
                    action_data.get("category")[key]
                )
        else:
            close_alert_details["rca_reason"].extend(
                action_data.get("category")[data.category]
            )
    else:
    ...
    if data.alert_section in ["VTS"] and data.bu in ["TAS", "LPG"]:
        if alert_data.get("action_on", "") in ["maker"]:
            ...
            close_alert_details["actions"] = {
                k: v for k, v in all_actions.items() if k in allowed
            }
...
async def alerts_vts_alert_manager(data: Alerts_Vts_Alert_ManagerParams):
    ...
    alert_status_value = next(
        (f.value for f in data.filters if f.key == "alert_status"), None
    )
    ...
    required_columns = [
        ...
        "maker",
        "checker",
        "actual_trip_end_date",
        "novex_alert_created_date",
    ...
    if alert_data["data"]:
        ...
        df = df.rename(
            {
                "unique_id": "alert_id",
                "interlock_name": "instance_level",
                "vehicle_number": "tt_number",
                "device_id": "instance_status",
                "assigned_user_roles": "maker",
                "last_escalated_to": "checker",
                "external_timestamp": "actual_trip_end_date",
                "created_at": "novex_alert_created_date",
            }
        )
...
async def alerts_bulk_send_to_unblock(data: Alerts_Bulk_Send_To_UnblockParams):
    try:
        ...
        return (
            True,
            "Request sent successfully, Processing the request. Please wait for sometime",
        )
    except Exception as e:
        print(traceback.format_exc())
        logger.error(e)
...
async def alerts_bulk_send_to_approve(data: Alerts_Bulk_Send_To_ApproveParams):
    try:
        ...
        return (
            True,
            "Approved successfully, Processing the approval. Please wait for sometime",
        )
    except Exception as e:
        print(traceback.format_exc())
        logger.error(e)
...
async def alerts_block_vts_truck(data: Alerts_Block_Vts_TruckParams):
    try:
        ...
        if ("HQO HSE SOD" not in rpt.get("novex_role", [])) and (
            "HQO LPG" not in rpt.get("novex_role", [])
        ):
        ...
        if truck_no and not VEHICLE_REGEX.match(truck_no):
            return {"status": "error", "message": "Please enter a valid truck number"}
        ...
        if not location_name or not zone or not region or not sap_id:
            ...
            """
            transporter_res = await urdhva_base.BasePostgresModel.get_aggr_data(
                transporter_query
            )
            if transporter_res.get("data"):
                transporter_code = transporter_res["data"][0].get(
                    "transporter_code", ""
                )
        ...
        total_wait_time_minutes = int(
            (end_date_utc - start_date_utc).total_seconds() / 60
        )
        ...
        alert_data = {
            ...
            "checkTicketClose": (
                data.check_ticket_close
                if data.check_ticket_close is not None
                else False
            ),
        }
        ...
        camunda_url = await helpers.get_camunda_url(
            data.bu.value, sap_id, alert_section="VTS"
        )
        await cls.create_alert(alert_data, camunda_url)
        return {
            "status": True,
            "message": "Truck has been moved check to completed trip or not",
        }
    except Exception as e:
...
async def alerts_unblock_vts_truck(
    ...
    try:
        ...
        if "HQO HSE SOD" not in rpt.get("novex_role", []) and "HQO LPG" not in rpt.get(
            "novex_role", []
        ):
            return {"status": False, "message": "Not Allowed To Perform This Action"}
        ...
        payload = {
            ...
            "processVariables": {"auto_unblock": {"value": "false", "type": "String"}},
        }
        ...
        if response.status_code != 204:
            ...
            return {
                "status": False,
                "message": "Failed to unblock the truck via workflow",
            }
...
async def alerts_get_vts_blocked_trucks(data: Alerts_Get_Vts_Blocked_TrucksParams):
    ...
    if tab == "vts":
        query = (
            f"vehicle_unblocked_date is null and alert_section='VTS'"
        ...
        return {
            "status": True,
            "message": "success",
            "data": {"vts_blocked_list": vts_blocked_data},
        }
    ...
    if tab == "alerts":
        ...
        return {
            "status": True,
            "message": "success",
            "data": {"alert_blocked_list": alert_blocked_data},
        }
...
async def alerts_get_vts_unblocked_trucks(data: Alerts_Get_Vts_Unblocked_TrucksParams):
    query = (
        f"vehicle_unblocked_date is not null and alert_section='VTS'"
        f"and interlock_name = 'Itdg Admin Blocked'"
    )
    ...
    if alert_data["data"]:
        return {"status": True, "message": "success", "data": alert_data["data"]}
...
async def alerts_alerts_get_vts_query(data: Alerts_Alerts_Get_Vts_QueryParams):
    ...
    try:
        ...
        result = await core_functions.process_vts_query(
            vehicle_number=data.vehicle_number, cross_filters=data.cross_filters
        )
        return result
        ...
        logger.error(
            f"Input data: {data.dict() if hasattr(data, 'dict') else 'Unable to serialize'}"
        )
        return {
            "success": False,
            "error": str(e),
            ...
            try:
                payload = [
                    {
                        ...
                        "blockingFrom": alert["vehicle_blocked_start_date"].strftime(
                            "%Y%m%d"
                        ),
                        "blockingTo": alert["vehicle_blocked_end_date"].strftime(
                            "%Y%m%d"
                        ),
                    }
                ]

                unblocking_status, error_msg = await vts_analysis.post_blocked_tt_ims(
                    payload
                )
                ...
                if closed_at:
                    ...
                    await alert_manager.AlertAction.update_alert_history(
                        input_data=alert, alert_data=alert
                    )
                    await Alerts(
                        **{
                            "id": alert_id,
                ...
                delete_url = (
                    f"{camunda_url}/engine-rest/process-instance/{process_instance_id}"
                )
...
async def alerts_attach_alert_blocked_file(
    ...
    try:
        ...
        if not record_resp.get("data"):
            return {"status": False, "message": "Record not found for given unique_id"}
        ...
        # -----------------------------------
        update_data = {"id": row_id, "file_uploaded_path": minio_path}
        ...
        return {"status": False, "message": "Failed to upload file", "error": str(e)}
...
async def alerts_attach_vts_blocked_file(
    ...
    try:
        ...
        # -----------------------------------
        params = urdhva_base.queryparams.QueryParams(q=f"id='{unblock_id}'", limit=1)
        record_resp = await VtsManualBlocked.get_all(params, resp_type="plain")

        if not record_resp.get("data"):
            return {"status": False, "message": "Record not found for given unblock_id"}
            ...
            payload = [
                {
                    ...
                    "blockingFlag": "N",
                    "blockingFrom": (
                        record["blocking_from"]
                        + datetime.timedelta(hours=5, minutes=30)
                    ).strftime("%Y%m%d"),
                    "blockingTo": (
                        record["blocking_to"] + datetime.timedelta(hours=5, minutes=30)
                    ).strftime("%Y%m%d"),
                }
            ]
            print("-" * 20)
...
async def alerts_hqo_blocked_vehicles(data: Alerts_Hqo_Blocked_VehiclesParams):
    ...
    return await service.get_blocked_trucks_service(
        data.alert_status, data.start_date, data.end_date
    )
...
async def alerts_add_rca_reason(data: Alerts_Add_Rca_ReasonParams):
    try:
        ...
        if not resp["data"]:
            return {"status": True, "message": "Alert Not Found"}
        ...
        await Alerts(**{"id": alert_id, "rca": rca}).modify()

        return {"status": True, "message": "Remarks updated successfully"}
    except Exception as e:
        return {"status": False, "message": "Failed to update remarks", "error": str(e)}
...
async def alerts_get_va_cleanliness_last_synced_time(
    data: Alerts_Get_Va_Cleanliness_Last_Synced_TimeParams,
):
    last_synced_time = "-"
    try:

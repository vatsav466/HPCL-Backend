...
async def dashboards_save_dashboards(data: Dashboards_Save_DashboardsParams):
    ...
    created_user = (
        rpt.get("given_name", "Zolix") + " " + rpt.get("family_name", "Engine")
    )
    ...
    for grp_ind, grp_id in enumerate(data.group_id):
        if grp_id == 0:
            grp_data = DashboardGroupsCreate(
                **{
                    "name": data.group_name[grp_ind],
            ...
            data.group_id = (
                [collected["id"]] if isinstance(collected, dict) else [collected.id]
            )
        else:
...
async def dashboards_get_dashboard_details(
    data: Dashboards_Get_Dashboard_DetailsParams,
):
    """
    Description:
    ...
    """
    return await charts_functions.fetch_dashboard_details(
        data.organization_id, data.name, data.value
    )
...
async def dashboards_get_dashboard_groups(data: Dashboards_Get_Dashboard_GroupsParams):
    ...
    """
    return await charts_functions.fetch_dashboard_group_details(
        data.organization_id, data.group_id
    )

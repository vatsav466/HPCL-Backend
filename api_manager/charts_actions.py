...
async def charts_get_tables(data: Charts_Get_TablesParams):
    ...
    charts_ins = Charts_Connection_Vault_RoutingParams(
        connection_id=data.connection_id, action="table_name"
    )
...
async def charts_get_columns(data: Charts_Get_ColumnsParams):
    ...
    # function = await charts_connection_vault_routing(Charts_Connection_Vault_RoutingParams)
    charts_ins = Charts_Connection_Vault_RoutingParams(
        connection_id=data.connection_id, action="column_names"
    )
    function = await charts_connection_vault_routing(charts_ins)
...
async def get_databases(connection_id: str):
    ...
    # function = await charts_connection_vault_routing(Charts_Connection_Vault_RoutingParams)
    charts_ins = Charts_Connection_Vault_RoutingParams(
        connection_id=connection_id, action="get_databases"
    )
    function = await charts_connection_vault_routing(charts_ins)
...
async def charts_get_unique_values(data: Charts_Get_Unique_ValuesParams):
    ...
    # function = await charts_connection_vault_routing(Charts_Connection_Vault_RoutingParams)
    charts_ins = Charts_Connection_Vault_RoutingParams(
        connection_id=data.connection_id, action="get_distinct_values"
    )
    function = await charts_connection_vault_routing(charts_ins)
    return await function(
...
async def create_charts(data: ChartsCreate):
    ...
    if not chart_data["status"]:
        return {
            "status": False,
            "message": str(chart_data["message"]),
...
async def drill_down_data(data: Charts_Drill_Down_DataParams):
    ...
    # function = await charts_connection_vault_routing(Charts_Connection_Vault_RoutingParams)
    charts_ins = Charts_Connection_Vault_RoutingParams(
        connection_id=data.connection_id, action="execute_query"
    )
    function = await charts_connection_vault_routing(charts_ins)
...
async def dashboard_charts(data: Charts_Dashboard_ChartsParams):
    ...
    return {
        "status": True,
        "message": "success",
        "data": await charts_functions.get_list_of_charts(),
    }
...
async def get_dashboard_chart_form(data: Charts_Get_Dashboard_Chart_FormParams):
    ...
    return {
        "status": True,
        "message": "success",
        "data": await charts_functions.get_chart_data(data.unique_id),
    }
...
async def charts_get_product_values(data: Charts_Get_Distinct_ValuesParams):
    ...
    # function = await charts_connection_vault_routing(Charts_Connection_Vault_RoutingParams)
    charts_ins = Charts_Connection_Vault_RoutingParams(
        connection_id=data.connection_id, action="get_product_values"
    )
    function = await charts_connection_vault_routing(charts_ins)
    ...
    if "SBU_Name" in [x.key for x in data.where_cond]:
        ...
        sbu_name = [
            x.value
            for x in data.where_cond
            if x.key == "SBU_Name" and x.value not in ["0", ""]
        ][0]
        ...
        if "data" in res_data:
            ...
            sorted_products = sorted(
                result_data,
                key=lambda x: (
                    retail_order.index(x) if x in retail_order else float("inf")
                ),
            )
...
async def charts_generate_dynamic_chart_query(
    data: Charts_Generate_Dynamic_Chart_QueryParams,
):
    """
    Description:
    ...
    redis_key = base64.b64encode(
        "_".join(data.query_context.lower().split()).encode()
    ).decode()
    ...
    # Encrypted key
    nlp_key = (
        "c2stcHJvai11NHNBWmJsTy1CWDVBaVZwUl95aEwwWjlzSkhJT0ZXV2NMU1FPTW9FaGxnLWZKd2NsZi0wQ3NuQ"
    ...
    input_context = {
        "model": "ft:davinci-002:personal::AKgLf2q5",
        "prompt": data.query_context,
        "temperature": 1,
        "max_tokens": 512,
        "top_p": 1,
        "frequency_penalty": 0,
    ...
    try:
        async with httpx.AsyncClient() as client:
            resp = await client.post(
                base_url, headers=headers, json=input_context, timeout=120
            )
            ...
            try:
                ...
                context_result["database"] = urdhva_base.settings.db_urls[
                    "postgres_async"
                ][0].path.split("/")[-1]
                ...
                print(
                    f"Exception while decoding response {e}, Traceback: {traceback.format_exc()}"
                )
                return False, (
                    "Error: The response text is not a valid JSON object. "
        ...
        print(
            f"Exception while running openai api {e}, Traceback: {traceback.format_exc()}"
        )
...
async def charts_save_charts(data: Charts_Save_ChartsParams):
    ...
    created_user = (
        rpt.get("given_name", "hpcl_ceg") + " " + rpt.get("family_name", "Engine")
    )
    ...
    filtered_fields = {
        ...
        for field in fields_included_in_hash
...
async def charts_get_dashboard_chart_form(
    data: Charts_Get_Dashboard_Chart_FormParams,
): ...
...
async def charts_connection_vault_routing(data: Charts_Connection_Vault_RoutingParams):
    ...
    module_path = (
        f"orchestrator.connection_vault."
...
async def charts_get_creds_details(data: Charts_Get_Creds_DetailsParams):
    ...
    try:
        ...
        return {
            "cred_model": creds_details["cred_model"],
            "cred_type": creds_details["cred_type"],
        }
    except Exception as e:
        raise ValueError(e)
...
async def charts_get_schema(data: Charts_Get_SchemaParams):
    ...
    # function = await charts_connection_vault_routing(Charts_Connection_Vault_RoutingParams)
    charts_ins = Charts_Connection_Vault_RoutingParams(
        connection_id=data.connection_id, action="get_schema"
    )
    function = await charts_connection_vault_routing(charts_ins)
    return await function(schema_name=data.schema)
...
async def charts_generate_vis_data(data: Charts_Generate_Vis_DataParams):
    ...
    return await widget_actions.WidgetActions.execute_widget_action(
        data.action,
...
async def charts_previous_present_month_sales(
    data: Charts_Previous_Present_Month_SalesParams,
):
    cross_filters = data.cross_filters
    limit = data.limit
    time_grain = data.time_grain
    ...
    if cross_filters:
        ...
        present_month_sales = await widget_actions.WidgetActions.apply_filter_drilldown(
            present_month_sales, access_filters, drill_state
        )
        present_month_sales += f" {sort_by}"
        if limit:
            present_month_sales += f" LIMIT {limit}"
...
async def charts_sales_drop_down(data: Charts_Sales_Drop_DownParams):
    ...
    if filters:
        ...
        for filt in filters:
            _filters.append(
                f""" {filt.key} in ({', '.join([f"'{i}'" for i in filt.value])}) """
            )
...
async def charts_previous_present_month_sales_by_product(
    data: Charts_Previous_Present_Month_Sales_By_ProductParams,
):
    cross_filters = data.cross_filters
    limit = data.limit
    time_grain = data.time_grain
...
async def charts_previous_present_month_amount_litres(
    data: Charts_Previous_Present_Month_Amount_LitresParams,
):
    cross_filters = data.cross_filters
    limit = data.limit
    time_grain = data.time_grain
    ...
    function = await charts_connection_vault_routing(charts_ins)
    present_month_sales = lpg_plant_queries.lpg_plant_query.get(
        "i_previous_current_month_amount_litres"
    )
    product_mapper = {
        "MS": "2811000",
        "HSD": "2812000",

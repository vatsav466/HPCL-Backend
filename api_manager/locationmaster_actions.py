...
async def locationmaster_upload_location_master(
...
async def locationmaster_download_location_master(
    data: Locationmaster_Download_Location_MasterParams,
):
...
async def locationmaster_fetch_global_stats(
    data: Locationmaster_Fetch_Global_StatsParams,
): ...


# Action download_template
@router.post("/download_template", tags=["LocationMaster"])
async def locationmaster_download_template(
    data: Locationmaster_Download_TemplateParams,
):
    """
    ...
    template_file_path = os.path.join(
        download_path, "templates", "location_master_template.csv"
    )
...
async def locationmaster_upload_tags_data(
    upload_file: fastapi.UploadFile = fastapi.File(None),
):
    """
    Upload Tags Data.
    ...
    try:
        ...
        return {
            "status": False,
            "message": "Invalid JSON file. Please upload a valid JSON.",
        }

    except Exception as e:
        logger.error(
            f"An error occurred while processing the JSON file: {traceback.format_exc()}"
        )
...
async def locationmaster_update_location_master(
    request: fastapi.Request, data: Locationmaster_Update_Location_MasterParams
):
    try:
        ...
        return {
            "status": False,
            "message": "No data found for the given sap_id",
            "data": [],
        }
...
async def locationmaster_get_sod_engineering_stats(
    data: Locationmaster_Get_Sod_Engineering_StatsParams,
):
...
async def locationmaster_location_command_control(
    data: Locationmaster_Location_Command_ControlParams,
):
    ...
    return await tas_command_control.publish_command(
        data.sap_id, data.action, bu, location_name, user_name, employee_id, "1"
    )


# Action get_dist_loc_details
@router.post("/get_dist_loc_details", tags=["LocationMaster"])
async def locationmaster_get_dist_loc_details(
    data: Locationmaster_Get_Dist_Loc_DetailsParams,
):
    return await sod_location_stats.get_filtered_location_data(
        bu=data.bu,
        location_onboard=data.location_onboard,
        specific_zone=data.zone,
        specific_sap_id=data.plant,
    )
...
async def locationmaster_get_location_metadata(
    data: Locationmaster_Get_Location_MetadataParams,
):

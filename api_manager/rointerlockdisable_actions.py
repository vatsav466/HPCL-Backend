...
async def rointerlockdisable_get_service_request_raise_details(
    data: Rointerlockdisable_Get_Service_Request_Raise_DetailsParams,
):
    return {
        "status": True,
        "message": "Success",
...
async def rointerlockdisable_submit_service_request(
    data: Rointerlockdisable_Submit_Service_RequestParams,
):
    entry = data.__dict__
    ...
    camunda_url = await helpers.get_camunda_url(
        bu=entry["location_type"], sap_id=entry["sap_id"], alert_section="RO"
    )
    _, alert_data = await alert_manager.create_alert(

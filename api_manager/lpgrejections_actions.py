...
async def lpgrejections_get_rejections(data: Lpgrejections_Get_RejectionsParams):
    ...
    cs_query = """ select * from cs_rejections """
    gd_query = """ select * from gd_rejections """
    pt_query = """ select * from pt_rejections """    
    if not data.days == 0:
        start_date = (datetime.datetime.now() - datetime.timedelta(days=data.days)).strftime('%Y-%m-%d')
        cs_query = f""" select * from cs_rejections WHERE process_date >= '{start_date}' """
        gd_query = f""" select * from gd_rejections WHERE process_date >= '{start_date}' """
        pt_query = f""" select * from pt_rejections WHERE process_date >= '{start_date}' """
    ...
    cs = cs.group_by(group_col
            ).agg(pl.col("rejection").mean().round(2) *100).rename({"rejection": "cs_rejection"})
    gd = gd.group_by(group_col
            ).agg(pl.col("rejection").mean().round(2) *100).rename({"rejection": "gd_rejection"})
    pt = pt.group_by(group_col
            ).agg(pl.col("rejection").mean().round(2) *100).rename({"rejection": "pt_rejection"})

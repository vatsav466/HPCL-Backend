...
async def lpgrejections_get_cs_rejections(
    data: Lpgrejections_Get_Cs_RejectionsParams,
): ...


# Action get_gd_rejections
@router.post("/get_gd_rejections", tags=["LpgRejections"])
async def lpgrejections_get_gd_rejections(
    data: Lpgrejections_Get_Gd_RejectionsParams,
): ...


# Action get_pt_rejections
@router.post("/get_pt_rejections", tags=["LpgRejections"])
async def lpgrejections_get_pt_rejections(
    data: Lpgrejections_Get_Pt_RejectionsParams,
): ...
...
async def lpgrejections_get_rejections(data: Lpgrejections_Get_RejectionsParams):
    ...
    if not data.days == 0:
        start_date = (
            datetime.datetime.now() - datetime.timedelta(days=data.days)
        ).strftime("%Y-%m-%d")
    ...
    # CS rejection
    resp = await function(query=cs_query)
    cs = pl.DataFrame(resp)
    # GD rejection
    resp = await function(query=gd_query)
    gd = pl.DataFrame(resp)
    # PT rejection
    resp = await function(query=pt_query)
    ...
    cs = (
        cs.group_by(group_col)
        .agg(pl.col("sortoutpercentage").mean().round(2) * 100)
        .rename({"sortoutpercentage": "cs_rejection"})
    )
    gd = (
        gd.group_by(group_col)
        .agg(pl.col("sortoutpercentage").mean().round(2) * 100)
        .rename({"sortoutpercentage": "gd_rejection"})
    )
    pt = (
        pt.group_by(group_col)
        .agg(pl.col("sortoutpercentage").mean().round(2) * 100)
        .rename({"sortoutpercentage": "pt_rejection"})
    )
    ...
    rejections = rejections.with_columns(
        (
            pl.col("cs_rejection") + pl.col("gd_rejection") + pl.col("pt_rejection")
        ).alias("TotalRejections")
    )
    if data.daywise == True:

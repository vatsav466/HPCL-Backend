...
async def users_fetch_users(data: Users_Fetch_UsersParams):
    params = queryparams.QueryParams(
        search_text=data.search_string, limit=data.limit, skip=data.skip
    )
    # return await Users.
...
async def users_create_user(data: Users_Create_UserParams):
    return await auth_manager.AuthenticationManager.create_user(
        data.username,
        data.password,
        data.role,
        data.first_name,
...
async def users_login(request: fastapi.Request, data: Users_LoginParams):
    status, resp, user_info = await auth_manager.AuthenticationManager.login(
        data.username, data.password, data.login_type
    )
    if not status:
        ...
        user_agent = await parse_device_info(
...
async def users_applogin(data: Users_ApploginParams):
    status, resp, user_info = await auth_manager.AuthenticationManager.login(
        data.username, data.password, data.login_type, jwt_auth=True
    )
    if not status:
        response = fastapi.responses.JSONResponse({"status": False, "msg": resp}, 401)
    else:
...
async def users_update_user_status(
    request: fastapi.Request, data: Users_Update_User_StatusParams
):
...
async def sso_auth_callback(
    request: fastapi.Request,
    code: typing.Optional[str] = None,
    entity_id: typing.Optional[str] = "",
):
    status, resp, user_info = await saml_validation.auth_callback(
        code,
        urdhva_base.settings.saml_tenant_id,
...
async def sso_auth_url(request: fastapi.Request):
    return await saml_validation.get_redirect_url(
        urdhva_base.settings.saml_tenant_id,
        urdhva_base.settings.saml_client_id,
        urdhva_base.settings.saml_client_secret,
        urdhva_base.settings.saml_redirect_uri,

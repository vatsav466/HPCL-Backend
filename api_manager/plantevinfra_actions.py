...
async def plantevinfra_upload_plant_ev_file(file: fastapi.UploadFile):
    try:
        ...
        def extract_date(ws):
            for cell in ["F1", "G1", "F2", "G2"]:
                ...
                if isinstance(value, datetime):
                    ...
                    try:
                        return datetime.strptime(value.strip(), "%d/%b/%y").strftime(
                            "%d/%m/%Y"
                        )
                    except Exception:
                        continue
        ...
        df = df[
            ~(
                df["Omc"].astype(str).str.upper().str.contains("OMC")
        ...
        df_transformed = pd.DataFrame(
            {
                ...
                "ro_name": df["Ro Name"],
                "city": df["City"],
                "battery_swapping_stations": pd.to_numeric(
                    df.get("No. Of Battery Swapping Stations", 0), errors="coerce"
                )
                .fillna(0)
                .astype(int),
                "vehicle_charging_stations": pd.to_numeric(
                    df.get("No. Of Vehicle Charging Stations", 0), errors="coerce"
                )
                .fillna(0)
                .astype(int),
        ...
        return {
            ...
            "rows_processed": len(final_records),
        }
...
async def plantevinfra_get_plant_ev_count_infra(
    data: Plantevinfra_Get_Plant_Ev_Count_InfraParams,
):
    return await plant_retail_functions.get_plant_ev_count_info(
...
async def plantevinfra_get_all_ev_infra(data: Plantevinfra_Get_All_Ev_InfraParams):
    return await plant_retail_functions.get_all_ev_info(
        filters=data.filters,
        cross_filters=data.cross_filters,
        drill_state=data.drill_state,
        limit=data.limit,
...
async def plantevinfra_get_ev_company_infra(
    data: Plantevinfra_Get_Ev_Company_InfraParams,
):
    return await plant_retail_functions.get_ev_company_info(
        filters=data.filters,
        cross_filters=data.cross_filters,
        drill_state=data.drill_state,
        limit=data.limit,
...
async def plantevinfra_get_distinct_ev_retail_infra(
    data: Plantevinfra_Get_Distinct_Ev_Retail_InfraParams,
):
    return await plant_retail_functions.get_distinct_ev_retail_info(
        data.sbu, data.company, data.zone, data.state, data.status
    )


# Action get_zone_wise_ev_infra
@router.post("/get_zone_wise_ev_infra", tags=["PlantEvInfra"])
async def plantevinfra_get_zone_wise_ev_infra(
    data: Plantevinfra_Get_Zone_Wise_Ev_InfraParams,
):
    return await plant_retail_functions.get_zone_wise_ev_info(
        filters=data.filters,
        cross_filters=data.cross_filters,
        drill_state=data.drill_state,
        limit=data.limit,

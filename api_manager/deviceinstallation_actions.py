    ...
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            response = await client.post(
                urdhva_base.settings.commisioning_url,
                json=payload,
...
async def call_decommissioning_api(payload: dict):
    async with httpx.AsyncClient(timeout=60) as client:
        try:
            response = await client.post(
                urdhva_base.settings.decommisioning_url,
                json=payload,
                headers={"Content-Type": "application/json"},
            )
            if response.status_code // 100 != 2:
...
async def deviceinstallation_validate_aot_details(
    data: Deviceinstallation_Validate_Aot_DetailsParams,
):
    """
    ...
    try:
        ...
        if not comm_success or status_code == 1:
            ...
            return {
                "status": False,
                "message": f"Device installation failed {status_messages}",
                "data": {
                    "device_id": device_id,
                    "commissioning": {
                        "success": False,
        ...
        return {
            "status": False,
            "message": f"Failed to process device installation: {str(e)}",
            "data": {},
        }
...
async def update_device_installation(
    ...
    try:
        ...
        if not rows:
            return {"status": False, "message": "Not able to update to the record"}
        ...
        if certificate_file:
            upload_dir = os.path.join(
                urdhva_base.settings.uploads, "device_installation"
            )
            ...
            status, minio_path = minio_connector.upload_to_minio(
                "VTS", "Device_Installation_Certificates", sap_tt_no, file_path
            )
...
async def deviceinstallation_action_device_vts(payload: dict):
    ...
    try:
        ...
        if not rows:
            return {
                "status": False,
                "message": f"No record found for SAP TT No",
                "data": [],
            }
        ...
        return {
            ...
            "data": {
                "commissioning": {"success": comm_success, "response": comm_response}
            },
        }
...
async def deviceinstallation_action_decommissioning(payload: dict):
    ...
    try:
        ...
        if reason_for_cancel and not status_decommissioning:
            ...
            record_dict = (
                new_record if isinstance(new_record, dict) else new_record.__dict__
            )
            # new_device_id = record_dict.get("id")

            return {
                "status": True,
                "message": "decommissioning requested",
                "data": {
            ...
            status_decommissioning = (
                "Approved" if status_decommissioning == "Accepted" else "Rejected"
            )
            ...
            if status_decommissioning == "Approved":
                ...
                comm_success, comm_response = await call_decommissioning_api(
                    de_comm_payload
                )
                ...
                return {
                    "status": True,
                    "message": f"Status updated to {status_decommissioning}",

...
async def performanceindex_get_pi_score(data: Performanceindex_Get_Pi_ScoreParams):
    ...
    try:
        ...
        resp = (
            await widget_actions.WidgetActions().generate_filter_clause(data.filters)
            if data.filters
            else ""
        )
        ...
        if data.bu in ["TAS", "LPG", "RO"]:
            ...
            ).get_aggr_data(query, limit=100000)
            print(query)
            ...
            if is_plant:
                ...
                for rec in score_data["data"]:
                    ...
                    if sid not in sap_scores:
                        ...
                        sap_scores[sid]["name"] = sap_scores[sid]["name"] or rec.get(
                            "name"
                        )
                        sap_scores[sid]["region"] = sap_scores[sid][
                            "region"
                        ] or rec.get("region")
                        sap_scores[sid]["zone"] = sap_scores[sid]["zone"] or rec.get(
                            "zone"
                        )

                    sap_scores[sid]["total_scores"].append(
                        safe_float(rec.get("score", 0))
                    )
                    ...
                    for category_ in rec.get("category", []):
                        ...
                        cat_det = sap_scores[sid]["category_details"].setdefault(
                            cat_name,
                            {
                                "score_sum": 0.0,
                                "weight_sum": 0.0,
                        ...
                        cat_det["weight_sum"] += safe_float(
                            category_.get("weightage", 0)
                        )
                        cat_det["count"] += 1
                        ...
                        for res in category_.get("results", []):
                            ...
                            if not res_name:
                                continue
                            res_det = cat_det["results"].setdefault(
                                res_name,
                                {
                                    "score_sum": 0.0,
                                    "weight_sum": 0.0,
                ...
                for sid, details in sap_scores.items():
                    ...
                    if total_scores:
                        ...
                        full_category = []
                        for cat_name, cat_det in details.get(
                            "category_details", {}
                        ).items():
                            ...
                            c_obj = {
                                "name": cat_name,
                                "score": (
                                    round(cat_det["score_sum"] / c_count, 2)
                                    if c_count
                                    else 0
                                ),
                                "weightage": (
                                    round(cat_det["weight_sum"] / c_count, 2)
                                    if c_count
                                    else 0
                                ),
                                "results": [],
                            ...
                            for res_name, res_det in cat_det["results"].items():
                                ...
                                r_obj = {
                                    "name": res_name,
                                    "score": (
                                        round(res_det["score_sum"] / r_count, 2)
                                        if r_count
                                        else 0
                                    ),
                                    "weightage": (
                                        round(res_det["weight_sum"] / r_count, 2)
                                        if r_count
                                        else 0
                                    ),
                                }
                ...
                temp_result.sort(
                    key=lambda x: (-x["overall_oi_score"], str(x["sap_id"]))
                )
                rank = 1
                prev_score = None
...
async def performanceindex_get_pi_score_by_category(
    data: Performanceindex_Get_Pi_Score_By_CategoryParams,
): ...

...
class ai_textsSchema(UrdhvaPostgresBase):
...
class ai_textsCreate(urdhva_base.postgresmodel.BasePostgresModel):
    __tablename__ = 'ai_texts'
    ...
    class Config:
        ...
        if urdhva_base.settings.disable_api_extra_inputs:
            extra = "forbid"  # Disallow extra fields
        schema_class = ai_textsSchema
        upsert_keys = []


class ai_texts(urdhva_base.postgresmodel.PostgresModel):
    __tablename__ = 'ai_texts'
...
class ai_textsGetResp(pydantic.BaseModel):
    data: typing.List[ai_texts]
    total: int = pydantic.Field(0)
    count: int = pydantic.Field(0)

...
async def vts_ingest_data_blocked_trucks(data: Vts_Ingest_Data_Blocked_TrucksParams):
    ...
    try:
        logger.info(
            f"Received VTS data ingestion from TT Blocked {data.location_id}({data.location_type}) {data.dict()}"
        )
        return True, "Success"
    except Exception as e:
        print(traceback.format_exc())
...
async def vts_ingest_data_un_blocked_trucks(
    data: Vts_Ingest_Data_Un_Blocked_TrucksParams,
):
    """
    Args:
        data:
    ...
    try:
        ...
        for unlock_data in data.data:
            ...
            unlock_data["vehicle_blocked_start_date"] = datetime.datetime.strptime(
                unlock_data["vehicle_blocked_start_date"], "%Y-%m-%d %H:%M:%S"
            ).strftime("%Y-%m-%d")
            tl_number = unlock_data["tt_no"]
            query = (
                f"select id from alerts where vehicle_number = '{tl_number}' "

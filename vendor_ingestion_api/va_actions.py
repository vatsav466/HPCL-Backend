...
async def va_ingest_data(data: Va_Ingest_DataParams):
    ...
    try:
        logger.info(
            f"Received VA data ingestion from vendor {data.location_id}({data.location_type}) {data.dict()}"
        )
            ...
            enriched_data = [
                {
                    **entry.dict(),
                    ...
                    "location_type": (
                        ...
                        if hasattr(data.location_type, "value")
                        else str(data.location_type)
                    ),
                }
        ...
        for entry in enriched_data:
            ...
            entry["alert_timestamp"] = datetime.datetime.strptime(
                entry["alert_timestamp"], "%m/%d/%Y %I:%M:%S %p"
                ...
                if diff.total_seconds() >= 2 * 3600:
                    if not await va_analysis.is_alert_exists(entry["alert_id"]):
                        ...
                        await alert_manager.create_alert(
                            {**entry, "alert_type": "VA"}, camunda_url=camunda_url
                        )
                    else:
                        return True, "Success"
                else:
                    ...
                    if "camunda_host" in entry.keys():
                        camunda_url = (
                            f"http://{entry['camunda_host']}:{entry['camunda_port']}"
                        )
                    await alert_manager.create_alert(
                        {**entry, "alert_type": "VA"}, camunda_url=camunda_url
                    )
...
async def va_ingest_data_close(data: Va_Ingest_Data_CloseParams):
    try:
        ...
        if va_alert.get("data", []):
            ...
            va_alert["closed_at"] = datetime.datetime.strptime(
                data.closed_at, "%m/%d/%Y %I:%M:%S %p"
            ) + datetime.timedelta(hours=5, minutes=30)
        ...
        if alert_data1.get("data", []):
            return {
                "status": True,
                "message": "Alert already Closed in Novex",
                "data": [],
            }
        ...
        alert_data["alert_history"].append(
            {
                ...
                "alert_status": "Close",
                "action_msg": data.action_code or data.status,
                "remarks": data.action_description,
        ...
        camunda_url = await helpers.get_camunda_url(
            bu=alert_data["bu"], sap_id=alert_data["bu"], alert_section="VA"
        )
...
async def va_ro_no_video_upload_list(data: Va_Ro_No_Video_Upload_ListParams):
    try:
        logger.info(
            f"Received No Video Upload data ingestion from vendor {data.model_dump()}"
        )
        print("*" * 200)
        ro_cleanliness_data = data.model_dump()
        print(ro_cleanliness_data)
        print("*" * 200)
        await ro_va_alert_handler.ROVaAlertHandler().ro_cleanliness_master_data(
            ro_cleanliness_data.get("data", [])
        )
        return {"status": True, "message": "Success", "data": []}
    except Exception as e:
        print(traceback.format_exc())
...
async def va_ro_no_video_upload_list_status(
    data: Va_Ro_No_Video_Upload_List_StatusParams,
):
    try:
        logger.info(
            f"Received No Video Upload Status data ingestion from vendor {data.model_dump()}"
        )
        ro_cleanliness_status_data = data.model_dump()
        print("*" * 200)
        print(ro_cleanliness_status_data["data"])
        print("*" * 200)
        await ro_va_alert_handler.ROVaAlertHandler().ro_cleanliness_uploaded_master_data(
            ro_cleanliness_status_data.get("data", [])
        )
        return {"status": True, "message": "Success", "data": []}
    except Exception as e:
        print(traceback.format_exc())

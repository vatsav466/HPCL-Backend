...
class TelemetryService:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    telemetry_url, json=payload, headers=headers, ssl=False
                ) as response:
                    if response.status == 200:
                        ...
                        print(
                            f"Failed to post telemetry for deviceKey: {device_key}. Status code: {response.status}"
                        )
                        return False
        except Exception as e:
            print(
                f"Error posting telemetry for deviceKey: {device_key}. Error: {str(e)}"
            )
            return False
    ...
    async def process_tags_data(location_id, tags_data, site_id):
        ...
        if location_id != site_id:
            print(
                f"Location ID {location_id} does not match SITE_ID {site_id}. Ignoring message."
            )
            return False
            ...
            for component in site_data["data"]:
                ...
                for sensor in component["sensors"]:
                    ...
                    if tag_path in tags_data:
                        ...
                        tasks.append(
                            TelemetryService.post_telemetry(
                                device_key, sensor["sensor_name"], value
                            )
                        )  # pass thingsboard_url by calling get_thingsboard_url function if required
                    # else:
...
async def rabbitmq_listener(sap_id):
    ...
    while True:
        try:
                ...
                async for message in queue:
                    async with message.process():
                        try:
                            ...
                            success = await TelemetryService.process_tags_data(
                                location_id, tags_data, SITE_ID
                            )
                            if success:
                                print(
                                    f"All tags processed successfully for {queue_name}."
                                )
                            else:
            ...
            print(
                f"Connection lost for {queue_name}. Reconnecting in 5 seconds... Error: {e}"
            )
...
async def main():
    sap_ids = [
        ...
        "1636",
        "1845",
    ]  # Add all the SAP IDs you want to listen for

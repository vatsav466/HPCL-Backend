...
def get_and_insert_data(cursor, query, params=None):
    ...
    cursor.execute(f"""
SELECT *
FROM PS.EDW_PRIM_SLS_FCT
WHERE SLS_TRNS_DT BETWEEN '2023-04-01' AND '2023-04-30'
    """)
    print("query ran")
    his_data = cursor.fetchall()
    print("got data from query")
    his_columns = [column[0] for column in cursor.description]
    print("got columns from query")
    print(len(his_data))
    data_his = pd.DataFrame.from_records(his_data, columns=his_columns)
    data_his.to_csv('/tmp/data_his.csv',index = False) 
    cursor.execute("select * FROM PS.EDW_CUST_DIM")
    dim_data = cursor.fetchall()
    dim_columns = [column[0] for column in cursor.description]
    data_dim   = pd.DataFrame.from_records(dim_data, columns=dim_columns)
    
    print(len(data_his))
    print(len(data_dim))
    data_dim.to_csv('/tmp/data_dim.csv',index = False)
    exit()
    print("-" * 50)
    print("Running Query ...")

...
class CheckRegularViolation:
    ...
    async def check_regular(self, params):
        ...
        try:
            ...
            last_notified_to = ", ".join(
                f"'{roles_name}'"
                for roles_name in alert_data.get("last_notified_to", [])
            )
            if not params["regular"]:
                ...
                if alert_history[0]["action_type"] in ["Approved"]:
                    if alert_history[2]["action_type"] in ["Justification"]:
                        alert_message = f"Justified violation approved by {last_notified_to}. The violation will be considered a false violation and the violation has been moved to the Close tab."
                    if alert_history[2]["action_type"] in ["FalseViolation"]:
                        alert_message = f"False violation allegation approved by {last_notified_to}. The violation will be considered a false violation and the violation has been moved to the Close tab."
                    alert_data["action_msg"] = alert_message
                    alert_data["action_type"] = "Resolved"
                    await alert_manager.AlertAction().update_alert_history_vts(
            ...
            if params["regular"]:
                alert_message = f"The violation will be considered a true violation and the violation has been moved to the Close tab."
                if alert_history[0]["action_type"] in ["AcceptViolation"]:
                    alert_message = f"Violation accepted by {last_notified_to}. The violation will be considered a true violation and the violation has been moved to the Close tab."
                if alert_history[0]["action_type"] in ["Rejected"]:
                    if alert_history[2]["action_type"] in ["Justification"]:
                        alert_message = f"Justified violation rejected by {last_notified_to}. The violation will be considered a true violation and the violation has been moved to the Close tab."
                    if alert_history[2]["action_type"] in ["FalseViolation"]:
                        alert_message = f"False violation allegation rejected by {last_notified_to}. The violation will be considered a true violation and the violation has been moved to the Close tab."
                alert_data["action_msg"] = alert_message
                alert_data["action_type"] = "Resolved"
                await alert_manager.AlertAction().update_alert_history_vts(
                    input_data=alert_data, alert_data=alert_data

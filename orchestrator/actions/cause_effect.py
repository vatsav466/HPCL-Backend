...
class CauseEffect:
    ...
    async def handle_cause_alert(self, params):
        ...
        if isinstance(effect_sop_id, list):
            effect_sop_id = ",".join(
                [f"'{item.strip('[]')}'" for item in effect_sop_id]
            )
        ...
        effect_alerts = await hpcl_ceg_model.Alerts.get_all(
            query_params, resp_type="plain"
        )
        processed_time = datetime.datetime.now(datetime.timezone.utc)

        # Get the cause alert to update its history
        cause_alert = (
            await hpcl_ceg_model.Alerts.get(cause_alert_id) if cause_alert_id else None
        )
        existing_cause_history = (
            getattr(cause_alert, "alert_history", []) or [] if cause_alert else []
        )
            ...
            if not cause_exists:
                new_effect_entry = {
                    ...
                    "allocated_time": (
                        ...
                        else last_processed_time.isoformat()
                    ),
                ...
                effect_data_obj = hpcl_ceg_model.Alerts(
                    id=effect_alert_id, alert_history=updated_effect_history
                )
        ...
        if cause_alert and existing_cause_history:
            cause_data_obj = hpcl_ceg_model.Alerts(
                id=cause_alert_id, alert_history=existing_cause_history
            )
    ...
    async def handle_effect_alert(self, params):
        ...
        cause_alerts = await hpcl_ceg_model.Alerts.get_all(
            query_params, resp_type="plain"
        )
        processed_time = datetime.datetime.now(datetime.timezone.utc)
        ...
        if cause_alerts["data"]:
            ...
            for cause_alert in cause_alerts["data"]:
                ...
                related_cause_interlock_name = cause_alert.get(
                    "interlock_name", "Unknown Cause"
                )
                ...
                if not effect_exists:
                    ...
                    cause_data_obj = hpcl_ceg_model.Alerts(
                        id=cause_alert_id, alert_history=updated_cause_history
                    )
                    await cause_data_obj.modify()
        ...
        if effect_alert_id:
            ...
            if effect_alert:
                existing_effect_history = (
                    getattr(effect_alert, "alert_history", []) or []
                )
                last_processed_time = processed_time
    ...
    async def cause_effect_alert(self, params):
        ...
        try:
            if params.get("cause_effect") == "Cause":
                ...
                logger.warning(
                    f"Unknown Cause_Effect value: {params.get('Cause_Effect')}"
                )

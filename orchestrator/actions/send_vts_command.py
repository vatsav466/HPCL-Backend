...
class SendVtsCommand:
    ...
    async def sendvtscommand(self, params):
        ...
        if params.get("interrupt").lower() == "block":
            ...
            if (
                alert_data["bu"] in ["TAS"]
                and alert_data.get("tt_type", "").lower() in ["bulk"]
            ) or (
                alert_data["bu"] in ["LPG"]
                and alert_data.get("tt_type", "").lower() in ["packed"]
            ):
                ...
                payload = [
                    {
                        ...
                        "blockingTo": (
                            alert_data["vehicle_blocked_end_date"]
                            + datetime.timedelta(hours=5, minutes=30)
                        ).strftime("%Y%m%d"),
                    }
                ]
                blocking_status, error_msg = await vts_analysis.post_blocked_tt_ims(
                    payload
                )

                if not blocking_status:
                    logger.error(
                        f"Blocking Payload Not posted to IMS {alert_data['unique_id']} {alert_data['id']}"
                    )
                    alert_message = f"{error_msg}"
                    ...
                    blocking_status
                    and isinstance(blocking_status, list)
                    and blocking_status[0]["successFlag"] not in ["Y"]
                ):
                    logger.error(
                        f"Blocking Payload Not posted to IMS {alert_data['unique_id']} {alert_data['id']}"
                    )
                    alert_message = f"{blocking_status[0]['message']}"
                    alert_data["action_msg"] = alert_message
                    alert_data["action_type"] = "BlockFailed"
                    await alert_manager.AlertAction().update_alert_history(
                        input_data=alert_data, alert_data=alert_data
                    ...
                    alert_message = f"{blocking_status.get('message','Blocking Payload Not posted to IMS')}"
                    alert_data["action_msg"] = alert_message
                    alert_data["action_type"] = "BlockFailed"
                    await alert_manager.AlertAction().update_alert_history(
                        input_data=alert_data, alert_data=alert_data
            ...
            if alert_data["bu"] in ["LPG"] and alert_data.get(
                "tt_type", ""
            ).lower() in ["bulk"]:
                ...
                if not blocking_status:
                    logger.error(
                        f"Blocking Payload Not posted to SAP {alert_data['unique_id']} {alert_data['id']}"
                    )
                    alert_message = f"{error_msg}"
                    alert_data["action_msg"] = alert_message
                    alert_data["action_type"] = "BlockFailed"
                ...
                if (
                    ...
                    and blocking_status.get("Response", {}).get("Remark")
                    not in ["Request already processed with the given Request ID"]
                ):
            ...
            alert_message = f"Alert details Alert ID: {alert_data.get('unique_id', '')}, status: Block, Vehicle: {alert_data.get('vehicle_number', '')} trip details are sent successfully to VTS to block the Vehicle "
            alert_data["action_msg"] = alert_message
        ...
        if params.get("interrupt").lower() == "unblock":
            ...
            if (
                ...
                if not unblocking_status:
                    logger.error(
                        f"UnBlocking Payload Not posted to IMS {alert_data['unique_id']} {alert_data['id']}"
                    )
                    alert_message = f"{error_msg}"
                    alert_data["action_msg"] = alert_message
                    ...
                    unblocking_status
                    and isinstance(unblocking_status, list)
                    and unblocking_status[0]["successFlag"] not in ["Y"]
                ):
                    logger.error(
                        f"UnBlocking Payload Not posted to IMS {alert_data['unique_id']} {alert_data['id']}"
                    )
                    alert_message = f"{unblocking_status[0]['message']}"
                    alert_data["action_msg"] = alert_message
                    alert_data["action_type"] = "UnblockFailed"
                    await alert_manager.AlertAction().update_alert_history(
                        input_data=alert_data, alert_data=alert_data
                    ...
                    alert_message = f"{unblocking_status.get('message','UnBlocking Payload Not posted to IMS')}"
                    alert_data["action_msg"] = alert_message
                    alert_data["action_type"] = "UnblockFailed"
                    await alert_manager.AlertAction().update_alert_history(
                        input_data=alert_data, alert_data=alert_data
            ...
            if alert_data["bu"] in ["LPG"] and alert_data.get(
                ...
                if not unblocking_status:
                    logger.error(
                        f"UnBlocking Payload Not posted to SAP {alert_data['unique_id']} {alert_data['id']}"
                    )
                    alert_message = f"{error_msg}"
                    alert_data["action_msg"] = alert_message
                    alert_data["action_type"] = "UnblockFailed"
                ...
                if (
                    ...
                    and unblocking_status.get("Response", {}).get("Remark")
                    not in [
                        "Request already processed with the given Request ID",
                        "Vehicle is already unblocked in SAP",
                    ]
                ):
                    logger.error(
                        f"UnBlocking Payload Not posted to SAP {alert_data['unique_id']} {alert_data['id']}"
            ...
            if not params["auto_unblock"]:
                if alert_data["interlock_name"] not in [
                    "No VTS No Load",
                    "Itdg Admin Blocked",
                ]:
                    query = (
                        f"location_id='{alert_data['sap_id']}' and tl_number='{alert_data['vehicle_number']}' "
                ...
                alert_message = f"Alert details Alert ID: {alert_data.get('unique_id', '')}, status: Unblock, Vehicle: {alert_data.get('vehicle_number', '')} trip details are sent successfully to VTS to Unblock the Vehicle "

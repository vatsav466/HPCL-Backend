    ...
    async def sendnozzlecommand(self, params):
        ...
        if params.get("interrupt").lower() == "block":
            ...
            (
                blocking_status,
                error_msg,
            ) = await ro_interlock_handler.RoInterlockHandler().ro_blocking(
                [alert_data.get("sap_id", "")]
            )
            ...
            if (
                ...
                and failed_resp
                and failed_resp[0].get(alert_data.get("sap_id", ""))
                in ["Outlet Not Communicating"]
            ):
                alert_message = f"Outlet Not Communicating"
                ...
                await hpcl_ceg_model.Alerts(
                    **{"id": alert_data["id"], "block_status": None, "ro_offline": True}
                ).modify()
                return True, {"blocked": False, "offline": True}
            elif not blocking_status:
                alert_message = f"Failed to Block the Outlet {alert_data.get('location_name', '')}, status: Failed, RO: {alert_data.get('sap_id', '')}"
                if failed_resp:
                ...
                alert_message = f"Succefully Blocked Outlet {alert_data.get('location_name', '')}, status: Block, RO: {alert_data.get('sap_id', '')} details are sent successfully to CRIS to block the Outlet"
                alert_data["action_msg"] = alert_message
        ...
        if params.get("interrupt").lower() == "unblock":
            ...
            closure_reason = (
                "PICTURE_UPLOADED"
                if alert_data.get("image_uploaded")
                else "DNC_UNBLOCKED"
            )
            (
                unblocking_status,
                error_msg,
            ) = await ro_interlock_handler.RoInterlockHandler().ro_unblocking(
                [alert_data.get("sap_id", "")]
            )
            success_resp, failed_resp = error_msg
            if (
                ...
                alert_message = f"Failed to Unblock the Outlet {alert_data.get('location_name', '')}, status: Failed, RO: {alert_data.get('sap_id', '')}"
                alert_data["action_msg"] = alert_message
                ...
                alert_message = f"Successfully Unblocked Outlet {alert_data.get('location_name', '')}, status: Unblock, RO: {alert_data.get('sap_id', '')} details are sent successfully to CRIS to unblock the Outlet "
                alert_data["action_msg"] = alert_message

...
class MakerCheckerAcknowledgment:
    ...
    async def maker_checker_justification(self, params):
        ...
        roles = ", ".join(
            f"'{roles_name}'" for roles_name in alert_data["assigned_user_roles"]
        )

        if params.get("vts_alert_justified", "") == "checker":
            alert_message = f"Vehicle UnBlocking Rejected by the checker {roles}"

        if params.get("vts_alert_justified") == "maker":
            alert_message = f"Vehicle Blocking Accepted by the maker {roles}"

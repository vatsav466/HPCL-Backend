...
class CloseVtsAssociatedAlert:
    ...
    async def closeWorkflow(self, original_altid):
        ...
        data = {
            ...
            "variables": {
                "alert_id": {"value": workflow_id, "type": "String"},
                "closed": {"value": True, "type": "Boolean"},
            },
        }
        r = httpx.post(
            url, headers={"Content-Type": "application/json"}, json=data, verify=False
        )
    ...
    async def closeVtsAssociatedAlt(self, params):
        ...
        for i in range(3):
            try:
                ...
                alert_data["role"] = ""
                alert_data["rolelist"] = ""
                alert_data["updated_at"] = datetime.datetime.utcnow().replace(
                    microsecond=0
                )
                ...
                print(
                    "Exception in closing the alert:%s AlertId:%s"
                    % (e, params.get("alert_id"))
                )

...
class CreateVTSBlockAlert:
    ...
    async def get_instance(
        self, tt_number: str, sap_id: str, bu: str, get_raw_data=False
    ):
        query = f"select * from vts_truck_details where truck_regno = '{tt_number}'"
        vts_truck_data = await hpcl_ceg_model.VtsTruckDetails.get_aggr_data(
            query, limit=0
        )
        if get_raw_data:
    ...
    async def get_vts_instance(
        self, tt_number: str, sap_id: str, bu: str, violation_type: str
    ):
        ...
        if vts_closed_alert_data:
            ...
            if start_date_ <= last_updated_at_ist.date() <= end_date_:
                query = (
                    f"select DISTINCT ON (vehicle_number, invoice_number) violation_type, id from violation_history_vts where vehicle_number = '{tt_number}' "
    ...
    async def create_bu_alert(
        self, alert_data, camunda_url=urdhva_base.settings.camunda_url
    ):
        """
        ...
        status, location_details = await alert_helper.get_location_details(
            alert_data["bu"], alert_data["sap_id"]
        )
        if not status:
            logger.info(
        ...
        vts_alert_data["vehicle_blocked_start_date"] = (
            urdhva_base.utilities.get_present_time() + datetime.timedelta(days=1)
        ).isoformat()
        days_to_add = (
            instance_data["block_duration"] + 1
            if not await self.is_vehicle_blacklisted(alert_data["vehicle_number"])
            else 1826
        )
        vts_alert_data[
    ...
    async def get_updated_vts_instance(
        self, tt_number: str, sap_id: str, bu: str, violation_type: str
    ):
        vts_map = vts_mapping.vts_interlock_mapping
        instance_mapping = vts_instance_mapping.instance_mapping
    ...
    async def update_alert_id_to_vts_history(
        self, alert_id: str, vts_alert_id: list[str]
    ):
        if vts_alert_id:
            ...
            query = (
                f"""update violation_history_vts set alert_id='{alert_id}' """
    ...
    async def update_vts_instance(self, alert_data):
        ...
        instance_data, violation_name, vts_alert_history_ids, alert_id = (
            await self.get_updated_vts_instance(
                alert_data["vehicle_number"],
                alert_data["sap_id"],
                alert_data["bu"],
                alert_data["violation_type"],
        ...
        await self.update_alert_id_to_vts_history(
    ...
    async def block_vehicle(self, params):
        ...
        try:
            # print("params --->", params)
            alert_data = await hpcl_ceg_model.ViolationHistoryVts.get(
                params.get("alert_id")
            )
            if not isinstance(alert_data, dict):
                alert_data = alert_data.__dict__
            ...
            if await self.is_vehicle_blacklisted(alert_data["vehicle_number"]):
                ...
                truck_history.append(
                    {
                        "violated_date": (
                            alert_data["vts_end_datetime"].isoformat()
                            if isinstance(
                                alert_data["vts_end_datetime"], datetime.datetime
                            )
                            else alert_data["vts_end_datetime"]
                        ),

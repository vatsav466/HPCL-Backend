...
class IndentDryOut:
        ...
        if not total_indent:
            ...
            for each_indent in total_indent:
                ...
                if alerts_data["data"]:
                    # If dry_out_days found diff, update alert
                    query = (
                        f"""update alerts set indent_no='{each_indent["INDENT_NO"]}', """
    ...
    async def generate_dry_out_history(cls, location_id, product_code, item_name):
        ...
        """
        query = (
            f"SELECT sap_id, product_no from dry_out_history where sap_id='{location_id}' "
    ...
    async def check_indent_status(self, params: dict):
        ...
        if not self.params.get("indent_no"):
            # TODO:
            workflow_date = datetime.datetime.strptime(
                self.params.get("workflow_datetime"), "%Y-%m-%dT%H:%M:%S.%fZ"
            )
            ...
            query = (
                f"""SELECT COUNT(*) AS "count", a."INDENT_NO" AS "INDENT_NO" , b."PROD" AS "PROD", a."LOCN_CODE" AS "LOCN_CODE", a."INDENT_DATE" AS "INDENT_DATE" """
                f"""FROM "IMS_SAP"."INDENT_REQUEST" a, "IMS_SAP"."INDENT_PRODUCTS" b WHERE SUBSTR(a."DEALER_CODE",1,10) = '{dealer_code}' """
                f"""AND a."PROD_REQD_DT" BETWEEN TO_DATE('{now}', 'YYYY-MM-DD') AND TO_DATE('{next_date}', 'YYYY-MM-DD') """
                f"""AND b."PROD" = '{prod_code}' """
                f"""AND a."LOCN_CODE" = b."LOCN_CODE" AND a."INDENT_NO" = b."INDENT_NO" AND a."CANCEL_INDENT" IS NULL """
            ...
            query = (
                f"""SELECT COUNT(*) AS "count", a."INDENT_NO" AS "INDENT_NO" , b."PROD" AS "PROD", a."LOCN_CODE" AS "LOCN_CODE" """
                f"""FROM "IMS_SAP"."INDENT_REQUEST" a, "IMS_SAP"."INDENT_PRODUCTS" b WHERE SUBSTR(a."DEALER_CODE",1,10) = '{dealer_code}' """
                f"""AND a."PROD_REQD_DT" BETWEEN TO_DATE('{now}', 'YYYY-MM-DD') AND TO_DATE('{next_date}', 'YYYY-MM-DD') """
                f"""AND b."PROD" = '{prod_code}' """
    ...
    async def is_invoice_created(self, params: dict):
        ...
        query = (
            f"""SELECT * FROM "IMS_SAP"."INDENT_PRODUCTS" where SUBSTR("DEALER_CODE",1,10) = '{dealer_code}' """
    ...
    async def close_supply_chain_alert(
        ...
        # )
        query = (
            f"""UPDATE "HPCL_HOS".sch_inventory_forecast_dashboard SET "indent_status" = 'Completed' """
        ...
        if camunda_url != "error":
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    ...
                    print(
                        f"Request error for {camunda_url} {instance_id} (attempt {attempt + 1}): {e}"
                    )
                    logger.info(
                        f"Request error for {camunda_url} {instance_id} (attempt {attempt + 1}): {e}"
                    )

...
class LpgRejections:
    ...
    async def get_current_cs_rejections(self):
        yesterday = (
            datetime.datetime.now(pytz.timezone("Asia/Kolkata")) - relativedelta(days=1)
        ).strftime("%Y-%m-%d")
        ...
        rejections = rejections.with_columns(
            ((pl.col("totalsortout") / pl.col("total")) * 100).alias("rejection")
        )
        rejections = rejections.with_columns(
            pl.lit("cs_rejections").alias("rejection_type")
        )
        rejections = rejections.sort("rejection").with_columns(
            pl.col("rejection")
            .fill_null(0)
            .cast(pl.Float64)
            .round(2)
        ...
        for data in rejections.iter_rows(named=True):
            ...
            self.params["message"] = (
                f"Check Scale rejection is going above 8%. The current rejection rate is {str(data['rejection'])}"
            )
            self.params["alert_history"] = [
                {"action_msg": self.params["message"], "action_type": "Created"}
            ]
            camunda_url = await helpers.get_camunda_url(
                bu=self.params["alert_type"],
                sap_id=self.params["sap_id"],
    ...
    async def get_current_gd_rejections(self):
        ...
        rejections = rejections.with_columns(
            pl.lit("gd_rejections").alias("rejection_type")
        )
        rejections = rejections.sort("rejection").with_columns(
            pl.col("rejection")
            .fill_null(0)
        ...
        rejections = rejections.filter(
            (pl.col("rejection") > 6) | (pl.col("rejection") < 1)
        )
        check_alerts = f""" SELECT 
                                sap_id, device_name, created_at
                            FROM
        ...
        for data in rejections.iter_rows(named=True):
            ...
            if float(data["rejection"]) < 1:
                self.params["message"] = (
                    f"Valve Leak rejection is going below 1%. The current rejection rate is {str(data['rejection'])}"
                )
            else:
                self.params["message"] = (
                    f"Valve Leak rejection is going above 6%. The current rejection rate is {str(data['rejection'])}"
                )
            self.params["alert_history"] = [
                {"action_msg": self.params["message"], "action_type": "Created"}
            ]
    ...
    async def get_current_pt_rejections(self):
        ...
        rejections = rejections.with_columns(
            pl.lit("pt_rejections").alias("rejection_type")
        )
        rejections = rejections.sort("rejection").with_columns(
            pl.col("rejection")
            .fill_null(0)
        ...
        for data in rejections.iter_rows(named=True):
            ...
            if float(data["rejection"]) < 1:
                self.params["message"] = (
                    f"O-Ring Leak rejection is going below 1%. The current rejection rate is {str(data['rejection'])}"
                )
            else:
                self.params["message"] = (
                    f"O-Ring Leak rejection is going below 6%. The current rejection rate is {str(data['rejection'])}"
                )
            self.params["alert_history"] = [
                {"action_msg": self.params["message"], "action_type": "Created"}
            ]
    ...
    async def check_rejections(self, params):
        ...
        if (
            check_alerts["location_name"][-1]
            in previous_rejection["location_name"].unique().to_list()
        ):
            return True, {"rejection_status": "increased"}
        elif (
            check_alerts["location_name"][-1]
            not in previous_rejection["location_name"].unique().to_list()
        ):

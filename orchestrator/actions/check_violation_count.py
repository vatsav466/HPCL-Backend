...
class CheckViolationCount:
    async def check_violation_count(
        self, sap_id, bu, vehicle_number, violation_type, created_at=None
    ):
        """
        ...
        try:
            # Construct the query
            query = (
                f"sap_id='{sap_id}' and bu='{bu}' and vehicle_number='{vehicle_number}' "
            ...
            count = await hpcl_ceg_model.Alerts.get_all(
    ...
    async def check_violation_all_count(
        self, sap_id, bu, vehicle_number, violation_type
    ):
        """
        This method is used to get the violation count for a given vehicle from the DB
    ...
    async def check_interlock(
        self, sap_id, bu, vehicle_number, interlockname, violation_type
    ):
        """
        This method is used to get the violation count for a given vehicle from the DB
        based on the given violation type, SAP ID, business unit, and vehicle number.
        ...
        try:
            ...
            data = await hpcl_ceg_model.Alerts.get_all(
                urdhva_base.queryparams.QueryParams(q=query), resp_type="plain"
            )
            if len(data["data"]):
                return data["data"][0]
    ...
    async def get_violation_period(self, duration_days=15):
        ...
        cycle_number = (
            days_difference // duration_days
        )  # Which 15-day cycle it belongs to

        # Compute the correct violation start and end dates
        violation_start = base_start + datetime.timedelta(
            days=cycle_number * duration_days
        )

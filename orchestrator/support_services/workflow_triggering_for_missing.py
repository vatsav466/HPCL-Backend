...
async def is_workflow_running(business_key: str, camunda_url: str) -> bool:
    ...
    params = {"businessKey": business_key, "active": "true"}
...
async def trigger_workflow_for_missing_data():
    ...
    for alert in alerts_data.get("data", []):
        ...
        prod_reqd_dt = ""
        payload = {
            "businessKey": alert.get("unique_id", ""),
            "variables": {
                "alert_id": {"value": alert["id"], "type": "String"},
                "interlock_name": {
                    "value": alert.get("interlock_name"),
                    "type": "String",
                },
                "interlock_id": {"value": alert.get("interlock_id"), "type": "String"},
                "location_device_id": {
                    "value": alert.get("device_id", ""),
                    "type": "String",
                },
                ...
                "product_code": {
                    "value": str(alert.get("product_code", "")),
                    "type": "String",
                },
                "workflow_datetime": {
                    "value": to_iso_string(alert.get("workflow_datetime")),
                    "type": "String",
                },
                "indent_no": {"value": alert.get("indent_no", ""), "type": "String"},
                "indent_raised_date": {
                    "value": to_iso_string(alert.get("indent_raised_date")),
                    "type": "String",
                },
                "terminal_plant_name": {
                    "value": alert.get("terminal_plant_name", ""),
                    "type": "String",
                },
                "prod_reqd_dt": {"value": prod_reqd_dt, "type": "String"},
                "va_level": {"value": alert_level, "type": "String"},
                "terminal_plant_id": {
                    "value": alert.get("terminal_plant_id", ""),
                    "type": "String",
                },
                "cause_effect": {
                    "value": alert.get("Cause_Effect", ""),
                    "type": "String",
                },
                "alert_section": {
                    "value": alert.get("alert_section", ""),
                    "type": "String",
                },
                "cause_sop_id": {
                    "value": alert.get("cause_sop_id", ""),
                    "type": "String",
                },
                "effect_sop_id": {
                    "value": alert.get("effect_sop_id", ""),
                    "type": "String",
                },
                ...
                "device_type": {
                    "value": alert.get("device_type", ""),
                    "type": "String",
                },
                "tas_device_name": {
                    "value": alert.get("tas_device_name", ""),
                    "type": "String",
                },
            },
        }
        ...
        if await is_workflow_running(
            business_key=alert.get("unique_id", ""), camunda_url=CAMUNDA_BASE_URL
        ):
            print(
                f"Workflow already running for businessKey={alert.get('unique_id', '')}, skipping trigger"
            )
            continue
        else:
            ...
            try:
                await Camunda().start_workflow(
                    payload, workflow_id, camunda_url=CAMUNDA_BASE_URL
                )
                print(f"Workflow triggered successfully for alert id {alert.get('id')}")
            except Exception as e:
                print(
                    f"Failed to trigger workflow for alert id {alert.get('id')}. Error: {str(e)}"
                )
    print("*" * 20)
    print(
        "Alerts for which workflow is not running and can be triggered:",
        not_running_alerts,
    )
    print("*" * 20)

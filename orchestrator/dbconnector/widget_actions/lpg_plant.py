...
class LPGPlantActions:
    ...
    async def get_production_details(filters, drill_state):
        ...
        if filters:
            production_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    production_query, filters, drill_state
                )
            )
        else:
            production_query_ = production_query
        try:
            prod_keys, production_res = (
                connector_factory.PostgreSQLConnector().execute_query(production_query_)
            )
        except psycopg2.errors.UndefinedColumn as e:
            print(e)
            prod_keys, production_res = (
                connector_factory.PostgreSQLConnector().execute_query(production_query)
            )
        production_data = (
            connector_factory.PostgreSQLConnector().process_recommendations(
                prod_keys, production_res
            )
        )
        ...
        if filters:
            rejection_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    rejection_query, filters, drill_state
                )
            )
        else:
            rejection_query_ = rejection_query
        try:
            rej_keys, rejection_res = (
                connector_factory.PostgreSQLConnector().execute_query(rejection_query_)
            )
        except psycopg2.errors.UndefinedColumn as e:
            print(e)
            rej_keys, rejection_res = (
                connector_factory.PostgreSQLConnector().execute_query(rejection_query)
            )
        cs_rejection_data = (
            connector_factory.PostgreSQLConnector().process_recommendations(
                rej_keys, rejection_res
        ...
        if filters:
            total_gd_rejection_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    total_gd_rejection, filters, drill_state
                )
            )
        else:
        ...
        try:
            (
                gd_rej_keys,
                gd_rejection_res,
            ) = connector_factory.PostgreSQLConnector().execute_query(
                total_gd_rejection_
            )
        except psycopg2.errors.UndefinedColumn as e:
            print(e)
            gd_rej_keys, gd_rejection_res = (
                connector_factory.PostgreSQLConnector().execute_query(
                    total_gd_rejection
                )
            )
        ...
        if filters:
            total_pt_rejection_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    total_pt_rejection, filters, drill_state
                )
            )
        else:
        ...
        if not drill_state:
            drill_state = "column"
        drill_down_column = await LPGPlantActions.get_next_level_drill_params(
            drill_state
        )
        return {
            "status": True,
    ...
    async def get_productivity_cyl_per_hour(filters, drill_state):
        productivity_cyl_per_hour = lpg_plant_queries.lpg_plant_query.get(
            "productivity_cyl_per_hour"
        )
        productivity_cyl_per_hour_ = productivity_cyl_per_hour
        if filters:
            productivity_cyl_per_hour_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    productivity_cyl_per_hour, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
                productivity_cyl_per_hour_
            )
        except psycopg2.errors.UndefinedColumn as e:
            print(e)
        ...
        data = connector_factory.PostgreSQLConnector().process_recommendations(
            keys, res
        )
        if not drill_state:
            drill_state = "column"
    ...
    async def get_rejections_by_zones(filters, drill_state):
        rejections_by_zones_query = lpg_plant_queries.lpg_plant_query.get(
            "rejections_by_zones"
        )
        rejections_by_zones_query_ = rejections_by_zones_query
        if filters:
            rejections_by_zones_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    rejections_by_zones_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
    ...
    async def get_daily_productivity_cyl_per_hour(filters, drill_state):
        daily_prod_query = lpg_plant_queries.lpg_plant_query.get(
            "daily_productivity_cyl_per_hour"
        )
        daily_prod_query_ = daily_prod_query
        if filters:
            daily_prod_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    daily_prod_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
    ...
    async def get_cyl_rejection_in_check_scale(filters, drill_state):
        cyl_rej_query = lpg_plant_queries.lpg_plant_query.get(
            "cyl_rejection_in_check_scale"
        )
        cyl_rej_query_ = cyl_rej_query
        if filters:
            cyl_rej_query_ = await widget_actions.WidgetActions.apply_filter_drilldown(
                cyl_rej_query, filters, drill_state
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
                cyl_rej_query_
    ...
    async def get_cyl_count_by_carousel(filters, drill_state):
        cyl_carousel_query = lpg_plant_queries.lpg_plant_query.get(
            "cyl_count_by_carousel"
        )
        cyl_carousel_query_ = cyl_carousel_query
        if filters:
            cyl_carousel_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    cyl_carousel_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
    ...
    async def get_cyl_count_by_zone(filters, drill_state):
        ...
        if filters:
            cyl_zone_query_ = await widget_actions.WidgetActions.apply_filter_drilldown(
                cyl_zone_query, filters, drill_state
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
                cyl_zone_query_
    ...
    async def get_bottom_cs_plants(filters, drill_state):
        ...
        if filters:
            cs_plants_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    cs_plants_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
    ...
    async def get_bottom_gd_plants(filters, drill_state):
        ...
        if filters:
            gd_plants_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    gd_plants_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
    ...
    async def get_bottom_pt_plants(filters, drill_state):
        ...
        if filters:
            pt_plants_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    pt_plants_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
    ...
    async def get_bottom_productivity_plants(filters, drill_state):
        prod_plants_query = lpg_plant_queries.lpg_plant_query.get(
            "bottom_productivity_plants"
        )
        prod_plants_query_ = prod_plants_query
        if filters:
            prod_plants_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    prod_plants_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
    ...
    async def get_productivity_by_zone(filters, drill_state):
        prod_by_zone_query = lpg_plant_queries.lpg_plant_query.get(
            "productivity_by_zone"
        )
        prod_by_zone_query_ = prod_by_zone_query
        if filters:
            prod_by_zone_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    prod_by_zone_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
    ...
    async def get_productivity_by_location(filters, drill_state):
        prod_by_location_query = lpg_plant_queries.lpg_plant_query.get(
            "productivity_by_location"
        )
        prod_by_location_query_ = prod_by_location_query
        if filters:
            prod_by_location_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    prod_by_location_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
    ...
    async def get_consolidated_table(filters, drill_state):
        consolidated_tab_query = lpg_plant_queries.lpg_plant_query.get(
            "consolidated_table"
        )
        consolidated_tab_query_ = consolidated_tab_query
        if filters:
            consolidated_tab_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    consolidated_tab_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
    ...
    async def get_top_productivity_plants(filters, drill_state):
        top_prod_query = lpg_plant_queries.lpg_plant_query.get(
            "top_productivity_plants"
        )
        top_prod_query_ = top_prod_query
        if filters:
            top_prod_query_ = await widget_actions.WidgetActions.apply_filter_drilldown(
                top_prod_query, filters, drill_state
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector().execute_query(
                top_prod_query_
    ...
    async def high_alert_locations(filters, drill_state):
        ...
        if filters:
            high_alert_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    high_alert_query, filters, drill_state
                )
            )
        ...
        try:
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(high_alert_query_)
        except psycopg2.errors.UndefinedColumn as e:
            print(e)
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(high_alert_query)
    ...
    async def critical_alert_locations(filters, drill_state):
        critical_alert_query = lpg_plant_queries.lpg_plant_query.get(
        ...
        if filters:
            critical_alert_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    critical_alert_query, filters, drill_state
                )
            )
        ...
        try:
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(critical_alert_query_)
        except psycopg2.errors.UndefinedColumn as e:
            print(e)
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(critical_alert_query)
        data = connector_factory.PostgreSQLConnector(
            "LPG_PLANT"
        ).process_recommendations(keys, res)
        if not drill_state:
    ...
    async def sod_terminal(filters, drill_state):
        ...
        if filters:
            sod_terminal_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    sod_terminal_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(sod_terminal_query_)
        except psycopg2.errors.UndefinedColumn as e:
            print(e)
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(sod_terminal_query)
        data = connector_factory.PostgreSQLConnector(
            "LPG_PLANT"
        ).process_recommendations(keys, res)
        if not drill_state:
    ...
    async def alert_categories(filters, drill_state):
        alert_categories_query = lpg_plant_queries.lpg_plant_query.get(
            "alert_categories"
        )
        ...
        if filters:
            alert_categories_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    alert_categories_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(alert_categories_query_)
        except psycopg2.errors.UndefinedColumn as e:
            print(e)
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(alert_categories_query)
        data = connector_factory.PostgreSQLConnector(
            "LPG_PLANT"
        ).process_recommendations(keys, res)
        if not drill_state:
    ...
    async def tas_alerts(filters, drill_state):
        ...
        if filters:
            tas_alerts_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    tas_alerts_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(tas_alerts_query_)
        except psycopg2.errors.UndefinedColumn as e:
            print(e)
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(tas_alerts_query)
        data = connector_factory.PostgreSQLConnector(
            "LPG_PLANT"
        ).process_recommendations(keys, res)
        if not drill_state:
    ...
    async def non_tas_alerts(filters, drill_state):
        ...
        if filters:
            non_tas_alerts_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    non_tas_alerts_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(non_tas_alerts_query_)
        except psycopg2.errors.UndefinedColumn as e:
            print(e)
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(non_tas_alerts_query)
        data = connector_factory.PostgreSQLConnector(
            "LPG_PLANT"
        ).process_recommendations(keys, res)
        if not drill_state:
    ...
    async def no_of_terminals(filters, drill_state):
        ...
        if filters:
            no_of_terminals_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    no_of_terminals_query, filters, drill_state
                )
            )
        try:
            ...
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(no_of_terminals_query_)
        except psycopg2.errors.UndefinedColumn as e:
            print(e)
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(no_of_terminals_query)
        data = connector_factory.PostgreSQLConnector(
            "LPG_PLANT"
        ).process_recommendations(keys, res)
        if not drill_state:
    ...
    async def alert_distributions(filters, drill_state):
        alert_distributions_query = lpg_plant_queries.lpg_plant_query.get(
            "alert_distributions"
        )
        ...
        if filters:
            alert_distributions_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    alert_distributions_query, filters, drill_state
                )
            )
        try:
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(alert_distributions_query_)
        except psycopg2.errors.UndefinedColumn as e:
            print(e)
            keys, res = connector_factory.PostgreSQLConnector(
                "LPG_PLANT"
            ).execute_query(alert_distributions_query)
        data = connector_factory.PostgreSQLConnector(
            "LPG_PLANT"
        ).process_recommendations(keys, res)
        if not drill_state:

...
async def download_streaming_data(df: pl.DataFrame):
    df = df.with_columns(pl.col(cs.datetime(time_zone="*")).dt.replace_time_zone(None))
...
class VTSAnalyticsActions:
    ...
    async def vts_card_chart(filters, cross_filters, drill_state, payload):
        try:
            ...
            return {
                "status": True,
                "message": "success",
                "data": await VTSAnalyticsActions.execute_query(
                    final_query, engine="dict"
                ),
            }
        ...
        try:
            ...
            if truck_number:
                ...
                view_query = f"""
                    ...
                    """
                conditions = VTSAnalyticsActions.build_filter_conditions(
                    filters, cross_filters, view_query
                )
                conditions = VTSAnalyticsActions.add_alert_type_conditions(
                    conditions, alert_type
                )
                view_query = VTSAnalyticsActions.apply_conditions_to_query(
                    view_query, conditions
                )
                print(view_query)

                df_view = await VTSAnalyticsActions.execute_query(
                    view_query, engine="polars"
                )

                if df_view.is_empty():
                    return {
                        "status": True,
                        "message": "No data found for this truck",
                        "data": [],
                    }
            ...
            if violation_types:
                violation_type_query = vts_query.vts_query.get(
                ...
                conditions = VTSAnalyticsActions.build_filter_conditions(
                    filters, cross_filters, violation_type_query
                )
                conditions = VTSAnalyticsActions.add_alert_type_conditions(
                    conditions, alert_type
                )
                ...
                for v_type in violation_types:
                    having_parts.append(
                        f"SUM(CASE WHEN violation_type = '{v_type}' THEN 1 ELSE 0 END) > 0"
                    )

                select_clause = ",\n        ".join(select_parts)
                having_clause = " AND ".join(having_parts)

                final_query = violation_type_query.format(
                    select_clause=select_clause, having_clause=having_clause
                )
                final_query = VTSAnalyticsActions.apply_conditions_to_query(
                ...
                df_data = await VTSAnalyticsActions.execute_query(
    ...
    async def vts_insite_violation(filters, cross_filters, drill_state, payload):
        ...
        try:
            ...
            has_qty_shortage_filter = (
                "qty_shortage" in violation_types if violation_types else False
            )
            vts_violation_types = (
                [v for v in violation_types if v != "qty_shortage"]
                if violation_types
                else []
            )
            ...
            if truck_number:
                ...
                if not df_shortage.is_empty():
                    df_shortage = df_shortage.filter(
                        pl.col("tl_number") == truck_number
                    )
                ...
                if df_view.is_empty() and df_shortage.is_empty():
                    return {
                        "status": True,
                        "message": "No data found for this vehicle",
                        "data": [],
                    }
                ...
                if "qty_shortage_detail" not in final_df.columns:
                    final_df = final_df.with_columns(qty_shortage_detail=pl.lit(""))
                ...
                if (
                    "transporter_name" not in final_df.columns
                    or final_df["transporter_name"].null_count() > 0
                ):
                ...
                return {
                    "status": True,
                    "message": "success",
                    "data": final_df.fill_null().to_dicts(),
                }
            ...
            if has_qty_shortage_filter and not vts_violation_types:
                ...
                df_shortage_all = await VTSAnalyticsActions.execute_query(
                    shortage_query
                )
                ...
                if df_shortage_all.is_empty():
                    return {
                        "status": True,
                        "message": "No shortage data found",
                        "data": [],
                    }
                ...
                if df_shortage_all.is_empty():
                    return {
                        "status": True,
                        "message": "No non-zero shortage data found",
                        "data": [],
                    }

                shortage_agg = df_shortage_all.group_by("invoice_number").agg(
                    pl.col("qty_shortage").sum(),
                    pl.col("zone").first(),
                ...
                tl_numbers_list = (
                    shortage_agg.get_column("tl_number").unique().to_list()
                )
                tl_numbers_str = "', '".join(map(str, tl_numbers_list))

                violation_columns = [
                    f"COALESCE({v_type}, 0) AS {v_type}" for v_type in all_violations
                ]
                ...
                conditions = VTSAnalyticsActions.build_filter_conditions(
                    filters, cross_filters, history_query
                )
                history_query = VTSAnalyticsActions.apply_conditions_to_query(
                    history_query, conditions
                )
                df_history = await VTSAnalyticsActions.execute_query(
                    history_query, engine="polars"
                )

                if df_history.is_empty():
                    df_history = pl.DataFrame(
                        schema=["tl_number", "invoice_number", *all_violations]
                    )
                ...
                truck_query = """
                              ...
                              FROM vts_truck_master \
                              """
                df_truck = await VTSAnalyticsActions.execute_query(
                    truck_query, engine="polars"
                )
                ...
                agg_df = merged_df.group_by("invoice_number").agg(aggs)
            ...
            if violation_types:
                select_parts = [
                    f"MAX({v_type}) AS {v_type}" for v_type in all_violations
                ]
                select_clause = ",\n           ".join(select_parts)

                if vts_violation_types:
                    having_parts = [
                        f"MAX({v_type}) > 0" for v_type in vts_violation_types
                    ]
                    having_clause = " AND ".join(having_parts)
                else:
                    having_clause = "1=1"
                ...
                violation_query = violation_query.format(
                    select_clause=select_clause, having_clause=having_clause
                )
                conditions = VTSAnalyticsActions.build_filter_conditions(
                ...
                df_history = await VTSAnalyticsActions.execute_query(
                    violation_query, engine="polars"
                )
                if not df_history.is_empty():
                    df_history = df_history.unique(
                ...
                violation_cols = [
                    col for col in all_violations if col in final_df.columns
                ]
                ...
                final_df = final_df.group_by("invoice_number").agg(aggs)
    ...
    async def violation_percentages(filters, cross_filters, drill_state, payload):
        try:
            ...
            emlock_conditions = VTSAnalyticsActions.build_filter_conditions(
                filters, cross_filters, emlock_open_query
            )
            emlock_query = VTSAnalyticsActions.apply_conditions_to_query(
                emlock_open_query, emlock_conditions
            )
            emlock_df = await VTSAnalyticsActions.execute_query(
                emlock_query, engine="polars"
            )
            ...
            for col in violation_cols:
                if col == "main_supply_removal_count":
                    ...
                    exprs.append(
                        pl.when(pl.col(col) >= 6).then(1).otherwise(0).alias(col)
                    )
                else:
            ...
            for key, count in violation_counts.items():
                percentages[key] = (
                    round(100 * count / total_all, 2) if total_all > 0 else 0
                )
    ...
    async def total_count_shortage(filters, cross_filters, drill_state, payload):
        try:
            ...
            location_df = await VTSAnalyticsActions.execute_query(
                location_query, engine="polars"
            )
    ...
    async def vts_drill_down_violation(filters, cross_filters, drill_state, payload):
        try:
            ...
            vts_df = await VTSAnalyticsActions.execute_query(
                vts_drill_query, engine="polars"
            )
            ...
            vts_df = vts_df.with_columns(
                created_at=pl.col("created_at").str.to_datetime(strict=False).dt.date()
            ...
            transporter_df = await VTSAnalyticsActions.execute_query(
                transporter_query, engine="polars"
            )
            ...
            merged_df = (
                merged_df.filter(pl.col(violation_type).fill_null(0) != 0)
                .sort("created_at", descending=False)
                .unique(
            ...
            if group_col != "tl_number":
                aggs.append(pl.col("tl_number").n_unique().alias("vehicle_count"))
            agg_df = merged_df.group_by(group_col).agg(aggs)
        ...
        try:
            ...
            if payload.get("download"):
                ...
                df2 = df2.with_columns(
                    swipeoutl1=pl.col("swipeoutl1")
                    .cast(pl.String)
                    .fill_null("")
                    .str.to_lowercase(),
                    swipeoutl2=pl.col("swipeoutl2")
                    .cast(pl.String)
                    .fill_null("")
                    .str.to_lowercase(),
                )
                ...
                shortage_query = (
                    """SELECT * FROM sales_trips_till_date WHERE load_status = '6' """
                )
                ...
                all_violations_df.write_excel(
                    workbook=output, worksheet="all_violations"
                )
                output.seek(0)
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            ...
            vts_df = vts_df.with_columns(
                created_at=(
                    pl.col("created_at").str.to_datetime(strict=False).dt.date()
                )
            ).sort("created_at", descending=False)
    ...
    async def vts_ongoing_trips(filters, cross_filters, drill_state, payload):
        try:
            ...
            if ongoing_trips_type:
                ongoing_trips_query = query.format(
                    ongoing_trips_type=ongoing_trips_type
                )
                conditions = VTSAnalyticsActions.build_filter_conditions(
                    filters, cross_filters, ongoing_trips_query
                )
                query = VTSAnalyticsActions.apply_conditions_to_query(
                    ongoing_trips_query, conditions
                )
            else:
                conditions = VTSAnalyticsActions.build_filter_conditions(
                    filters, cross_filters, query
            ...
            if not sap_id_list:
                ...
                """
                loc_df = await VTSAnalyticsActions.execute_query(
                    location_query, engine="polars"
                )
            ...
            invoice_list = (
                merged_df.get_column("invoice_no")
                .drop_nulls()
                .cast(pl.String)
                .unique()
                .to_list()
            ...
            if not invoice_list:
                return {
                    "status": True,
                    "message": "No invoices found in trips",
                    "data": [],
                }
            ...
            alert_df = await VTSAnalyticsActions.execute_query(
                completed_trips_query, engine="polars"
            )
            alert_invoice_list = (
                alert_df.get_column("invoice_no").cast(pl.String).to_list()
                if not alert_df.is_empty()
                else []
            )
            ...
            if status_filter == "live":
                merged_df = merged_df.filter(
                    ~pl.col("invoice_no").cast(pl.Utf8).is_in(alert_invoice_list)
                )
            ...
            if selected_tt:
                ...
                merged_df = merged_df.sort("created_at", descending=False).select(
            ...
            if group_col != "tt_number":
                summary_df = merged_df.group_by(group_col).agg(
                    pl.col("invoice_no").n_unique().alias("invoice_no"),
    ...
    async def location_level_voilation_breakup(
        ...
        try:
            ...
            loc_master_df = await VTSAnalyticsActions.execute_query(
                loc_master_query, engine="polars"
            )
    ...
    async def vts_alerts_violations(filters, cross_filters, drill_state, payload):
        try:
            ...
            alerts_df = alerts_df.filter(
                pl.col(group_by_column).is_not_null(), pl.col(group_by_column) != ""
            )
            ...
            alerts_df = alerts_df.group_by([group_by_column, "violation_type"]).len(
                name="count"
            )
            ...
            vt_df = alerts_df.group_by("violation_type").agg(
    ...
    async def violation_trends_over_time(filters, cross_filters, drill_state, payload):
        try:
            ...
            if (
                "violation_type" not in alerts_df.columns
                or "period" not in alerts_df.columns
            ):
                return {
                    "status": False,
            ...
            grouped = alerts_df.group_by(["period", "violation_type"]).len(name="count")
            ...
            result = []
            for (period,), period_data in grouped.group_by(
                "period"
            ):  # (name, data) per group
    ...
    async def alert_summary(filters, cross_filters, drill_state, payload):
        try:
            ...
            df = df.filter(
                ...
                pl.col(group_by_column).cast(pl.Utf8).str.strip_chars() != "",
            )
    ...
    async def card_chart_shortage(filters, cross_filters, drill_state, payload):
        try:
            ...
            vts_history = await VTSAnalyticsActions.execute_query(
                final_query, engine="polars"
            )
            ...
            total_qty_shortage = merged_df.select(pl.col("QTY_SHORTAGE").sum()).item()
            ...
            total_violation_count = (
                vts_history.select([pl.col(c).sum().alias(c) for c in violation_cols])
    ...
    async def integrate_shortage_trips(filters, cross_filters, drill_state, payload):
        ...
        if "load_date" in filtered_trips_df.columns:
            filtered_trips_df["load_date"] = pd.to_datetime(
                filtered_trips_df["load_date"]
            )
            ...
            if filtered_trips_df["load_date"].dt.tz is None:
                ...
                filtered_trips_df["load_date"] = filtered_trips_df[
                    "load_date"
                ].dt.tz_convert(ist)

            filtered_trips_df["load_date"] = filtered_trips_df["load_date"].dt.strftime(
                "%Y-%m-%d %H:%M:%S%z"
            )
    ...
    async def get_unblock_ageing(filters, cross_filters, drill_state, payload):
        try:
            ...
            df = df.with_columns(
                pl.col("vehicle_blocked_end_date")
                .str.to_datetime()
                .dt.replace_time_zone(None)
                .alias("vehicle_blocked_end_date"),
                pl.col("vehicle_blocked_start_date")
                .str.to_datetime()
                .dt.replace_time_zone(None)
                .alias("vehicle_blocked_start_date"),
            )
            ...
            counts = df.group_by(["sap_id", "zone", "tt_number", "violation_type"]).len(
                name="count"
            )
            ...
            avg_ageing = df.group_by(
                ["sap_id", "location_name", "transporter_code", "zone", "tt_number"]
            ).agg(pl.col("ageing").mean().alias("ageing"))

            df = avg_ageing.join(
                violation_counts, on=["sap_id", "tt_number"], how="left"
            )
            df = df.with_columns(pl.col("ageing").round(2).alias("ageing"))
            ...
            if drill_state:
                ...
                df = df.group_by(group_by_keys).agg(
    ...
    async def get_emlock_open_data(filters, cross_filters, drill_state, payload):
        try:
            ...
            total_invoice = df_false_any.select(
                pl.col("invoice_number").n_unique()
            ).item()
            ...
            if filters:
                ...
                if "zone" in filter_keys and "region" not in filter_keys:
                    ...
                    and "location_name" not in filter_keys
                ):
                    group_by_keys = [
                        "zone",
                        "region",
            ...
            grouped_df = df_false_any.group_by(group_by_keys).agg(
    ...
    async def power_disconnection(filters, cross_filters, drill_state, payload):
        try:
            ...
            df_transporter = await VTSAnalyticsActions.execute_query(
                trans_query, engine="polars"
            )
            ...
            merged_df = merged_df.filter(pl.col(violation_type).fill_null(0) >= 6)
            ...
            if selected_tl:
                ...
                # Rename columns for frontend
                merged_df = merged_df.rename(
                    mapping={"invoice_number": "invoice_no", "created_at": "created_at"}
                )
                return {
                    "status": True,
                    "message": f"{violation_type} details for vehicle {selected_tl}",
    ...
    async def action_device_vts(filters, cross_filters, drill_state, payload):
        ...
        try:
            ...
            if not rows:
                return {
                    "status": False,
                    "message": f"No record found for SAP TT No {sap_tt_no}",
                    "data": [],
                }
            ...
            if (
                (not action)
                and (remarks is None or remarks == "")
                and (not reason_for_cancel)
            ):
                return {
                    "status": True,
                    "message": f"Data found for SAP TT No {sap_tt_no}",
                    "data": [row],
                }
            ...
            return {
                "status": True,
                "message": f"Status updated to '{status}' for SAP TT No {sap_tt_no}",
                "data": updated_rows,
            }

...
class LPGCDCMSActions:        
        ...
        if filters:
            ...
            for each_float_col in ["Bookings", "Sales", "Pending"]:
                if each_float_col in resp.columns:
                    ...
                    resp[each_float_col] = resp[each_float_col].astype(int)
    ...
    async def lpg_cdcms_bookings_order_source_wise(filters, cross_filters, drill_state):
        ...
        if filters:
            ...
            resp["Total_Bookings"] = resp["Total_Bookings"].fillna(0).astype(int)
            return {"status": True, "message": "success", "data": resp}
        resp = await function(query=cdcms_order_source_query_)
        # Convert the response to a DataFrame for further processing
        resp = pd.DataFrame(resp)
    ...
    async def lpg_cdcms_current_financial_year_sales(filters, cross_filters, drill_state):
        ...
        if filters:
            ...
            resp["Sales"] = resp["Sales"].fillna(0).astype(int)
...
############################################################# SUBSIDY FAILURE STATISTICS ########################################################
    ...
    async def lpg_cdcms_pcc(filters, cross_filters, drill_state):
        ...
        if filters:
            ...
            lpg_cdcms_april_consumer_stats  += f' AND "SummaryDate" = \'{financial_start_date}\' AND 
                                                "Category" = \'Domestic\' AND "CategoryStatus" = \'Active\' AND 
                                                "RelationshipStatus" = \'A\' AND "RelationshipSubStatus" = \'1A\' AND 
                                                "ConsumerCategory" = \'A\' '
            ...
            if "where" not in lpg_cdcms_pcc_.lower():
                lpg_cdcms_april_consumer_stats  += f' WHERE "SummaryDate" = \'{financial_start_date}\' AND 
                                                "Category" = \'Domestic\' AND "CategoryStatus" = \'Active\' AND 
                                                "RelationshipStatus" = \'A\' AND "RelationshipSubStatus" = \'1A\' AND 
                                                "ConsumerCategory" = \'A\' '

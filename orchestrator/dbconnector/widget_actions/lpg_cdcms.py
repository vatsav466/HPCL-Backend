...
class LPGCDCMSActions:
    ...
    async def cdcms_dropdown(filters, cross_filters, drill_state):
        ...
        data = {
            "Month": months,
            "ZOName": df["ZOName"].unique().to_list(),
            "ROName": df["ROName"].unique().to_list(),
            "SAName": df["SAName"].unique().to_list(),
            "DistributorName": df["DistributorName"].unique().to_list(),
            "StateCode": df["StateCode"].unique().to_list(),
            "StateName": df["StateName"].unique().to_list(),
            "CylType": ["C142", "C5"],
            "ConsumerType": ["PMUY", "NPMUY"],
            "Financial_Year": await get_fy_list(),
        }
        return data
    ...
    async def lpg_cdcms_actual_vs_historic_sales(filters, cross_filters, drill_state):
        ...
        lpg_cdcms_sales_comparision_query_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_actual_vs_historic_sales"
        )
        today = datetime.now()
        if today.month < 4:
            start_year = today.year - 1
        ...
        if filters:
            ...
            lpg_cdcms_sales_comparision_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    lpg_cdcms_sales_comparision_query_, access_filters, drill_state
                )
            )
            if _fy:
            ...
            current_year = (
                resp[resp["Financial_Year"] == financial_year]
                .groupby("Quarter")["sales_volume"]
                .sum()
                .reset_index()
            )
            previous_year = (
                resp[resp["Financial_Year"] == prev_financial_year]
                .groupby("Quarter")["sales_volume"]
                .sum()
                .reset_index()
            )
            ...
            for _, row in resp.iterrows():
                comparison = {
                    ...
                    "Current_Sales": (
                        round(float(row["sales_volume_current"]) / 1000000)
                        if pd.notnull(row["sales_volume_current"])
                        else 0
                    ),
                    "Previous_Sale": (
                        round(float(row["sales_volume_previous"]) / 1000000)
                        if pd.notnull(row["sales_volume_previous"])
                        else 0
                    ),
                }
            ...
            if "Quarter" in filter_keys and "Month" not in filter_keys:
                ...
                for _, row in resp.iterrows():
                    comparison = {
                        ...
                        "Previous_Year": prev_financial_year,
                        "Current_Sales": (
                            round(float(row["sales_volume_current"]) / 1000000, 2)
                            if pd.notnull(row["sales_volume_current"])
                            else 0
                        ),
                        "Previous_Sale": (
                            round(float(row["sales_volume_previous"]) / 1000000, 2)
                            if pd.notnull(row["sales_volume_previous"])
                            else 0
                        ),
                    }
                ...
                current_year = (
                    resp[resp["Financial_Year"] == financial_year]
                    .groupby(["Month", "ZOName"])["sales_volume"]
                    .sum()
                    .reset_index()
                )
                previous_year = (
                ...
                "Month" in filter_keys
                and "ZOName" in filter_keys
                and "ROName" not in filter_keys
            ):
                current_year = (
                    resp[resp["Financial_Year"] == financial_year]
                    .groupby(["Month", "ZOName", "ROName"])["sales_volume"]
                    .sum()
                    .reset_index()
                )
                previous_year = (
                ...
                "Month" in filter_keys
                and "ZOName" in filter_keys
                and "ROName" in filter_keys
                and "SAName" not in filter_keys
            ):
                current_year = (
                    resp[resp["Financial_Year"] == financial_year]
                    .groupby(["Month", "ZOName", "ROName", "SAName"])["sales_volume"]
                    .sum()
                    .reset_index()
                )
                previous_year = (
                ...
                and "ZOName" in filter_keys
                and "ROName" in filter_keys
                and "SAName" in filter_keys
                and "DistributorName" not in filter_keys
            ):
                current_year = (
                    resp[resp["Financial_Year"] == financial_year]
                    .groupby(
                        ["Month", "ZOName", "ROName", "SAName", "DistributorName"]
                    )["sales_volume"]
                    .sum()
                    .reset_index()
                )
                ...
                resp = pd.merge(
                    current_year,
                    previous_year,
                    on=["Month", "ZOName", "ROName", "SAName", "DistributorName"],
                    how="outer",
                    suffixes=("_current", "_previous"),
                )
            final_data = []
    ...
    async def lpg_cdcms_monthly_sales(filters, cross_filters, drill_state):
        ...
        lpg_cdcms_month_query_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_monthly_sales"
        )
        ...
        if filters:
            ...
            lpg_cdcms_month_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    lpg_cdcms_month_query_, access_filters, drill_state
                )
            )
            ...
            for each_float_col in ["Total Sales"]:
                if each_float_col in resp.columns:
                    resp[each_float_col] = resp[each_float_col].fillna(0.0)

            # Fill missing values for string columns
            for each_str_col in ["Month", "ZOName"]:
        ...
        if filters:
            ...
            if "Month" in filter_keys and "ZOName" not in filter_keys:
                ...
                grouped_resp = resp.groupby(
                    ["Month", "ZOName", "ROName"], as_index=False
                ).agg({"Total Sales": "sum"})
            elif (
                "Month" in filter_keys
                and "ZOName" in filter_keys
                ...
                grouped_resp = resp.groupby(
                    ["Month", "ZOName", "ROName", "SAName"], as_index=False
                ).agg({"Total Sales": "sum"})
            elif (
                "Month" in filter_keys
                and "ZOName" in filter_keys
    ...
    @staticmethod
    async def lpg_cdcms_booking_vs_sales_vs_pending(
        ...
        lpg_cdcms_query_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_booking_vs_sales_vs_pending"
        )
        _filters = []
        if cross_filters:
            for filter in cross_filters:
        ...
        if filters:
            ...
            lpg_cdcms_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    lpg_cdcms_query_, access_filters, drill_state
                )
            )
            lpg_cdcms_query_ += ' GROUP BY "ZOName", "ROName", "SAName", "DistributorName", "ConsumerType", "CylType"'
            ...
            resp = resp.groupby(["ZOName"], as_index=False).agg(
                {"Bookings": "sum", "Sales": "sum", "Pending": "sum"}
            )
            # Fill missing values for numerical columns
        ...
        if filters:
            ...
            if "ZOName" in filter_keys and "ROName" not in filter_keys:
                ...
                "ZOName" in filter_keys
                and "ROName" in filter_keys
                and "SAName" not in filter_keys
            ):
                grouped_resp = resp.groupby(
                    ["ZOName", "ROName", "SAName"], as_index=False
                ).agg({"Bookings": "sum", "Sales": "sum", "Pending": "sum"})
            elif (
                "ZOName" in filter_keys
                and "ROName" in filter_keys
                and "SAName" in filter_keys
                ...
                grouped_resp = resp.groupby(
                    ["ZOName", "ROName", "SAName", "DistributorName"], as_index=False
                ).agg({"Bookings": "sum", "Sales": "sum", "Pending": "sum"})
            if grouped_resp is not None:
                for each_float_col in ["Bookings", "Sales", "Pending"]:
                    if each_float_col in grouped_resp.columns:
                        grouped_resp[each_float_col] = (
                            grouped_resp[each_float_col] / 1000
                        )
                        grouped_resp[each_float_col] = (
                            grouped_resp[each_float_col].fillna(0.0).round(2)
                        )
                return {
                    "status": True,
                    "message": "success",
        ...
        for each_float_col in ["Bookings", "Sales", "Pending"]:
            if each_float_col in resp.columns and type == "cyl":
                resp.loc[resp["CylType"] == "C142", each_float_col] = (
                    resp.loc[resp["CylType"] == "C142", each_float_col] * 14.2
                )
                resp.loc[resp["CylType"] == "C5", each_float_col] = (
                    resp.loc[resp["CylType"] == "C5", each_float_col] * 5
                )
            else:
                resp[each_float_col] = resp[each_float_col] / 1000
    ...
    async def lpg_cdcms_bookings_order_source_wise(filters, cross_filters, drill_state):
        ...
        cdcms_order_source_query_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_bookings_order_source_wise"
        )
        _filters = []
        if cross_filters:
            for filter in cross_filters:
        ...
        if filters:
            ...
            cdcms_order_source_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    cdcms_order_source_query_, access_filters, drill_state
                )
            )
            ...
            # Fill missing values for numerical columns
            for each_float_col in ["Total_Bookings"]:
                if each_float_col in resp.columns:
                    resp[each_float_col] = resp[each_float_col].fillna(0.0)

            # Fill missing values for string columns
            for each_str_col in ["OrderSourceName"]:
                if each_str_col in resp.columns:
                    resp[each_str_col] = resp[each_str_col].fillna("").astype(str)
        ...
        if filters:
            ...
            if "OrderSourceName" in filter_keys and "ZOName" not in filter_keys:
                grouped_resp = resp.groupby(
                    ["OrderSourceName", "ZOName"], as_index=False
                ).agg({"Total_Bookings": "sum"})
            elif (
                "OrderSourceName" in filter_keys
                and "ZOName" in filter_keys
                and "ROName" not in filter_keys
            ):
                grouped_resp = resp.groupby(
                    ["OrderSourceName", "ZOName", "ROName"], as_index=False
                ).agg({"Total_Bookings": "sum"})
            elif (
                "OrderSourceName" in filter_keys
                and "ZOName" in filter_keys
                ...
                grouped_resp = resp.groupby(
                    ["OrderSourceName", "ZOName", "ROName", "SAName"], as_index=False
                ).agg({"Total_Bookings": "sum"})
            elif (
                "OrderSourceName" in filter_keys
                and "ZOName" in filter_keys
    ...
    async def lpg_cdcms_pending_cosumer_type_wise(filters, cross_filters, drill_state):
        ...
        lpg_pending_query_ = lpg_plant_queries.lpg_plant_query.get(
        ...
        if filters:
            ...
            lpg_pending_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    lpg_pending_query_, access_filters, drill_state
                )
            )
        ...
        if resp:
            ...
            if filters:
                ...
                if "ConsumerType" in filter_keys and "ZOName" not in filter_keys:
                    grouped_resp = resp.groupby(
                        ["ConsumerType", "ZOName"], as_index=False
                    ).agg(
                        {
                            "Total_pending": "sum",
                        }
                ...
                if (
                    "ConsumerType" in filter_keys
                    and "ZOName" in filter_keys
                    and "ROName" not in filter_keys
                ):
                    grouped_resp = resp.groupby(
                        ["ConsumerType", "ZOName", "ROName"], as_index=False
                    ).agg(
                        {
                            "Total_pending": "sum",
                        }
                    ...
                    grouped_resp = resp.groupby(
                        ["ConsumerType", "ZOName", "ROName", "SAName"], as_index=False
                    ).agg(
                        {
                            "Total_pending": "sum",
                        }
                ...
                if grouped_resp is not None:
                    grouped_resp["Total_pending"] = (
                        grouped_resp["Total_pending"] / 100000
                    )
                    grouped_resp["Total_pending"] = grouped_resp["Total_pending"].round(
                        2
                    )
                    return {
                        "status": True,
    ...
    async def lpg_cdcms_ageing(filters, cross_filters, drill_state):
        ...
        if filters:
            ...
            conditions = []
            for rec in filters:
                if rec.key.replace('"', "") in [
                    "pending_1_3_days",
                    "pending_4_7_days",
                    "pending_8_15_days",
                    "pending_beyond_15_days",
                ]:
                    continue
                rec.value = rec.value.split(",")
                if isinstance(rec.value, str):
        ...
        if filters:
            ...
            if "pending_1_3_days" in filter_keys and "ZOName" not in filter_keys:
                grouped_resp = resp.groupby(
                    ["ConsumerType", "ZOName"], as_index=False
                ).agg({"pending_1_3_days": "sum"})
                grouped_resp = grouped_resp.pivot(
                    index="ZOName", columns="ConsumerType", values="pending_1_3_days"
                ).fillna(0)
                _index = "ZOName"
            elif (
                "pending_1_3_days" in filter_keys
                and "ZOName" in filter_keys
                and "ROName" not in filter_keys
            ):
                grouped_resp = resp.groupby(
                ...
                grouped_resp = grouped_resp.pivot(
                    index="ROName", columns="ConsumerType", values="pending_1_3_days"
                ).fillna(0)
                _index = "ROName"
            elif (
                "pending_1_3_days" in filter_keys
                ...
                grouped_resp = grouped_resp.pivot(
                    index="SAName", columns="ConsumerType", values="pending_1_3_days"
                ).fillna(0)
                _index = "SAName"
            elif (
                "pending_1_3_days" in filter_keys
                ...
                grouped_resp = grouped_resp.pivot(
                    ...
                    columns="ConsumerType",
                    values="pending_1_3_days",
                ).fillna(0)
                _index = "DistributorName"
            elif "pending_4_7_days" in filter_keys and "ZOName" not in filter_keys:
                grouped_resp = resp.groupby(
                    ["ConsumerType", "ZOName"], as_index=False
                ).agg({"pending_4_7_days": "sum"})
                grouped_resp = grouped_resp.pivot(
                    index="ZOName", columns="ConsumerType", values="pending_4_7_days"
                ).fillna(0)
                _index = "ZOName"
            elif (
                "pending_4_7_days" in filter_keys
                and "ZOName" in filter_keys
                and "ROName" not in filter_keys
            ):
                grouped_resp = resp.groupby(
                ...
                grouped_resp = grouped_resp.pivot(
                    index="ROName", columns="ConsumerType", values="pending_4_7_days"
                ).fillna(0)
                _index = "ROName"
            elif (
                "pending_4_7_days" in filter_keys
                ...
                grouped_resp = grouped_resp.pivot(
                    index="SAName", columns="ConsumerType", values="pending_4_7_days"
                ).fillna(0)
                _index = "SAName"
            elif (
                "pending_4_7_days" in filter_keys
                ...
                grouped_resp = grouped_resp.pivot(
                    index="DistributorName",
                    columns="ConsumerType",
                    values="pending_4_7_days",
                ).fillna(0)
                _index = "DistributorName"
            elif "pending_8_15_days" in filter_keys and "ZOName" not in filter_keys:
                grouped_resp = resp.groupby(
                    ["ConsumerType", "ZOName"], as_index=False
                ).agg({"pending_8_15_days": "sum"})
                grouped_resp = grouped_resp.pivot(
                    index="ZOName", columns="ConsumerType", values="pending_8_15_days"
                ).fillna(0)
                _index = "ZOName"
            elif (
                "pending_8_15_days" in filter_keys
                and "ZOName" in filter_keys
                and "ROName" not in filter_keys
            ):
                grouped_resp = resp.groupby(
                ...
                grouped_resp = grouped_resp.pivot(
                    index="ROName", columns="ConsumerType", values="pending_8_15_days"
                ).fillna(0)
                _index = "ROName"
            elif (
                "pending_8_15_days" in filter_keys
                ...
                grouped_resp = grouped_resp.pivot(
                    index="SAName", columns="ConsumerType", values="pending_8_15_days"
                ).fillna(0)
                _index = "SAName"
            elif (
                "pending_8_15_days" in filter_keys
                ...
                grouped_resp = grouped_resp.pivot(
                    index="DistributorName",
                    columns="ConsumerType",
                    values="pending_8_15_days",
                ).fillna(0)
                _index = "DistributorName"
            elif (
                "pending_beyond_15_days" in filter_keys and "ZOName" not in filter_keys
            ):
                grouped_resp = resp.groupby(
                    ["ConsumerType", "ZOName"], as_index=False
                ).agg({"pending_beyond_15_days": "sum"})
                grouped_resp = grouped_resp.pivot(
                    index="ZOName",
                    columns="ConsumerType",
                    values="pending_beyond_15_days",
                ).fillna(0)
                _index = "ZOName"
            elif (
                "pending_beyond_15_days" in filter_keys
                and "ZOName" in filter_keys
                and "ROName" not in filter_keys
            ):
                grouped_resp = resp.groupby(
            ...
            grouped_resp = (
                grouped_resp.assign(Total=grouped_resp["PMUY"] + grouped_resp["NPMUY"])
    ...
    async def lpg_cdcms_domestic_sales_table(filters, cross_filters, drill_state):
        ...
        lpg_sale_table_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_domestic_sales_table"
        )
        ...
        lpg_sale_table_ = await widget_actions.WidgetActions.apply_filter_drilldown(
            lpg_sale_table_, access_filters, drill_state
        )
        ...
        resp = resp.groupby(["ZOName", "CylType", "ConsumerType"], as_index=False).agg(
            {"Total_Booking": "sum", "Total_Sales": "sum", "Total_Pending": "sum"}
        )
        for each_float_col in ["Total_Booking", "Total_Sales", "Total_Pending"]:
            if each_float_col in resp.columns:
    ...
    @staticmethod
    async def lpg_cdcms_current_financial_year_sales(
        ...
        if _fy:
            ...
            financial_year = await get_financial_year()
        cumulative_sales_pmuy_npmuy_query_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_current_financial_year_sales"
        )
        if filters:
            filters += [
                dashboard_studio_model.WidgetFiltersCreate(**rec)
            ...
            cumulative_sales_pmuy_npmuy_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    cumulative_sales_pmuy_npmuy_query_, access_filters, drill_state
                )
            )
            ...
            # Fill missing values for string columns
            for each_str_col in ["ConsumerType"]:
                if each_str_col in resp.columns:
                    resp[each_str_col] = resp[each_str_col].fillna("").astype(str)
    ...
    async def lpg_cdcms_sakhi_registrations(filters, cross_filters, drill_state):
        ...
        financial_year = await get_financial_year()
        sakhi_registrations_query_ = lpg_plant_queries.lpg_plant_query.get(
        ...
        if filters:
            ...
            sakhi_registrations_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    sakhi_registrations_query_, access_filters, drill_state
                )
            )
            sakhi_registrations_query_ += (
                f" AND \"Financial_Year\" IN ('{financial_year}')"
            )
            ...
            if "where" not in sakhi_registrations_query_:
                sakhi_registrations_query_ += (
                    f" WHERE \"Financial_Year\" IN ('{financial_year}')"
                )
            else:
                sakhi_registrations_query_ += (
                    f" AND \"Financial_Year\" IN ('{financial_year}')"
        ...
        if filters:
            ...
            if "Month" in filter_keys and "ZOName" not in filter_keys:
                ...
                grouped_resp = resp.group_by(
                    ["Month", "ZOName", "ROName", "SAName"]
                ).agg([pl.sum("SakhiRegistered").alias("SakhiRegistered")])
            if grouped_resp is not None:
                return {
                    "status": True,
                    "message": "success",
                    "data": grouped_resp.to_dicts(),
                }
    ...
    async def lpg_cdcms_dbc_enrollments(filters, cross_filters, drill_state):
        ...
        if cross_filters:
            for filter in cross_filters:
                _filters.append({f"{filter.key}": f"{filter.value}"})
        dbc_enrollments_query_ = lpg_plant_queries.lpg_plant_query.get(
        ...
        if filters:
            ...
            dbc_enrollments_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    dbc_enrollments_query_, access_filters, drill_state
                )
            )
    ...
    async def lpg_cdcms_nc_stats(filters, cross_filters, drill_state):
        ...
        if cross_filters:
            for filter in cross_filters:
                _filters.append({f"{filter.key}": f"{filter.value}"})
        lpg_cdcms_nc_query_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_nc_query"
        )
        if filters:
            conditions = []
            ...
            lpg_cdcms_nc_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    lpg_cdcms_nc_query_, access_filters, drill_state
                )
            )
    ...
    async def lpg_cdcms_total_consumers(filters, cross_filters, drill_state):
        ...
        total_consumers_query_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_total_consumers"
        )
        _filters = []
        if cross_filters:
            for filter in cross_filters:
        ...
        if filters:
            ...
            total_consumers_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    total_consumers_query_, access_filters, drill_state
                )
            )
            total_consumers_query_ += (
                ' AND "Category" NOT IN (\'Others\') AND "ZOName" IS NOT NULL '
            )
            ...
            if not "where" in total_consumers_query_.lower():
                total_consumers_query_ += (
                    ' WHERE "Category" NOT IN (\'Others\') AND "ZOName" IS NOT NULL'
                )
            else:
                total_consumers_query_ += (
                    ' AND "Category" NOT IN (\'Others\') AND "ZOName" IS NOT NULL'
            ...
            # Fill missing values for numerical columns
            for each_float_col in ["Total_Consumers"]:
                if each_float_col in resp.columns:
                    resp[each_float_col] = resp[each_float_col].fillna(0.0)
            # Fill missing values for string columns
            for each_str_col in [
    ...
    async def lpg_cdcms_ekyc_statistics(filters, cross_filters, drill_state):
        ...
        df = pd.read_csv("/opt/ceg/algo/DistributorMappings.csv")
        ekyc_statistics_query_ = lpg_plant_queries.lpg_plant_query.get(
        ...
        if filters:
            ...
            ekyc_statistics_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    ekyc_statistics_query_, access_filters, drill_state
                )
            )
            ekyc_statistics_query_ += f"  AND \"ZOName\"  NOT IN ( 'Null') "
            ekyc_statistics_query_ += (
                ' GROUP BY   "ROName","SAName" ,"JDEDistributorCode","ZOName" '
            )
        else:
            access_filters = [
                dashboard_studio_model.WidgetFiltersCreate(**rec)
            ...
            resp_pie = resp.groupby(["temp"], as_index=False).agg(
                {"Completed": "sum", "Pending": "sum"}
            )
            del resp_pie["temp"]
            resp = resp.groupby(["ZOName"], as_index=False).agg(
                {"Completed": "sum", "Pending": "sum"}
            )
            for each_float_col in ["Completed", "Pending"]:
                if each_float_col in resp.columns:
                    resp[each_float_col] = resp[each_float_col].fillna(0.0)
            for each_str_col in ["ROName", "SAName", "JDEDistributorCode", "ZOName"]:
                if each_str_col in resp.columns:
                    resp[each_str_col] = resp[each_str_col].fillna("").astype(str)
            return {
                "status": True,
                "message": "success",
                "data_pie": resp_pie.to_dict(orient="records"),
                "data": resp,
            }
        ...
        if filters:
            ...
            if "ZOName" in filter_keys and "ROName" not in filter_keys:
                ...
                grouped_resp = resp.groupby(
                    ["ZOName", "ROName", "SAName"], as_index=False
                ).agg({"Completed": "sum", "Pending": "sum"})
            elif (
                "ZonesNames" in filter_keys
                and "ROName" in filter_keys
                and "SAName" in filter_keys
                and "DistributorName" not in filter_keys
            ):
    ...
    async def lpg_cdcms_total_suvidha(filters, cross_filters, drill_state):
        ...
        df = pd.read_csv("/opt/ceg/algo/DistributorMappings.csv")
        total_suvidha_query_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_total_suvidha"
        )
        _filters = []
        if cross_filters:
            for filter in cross_filters:
        ...
        if filters:
            ...
            total_suvidha_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    total_suvidha_query_, access_filters, drill_state
                )
            )
            ...
            return {
                "status": True,
                "message": "success",
                "data_pie": pie_resp.to_dict(orient="records"),
                "data": resp.to_dict(orient="records"),
            }
    ...
    async def lpg_cdcms_overall_ctc_statistics(filters, cross_filters, drill_state):
        ...
        df = pd.read_csv("/opt/ceg/algo/DistributorMappings.csv")
        overall_ctc_statistics_query_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_overall_ctc_statistics"
        )
        _filters = []
        if cross_filters:
            for filter in cross_filters:
        ...
        if filters:
            ...
            overall_ctc_statistics_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    overall_ctc_statistics_query_, access_filters, drill_state
                )
            )
            overall_ctc_statistics_query_ += (
                f" AND \"ZOName\"  NOT IN ( 'Null') AND \"Category\" NOT IN ('Others')"
            )
        ...
        if filters:
            ...
            if "Category" in filter_keys and "ZOName" not in filter_keys:
                ...
                "Category" in filter_keys
                and "ZOName" in filter_keys
                and "ROName" not in filter_keys
            ):
                grouped_resp = resp.groupby(
                    ["Category", "ZOName", "ROName"], as_index=False
                ).agg({"ACTC": "sum", "BCTC": "sum", "NCTC": "sum"})
                ...
                grouped_resp = resp.groupby(
                    ["Category", "ZOName", "ROName", "SAName"], as_index=False
                ).agg({"ACTC": "sum", "BCTC": "sum", "NCTC": "sum"})
            elif (
                "Category" in filter_keys
                and "ZOName" in filter_keys
                and "ROName" in filter_keys
                and "SAName" in filter_keys
                and "JDEDistributorCode" not in filter_keys
            ):
                grouped_resp = resp.groupby(
    ...
    @staticmethod
    async def lpg_cdcms_daywise_overall_ctc_statistics(
        filters, cross_filters, drill_state
    ):
        Charts_Connection_Vault_RoutingParams.connection_id = (
            connection_mapping.connection_mapping.get("hpcl_ceg", "1")
        ...
        daywise_overall_ctc_statistics_query_ = lpg_plant_queries.lpg_plant_query.get(
        ...
        if cross_filters:
            for filter in cross_filters:
                if "DATE" in filter.key:
                    f" '{filter.value.split(",")[0]}' AND '{filter.value.split(",")[-1]}' "
                    continue
                _filters.append({f"{filter.key}": f"{filter.value}"})
        if filters:
            conditions = []
            ...
            daywise_overall_ctc_statistics_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    daywise_overall_ctc_statistics_query_, access_filters, drill_state
                )
            )
    ...
    async def lpg_cdcms_safety_check_pending(filters, cross_filters, drill_state):
        ...
        df = pd.read_csv("/opt/ceg/algo/DistributorMappings.csv")
        overall_safety_check_pending_query_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_safety_check_pending"
        )
        _filters = []
        if cross_filters:
            for filter in cross_filters:
        ...
        if filters:
            ...
            overall_safety_check_pending_query_ += (
                f" AND \"ZOName\"  NOT IN ('Null') AND \"Category\" IN ('Domestic')"
            )
    ...
    async def lpg_cdcms_consumer_statistics_table(filters, cross_filters, drill_state):
        ...
        lpg_consumer_table_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_consumer_statistics_table"
        )
        if not filters and not cross_filters:
            lpg_consumer_table_ += f' WHERE "ZOName" IS NOT NULL'
            lpg_consumer_table_ += (
                ' GROUP BY "ZoneNames", "CylinderType", "SubCategory" '
            )
        ...
        for each_str_col in ["ZoneNames", "CylinderType", "SubCategory"]:
            if each_str_col in resp.columns:
                resp[each_str_col] = resp[each_str_col].fillna("").astype(str)
        resp = resp.groupby(
            ["ZoneNames", "CylinderType", "SubCategory"], as_index=False
        ).agg(
            {
                "Total_Consumers": "sum",
                "eKYCCompleted": "sum",
    ...
    async def lpg_cdcms_subsidy_failure_stats(filters, cross_filters, drill_state):
        ...
        lpg_failure_stats_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_subsidy_failure_stats"
        )
        _filters = []
        if cross_filters:
            for filter in cross_filters:
        ...
        if filters:
            filters += [
                dashboard_studio_model.WidgetFiltersCreate(**rec)
                for rec in await hpcl_ceg_model.LpgSubsidyFailureData.get_clause_conditions(
                    formated=True
                )
            ]
            conditions = []
            ...
            lpg_failure_stats_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    lpg_failure_stats_, access_filters, drill_state
                )
            )
            ...
            if resp.empty:
                return {"status": True, "message": "success", "data": []}
            resp = resp.groupby(["PaymentErrorName"], as_index=False).agg(
        ...
        if resp:
            ...
            if filters:
                ...
                if "PaymentErrorName" in filter_keys and "ZOName" not in filter_keys:
                    grouped_resp = resp.groupby(
                        ["PaymentErrorName", "ZOName"], as_index=False
                    ).agg({"Consumers": "sum", "Refills": "sum"})
                if (
                    "PaymentErrorName" in filter_keys
                    and "ZOName" in filter_keys
                    and "ROName" not in filter_keys
                ):
                    grouped_resp = resp.groupby(
                        ["PaymentErrorName", "ZOName", "ROName"], as_index=False
                    ).agg({"Consumers": "sum", "Refills": "sum"})
                elif (
                    "PaymentErrorName" in filter_keys
                    and "ZOName" in filter_keys
                    ...
                    grouped_resp = resp.groupby(
                        ["PaymentErrorName", "ZOName", "ROName", "SAName"],
                        as_index=False,
                    ).agg({"Consumers": "sum", "Refills": "sum"})
                elif (
                    "PaymentErrorName" in filter_keys
    ...
    @staticmethod
    async def lpg_cdcms_daywise_subsidy_failure_statistics(
        filters, cross_filters, drill_state
    ):
        Charts_Connection_Vault_RoutingParams.connection_id = (
            connection_mapping.connection_mapping.get("hpcl_ceg", "1")
        ...
        if not drill_state == "financial_year":
            daywise_failure_stats_query_ = lpg_plant_queries.lpg_plant_query.get(
        ...
        if filters:
            ...
            daywise_failure_stats_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    daywise_failure_stats_query_, access_filters, drill_state
                )
            )
            ...
            if daterange and not drill_state == "financial_year":
                daywise_failure_stats_query_ += (
                    f' AND "Delivery_Date" BETWEEN {daterange} '
                )
            if not drill_state == "financial_year":
            ...
            if (
                not "where" in daywise_failure_stats_query_.lower()
                and not daterange
                and not drill_state == "financial_year"
            ):
                ...
                daywise_failure_stats_query_ += (
                    f' WHERE "Delivery_Date" BETWEEN {daterange} '
                )
        ...
        try:
            ...
            resp = pl.from_pandas(resp)
            resp = resp.with_columns(
                pl.col("Refills").fill_null(0).cast(pl.Float64).alias("Refills")
            )
    ...
    async def lpg_cdcms_exception_stats(filters, cross_filters, drill_state):
        ...
        lpg_exception_stats_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_exception_stats"
        )
        _filters = []
        if cross_filters:
            for filter in cross_filters:
        ...
        if filters:
            filters += [
                dashboard_studio_model.WidgetFiltersCreate(**rec)
                for rec in await hpcl_ceg_model.LpgSubsidyExceptionData.get_clause_conditions(
                    formated=True
                )
            ]
            conditions = []
            ...
            lpg_exception_stats_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    lpg_exception_stats_, access_filters, drill_state
                )
            )
            ...
            if resp.empty:
                return {"status": True, "message": "success", "data": []}
            resp = resp.groupby(["ExceptionName"], as_index=False).agg(
                {"Consumers": "sum", "Refills": "sum"}
            )
        ...
        if resp:
            ...
            if filters:
                ...
                if "ExceptionName" in filter_keys and "ZOName" not in filter_keys:
                    grouped_resp = resp.groupby(
                        ["ExceptionName", "ZOName"], as_index=False
                    ).agg({"Consumers": "sum", "Refills": "sum"})
                if (
                    "ExceptionName" in filter_keys
                    and "ZOName" in filter_keys
                    and "ROName" not in filter_keys
                ):
                    grouped_resp = resp.groupby(
                        ["ExceptionName", "ZOName", "ROName"], as_index=False
                    ).agg({"Consumers": "sum", "Refills": "sum"})
                elif (
                    "ExceptionName" in filter_keys
                    and "ZOName" in filter_keys
                    ...
                    grouped_resp = resp.groupby(
                        ["ExceptionName", "ZOName", "ROName", "SAName"], as_index=False
                    ).agg({"Consumers": "sum", "Refills": "sum"})
                elif (
                    "ExceptionName" in filter_keys
                    and "ZOName" in filter_keys
    ...
    @staticmethod
    async def lpg_cdcms_daywise_subsidy_exception_statistics(
        filters, cross_filters, drill_state
    ):
        Charts_Connection_Vault_RoutingParams.connection_id = (
            connection_mapping.connection_mapping.get("hpcl_ceg", "1")
        ...
        if not drill_state == "financial_year":
            daywise_exception_stats_query_ = lpg_plant_queries.lpg_plant_query.get(
                "lpg_cdcms_daywise_subsidy_exception_statistics"
            )
        else:
            daywise_exception_stats_query_ = lpg_plant_queries.lpg_plant_query.get(
                "lpg_cdcms_daywise_subsidy_exception_statistics_m"
            )
        _filters = []
        daterange = None
        if cross_filters:
        ...
        if filters:
            ...
            daywise_exception_stats_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    daywise_exception_stats_query_, access_filters, drill_state
                )
            )
            if not daterange and not drill_state == "financial_year":
            ...
            if (
                not "where" in daywise_exception_stats_query_.lower()
                and not daterange
                and not drill_state == "financial_year"
            ):
    ...
    async def lpg_cdcms_subsidy_central_consumers(filters, cross_filters, drill_state):
        ...
        if cross_filters:
            for filter in cross_filters:
                if "Financial_Year" in filter.key:
                    _fy = True
                _filters.append({f"{filter.key}": f"{filter.value}"})
        lpg_cdcms_subsidy_central_consumers_query_ = (
            lpg_plant_queries.lpg_plant_query.get("lpg_cdcms_subsidy_central_consumers")
        )
        if filters:
            conditions = []
            for rec in filters:
            ...
            if (
                "where" not in lpg_cdcms_subsidy_central_consumers_query_.lower()
                and not _fy
            ):
                lpg_cdcms_subsidy_central_consumers_query_ += (
                    f" WHERE \"Financial_Year\" IN ('{financial_year}')"
        ...
        if filters:
            ...
            if "Month" in filter_keys and "ZOName" not in filter_keys:
                ...
                grouped_resp = resp.group_by(
                    ["ConsumerType", "Month", "ZOName", "ROName"]
                ).agg(
                    [
                        pl.sum("consumer_count").alias("consumer_count"),
                    ]
                ...
                grouped_resp = resp.group_by(
                    ["ConsumerType", "Month", "ZOName", "ROName", "SAName"]
                ).agg(
                    [
                        pl.sum("consumer_count").alias("consumer_count"),
                    ]
                ...
                _index = "DistributorName"
            if grouped_resp is not None:
                grouped_resp = grouped_resp.with_columns(
                    pl.col("consumer_count")
                    .fill_null(0)
                    .cast(pl.Float64)
                    .alias("consumer_count")
                )
                grouped_resp = grouped_resp.pivot(
                    index=_index, on="ConsumerType", values="consumer_count"
                )
    ...
    @staticmethod
    async def lpg_cdcms_subsidy_central_transaction(
        filters, cross_filters, drill_state
    ):
        Charts_Connection_Vault_RoutingParams.connection_id = (
            connection_mapping.connection_mapping.get("hpcl_ceg", "1")
        ...
        if cross_filters:
            for filter in cross_filters:
                if "Financial_Year" in filter.key:
                    _fy = True
                _filters.append({f"{filter.key}": f"{filter.value}"})
        lpg_cdcms_subsidy_central_transaction_query_ = (
            lpg_plant_queries.lpg_plant_query.get(
        ...
        if filters:
            ...
            if (
                "where" not in lpg_cdcms_subsidy_central_transaction_query_.lower()
                and not _fy
            ):
                lpg_cdcms_subsidy_central_transaction_query_ += (
                    f" WHERE \"Financial_Year\" IN ('{financial_year}')"
        ...
        if filters:
            ...
            if grouped_resp is not None:
                ...
                grouped_resp = grouped_resp.pivot(
                    index=_index, on="ConsumerType", values="transaction_count"
                )
                grouped_resp = grouped_resp.with_columns(pl.all().fill_null(0))
                result = []
                for row in grouped_resp.iter_rows(named=True):
    ...
    async def lpg_cdcms_subsidy_central_amount(filters, cross_filters, drill_state):
        ...
        if cross_filters:
            for filter in cross_filters:
                if "Financial_Year" in filter.key:
                    _fy = True
                _filters.append({f"{filter.key}": f"{filter.value}"})
        lpg_cdcms_subsidy_central_amount_query_ = lpg_plant_queries.lpg_plant_query.get(
        ...
        if filters:
            ...
            lpg_cdcms_subsidy_central_amount_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    lpg_cdcms_subsidy_central_amount_query_, access_filters, drill_state
                )
            )
            if not _fy:
                lpg_cdcms_subsidy_central_amount_query_ += (
            ...
            if (
                "where" not in lpg_cdcms_subsidy_central_amount_query_.lower()
                and not _fy
            ):
                lpg_cdcms_subsidy_central_amount_query_ += (
                    f" WHERE \"Financial_Year\" IN ('{financial_year}')"
        ...
        if filters:
            ...
            if grouped_resp is not None:
                grouped_resp = grouped_resp.with_columns(
                    pl.col("SubsidyAmount")
                    .fill_null(0)
                    .cast(pl.Float64)
                    .alias("SubsidyAmount")
                )
                grouped_resp = grouped_resp.pivot(
                    index=_index, on="ConsumerType", values="SubsidyAmount"
                )
                grouped_resp = grouped_resp.with_columns(pl.all().fill_null(0))
                result = []
                for row in grouped_resp.iter_rows(named=True):
        ...
        resp = resp.sort("month_number")
        resp = resp.with_columns(
            pl.col("SubsidyAmount").fill_null(0).cast(pl.Float64).alias("SubsidyAmount")
        )
        ...
        if cross_filters:
            for filter in cross_filters:
                if "Financial_Year" in filter.key:
                    _fy = True
                _filters.append({f"{filter.key}": f"{filter.value}"})
        lpg_cdcms_subsidy_state_consumers_query_ = (
            lpg_plant_queries.lpg_plant_query.get("lpg_cdcms_subsidy_state_consumers")
        )
        if filters:
            conditions = []
            for rec in filters:
            ...
            if (
                "where" not in lpg_cdcms_subsidy_state_consumers_query_.lower()
                and not _fy
            ):
                lpg_cdcms_subsidy_state_consumers_query_ += (
                    f" WHERE \"Financial_Year\" IN ('{financial_year}')"
    ...
    @staticmethod
    async def lpg_cdcms_subsidy_state_consumers_stacked_state(
        filters, cross_filters, drill_state
    ):
        Charts_Connection_Vault_RoutingParams.connection_id = (
            connection_mapping.connection_mapping.get("hpcl_ceg", "1")
        ...
        if filters:
            ...
            if "Month" in filter_keys and "ZOName" not in filter_keys:
                ...
                grouped_resp = resp.group_by(
                    ["StateCode", "Month", "ZOName", "ROName", "SAName"]
                ).agg(
                    [
                        pl.sum("consumer_count").alias("consumer_count"),
                    ]
        ...
        if cross_filters:
            for filter in cross_filters:
                if "Financial_Year" in filter.key:
                    _fy = True
                _filters.append({f"{filter.key}": f"{filter.value}"})
        lpg_cdcms_subsidy_state_transaction_query_ = (
            lpg_plant_queries.lpg_plant_query.get("lpg_cdcms_subsidy_state_transaction")
        )
        if filters:
            conditions = []
            for rec in filters:
            ...
            if (
                "where" not in lpg_cdcms_subsidy_state_transaction_query_.lower()
                and not _fy
            ):
                lpg_cdcms_subsidy_state_transaction_query_ += (
                    f" WHERE \"Financial_Year\" IN ('{financial_year}')"
    ...
    @staticmethod
    async def lpg_cdcms_subsidy_state_transaction_stacked_state(
        filters, cross_filters, drill_state
    ):
        Charts_Connection_Vault_RoutingParams.connection_id = (
            connection_mapping.connection_mapping.get("hpcl_ceg", "1")
    ...
    async def lpg_cdcms_subsidy_state_amount(filters, cross_filters, drill_state):
        ...
        if cross_filters:
            for filter in cross_filters:
                if "Financial_Year" in filter.key:
                    _fy = True
                _filters.append({f"{filter.key}": f"{filter.value}"})
        lpg_cdcms_subsidy_state_amount_query_ = lpg_plant_queries.lpg_plant_query.get(
        ...
        if filters:
            ...
            lpg_cdcms_subsidy_state_amount_query_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    lpg_cdcms_subsidy_state_amount_query_, access_filters, drill_state
                )
            )
            if not _fy:
                lpg_cdcms_subsidy_state_amount_query_ += (
    ...
    @staticmethod
    async def lpg_cdcms_subsidy_state_amount_stacked_state(
        filters, cross_filters, drill_state
    ):
        Charts_Connection_Vault_RoutingParams.connection_id = (
            connection_mapping.connection_mapping.get("hpcl_ceg", "1")
    ...
    async def lpg_cdcms_backlogs(filters, cross_filters, drill_state):
        ...
        lpg_cdcms_backlogs_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_backlogs"
        )
        lpg_cdcms_backlogs_today_ = lpg_plant_queries.lpg_plant_query.get(
            "lpg_cdcms_backlogs_today"
        )
        _filters = []
        if cross_filters:
            for filter in cross_filters:
        ...
        if filters:
            ...
            lpg_cdcms_backlogs_today_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    lpg_cdcms_backlogs_today_, access_filters, drill_state
                )
            )
            lpg_cdcms_backlogs_ = (
                await widget_actions.WidgetActions.apply_filter_drilldown(
                    lpg_cdcms_backlogs_, access_filters, drill_state
                )
            )
            ...
            latest_pending_bookings = await filter_data(
                latest_pending_bookings.to_pandas(), _filters
            )
            average_sales = pl.from_pandas(average_sales)
            latest_pending_bookings = pl.from_pandas(latest_pending_bookings)

            # Overall Number
            overall_average_sales = average_sales.with_columns(
                pl.lit("temp").alias("temp")
            )
            overall_average_sales = overall_average_sales.group_by("temp").agg(
                (pl.col("TotalSalesYesterday").sum() / 90).alias("AverageSales")
            )
            overall_latest_pending_bookings = latest_pending_bookings.with_columns(
                pl.lit("temp").alias("temp")
            )
            overall_latest_pending_bookings = overall_latest_pending_bookings.group_by(
                "temp"
            ).agg(pl.col("Total_Pending").sum().alias("TotalPendingBookings"))
            overall_backlog = overall_latest_pending_bookings.join(
                overall_average_sales, on="temp", how="left"
            ).with_columns(
                (pl.col("TotalPendingBookings") / pl.col("AverageSales")).alias(
                    "Backlog"
                )
            )

            average_sales = average_sales.group_by("ZOName").agg(
                (pl.col("TotalSalesYesterday").sum() / 90).alias("AverageSales")
            )
            latest_pending_bookings = latest_pending_bookings.group_by("ZOName").agg(
                pl.col("Total_Pending").sum().alias("TotalPendingBookings")
            )
            backlog = latest_pending_bookings.join(
                average_sales, on="ZOName", how="left"
            ).with_columns(
                (pl.col("TotalPendingBookings") / pl.col("AverageSales")).alias(
                    "Backlog"
                )
            ...
            return {
                "status": True,
                "message": "success",
                "overall_number": overall_backlog["Backlog"][-1],
                "data": backlog.to_dicts(),
            }
        ...
        overall_backlog = overall_latest_pending_bookings.join(
            ...
            (pl.col("TotalPendingBookings") / pl.col("AverageSales")).alias("Backlog")
        )
        ...
        if filters:
            ...
            backlog = None
            if "ZOName" in filter_keys and "ROName" not in filter_keys:
                average_sales = average_sales.group_by("ROName").agg(
                    (pl.col("TotalSalesYesterday").sum() / 90).alias("AverageSales")
                )
                latest_pending_bookings = latest_pending_bookings.group_by(
                    "ROName"
                ).agg(pl.col("Total_Pending").sum().alias("TotalPendingBookings"))
                backlog = latest_pending_bookings.join(
                    average_sales, on="ROName", how="left"
                ).with_columns(
                    (pl.col("TotalPendingBookings") / pl.col("AverageSales")).alias(
                        "Backlog"
                    )
                ...
                average_sales = average_sales.group_by("SAName").agg(
                    (pl.col("TotalSalesYesterday").sum() / 90).alias("AverageSales")
                )
                latest_pending_bookings = latest_pending_bookings.group_by(
                    "SAName"
                ...
                backlog = latest_pending_bookings.join(
                    average_sales, on="SAName", how="left"
                ).with_columns(
                    (pl.col("TotalPendingBookings") / pl.col("AverageSales")).alias(
                        "Backlog"
                    )
                ...
                average_sales = average_sales.group_by("DistributorName").agg(
                    (pl.col("TotalSalesYesterday").sum() / 90).alias("AverageSales")
                )
                latest_pending_bookings = latest_pending_bookings.group_by(
                    "DistributorName"
                ...
                backlog = latest_pending_bookings.join(
                    average_sales, on="DistributorName", how="left"
                ).with_columns(
                    (pl.col("TotalPendingBookings") / pl.col("AverageSales")).alias(
                        "Backlog"
                    )
    ...
    async def lpg_cdcms_pcc(filters, cross_filters, drill_state):
        try:
            ...
            lpg_cdcms_april_consumer_stats = lpg_plant_queries.lpg_plant_query.get(
                "lpg_cdcms_april_consumer_stats"
            )
            lpg_cdcms_current_consumer_stats = lpg_plant_queries.lpg_plant_query.get(
                "lpg_cdcms_current_consumer_stats"
            )
            lpg_cdcms_pcc_sales = lpg_plant_queries.lpg_plant_query.get(
                "lpg_cdcms_pcc_sales"
            )
            ...
            if filters:
                ...
                lpg_cdcms_pcc_sales = (
                    await widget_actions.WidgetActions.apply_filter_drilldown(
                        lpg_cdcms_pcc_sales, access_filters, drill_state
                    )
                )
                lpg_cdcms_april_consumer_stats = (
                    await widget_actions.WidgetActions.apply_filter_drilldown(
                        lpg_cdcms_april_consumer_stats, access_filters, drill_state
                    )
                )
                lpg_cdcms_current_consumer_stats = (
                    await widget_actions.WidgetActions.apply_filter_drilldown(
                        lpg_cdcms_current_consumer_stats, access_filters, drill_state
                    )
                )
            ...
            current_consumer_stats = await function(
                query=lpg_cdcms_current_consumer_stats
            )
            ...
            current_consumer_stats = current_consumer_stats.rename(
                {"SubCategory": "ConsumerType"}
            )
            april_consumer_stats = april_consumer_stats.rename(
                {"SubCategory": "ConsumerType"}
            )

            april_consumer_stats = await filter_data(
                april_consumer_stats.to_pandas(), _filters
            )
            current_consumer_stats = await filter_data(
                current_consumer_stats.to_pandas(), _filters
            )
            lpg_cdcms_pcc_sales = await filter_data(
                lpg_cdcms_pcc_sales.to_pandas(), _filters
            )
            ...
            current_consumer_stats = current_consumer_stats.join(
                df, on="JDEDistributorCode", how="left"
            )

            lpg_cdcms_pcc_sales = lpg_cdcms_pcc_sales.with_columns(
                pl.col("TotalSalesYesterday")
                .cast(pl.Float64)
                .alias("TotalSalesYesterday")
            )
            lpg_cdcms_pcc_sales = lpg_cdcms_pcc_sales.with_columns(
                pl.when(pl.col("CylType") == "C5")
                .then(pl.col("TotalSalesYesterday") * 5 / 14.2)
                .otherwise(pl.col("TotalSalesYesterday"))
            ...
            if filters:
                ...
                pcc = None
                if "ZOName" in filter_keys and "ROName" not in filter_keys:
                    april_consumer_stats = april_consumer_stats.group_by("ROName").agg(
                        (pl.col("ConsumerCount").sum()).alias("ConsumerCount_start")
                    )
                    current_consumer_stats = current_consumer_stats.group_by(
                        "ROName"
                    ).agg(
                        (pl.col("ConsumerCount").sum()).alias("ConsumerCount_current")
                    )
                    avg_consumer_count = (
                        april_consumer_stats.join(
                            current_consumer_stats, on="ROName", how="outer"
                        )
                        .with_columns(
                            (
                                (
                                    pl.col("ConsumerCount_start")
                                    + pl.col("ConsumerCount_current")
                                )
                                / 2
                            ).alias("avg_consumer_count")
                        )
                        .drop("ROName_right")
                    )
                    lpg_cdcms_pcc_sales = lpg_cdcms_pcc_sales.group_by("ROName").agg(
                        (pl.col("TotalRefillSales").sum()).alias("TotalRefillSales")
                    )
                    pcc = avg_consumer_count.join(
                        lpg_cdcms_pcc_sales, on="ROName", how="outer"
                    ).drop("ROName_right")
                elif (
                    "ZOName" in filter_keys
                    and "ROName" in filter_keys
                    ...
                    april_consumer_stats = april_consumer_stats.group_by("SAName").agg(
                        (pl.col("ConsumerCount").sum()).alias("ConsumerCount_start")
                    )
                    current_consumer_stats = current_consumer_stats.group_by(
                        "SAName"
                    ...
                    avg_consumer_count = (
                        april_consumer_stats.join(
                            current_consumer_stats, on="SAName", how="outer"
                        )
                        .with_columns(
                            (
                                (
                    ...
                    lpg_cdcms_pcc_sales = lpg_cdcms_pcc_sales.group_by("SAName").agg(
                        (pl.col("TotalRefillSales").sum()).alias("TotalRefillSales")
                    )
                    pcc = avg_consumer_count.join(
                        lpg_cdcms_pcc_sales, on="SAName", how="outer"
                    ).drop("SAName_right")
                elif (
                    "ZOName" in filter_keys
                    and "ROName" in filter_keys
                    ...
                    april_consumer_stats = april_consumer_stats.group_by(
                        "DistributorName"
                    ).agg((pl.col("ConsumerCount").sum()).alias("ConsumerCount_start"))
                    current_consumer_stats = current_consumer_stats.group_by(
                        "DistributorName"
                    ).agg(
                        (pl.col("ConsumerCount").sum()).alias("ConsumerCount_current")
                    ...
                    avg_consumer_count = (
                        april_consumer_stats.join(
                            current_consumer_stats, on="DistributorName", how="outer"
                        )
                        .with_columns(
                            (
                                (
                        ...
                        .drop("DistributorName_right")
                    )
                    lpg_cdcms_pcc_sales = lpg_cdcms_pcc_sales.group_by(
                        "DistributorName"
                    ).agg((pl.col("TotalRefillSales").sum()).alias("TotalRefillSales"))
                    pcc = avg_consumer_count.join(
                        lpg_cdcms_pcc_sales, on="DistributorName", how="outer"
                    ).drop("DistributorName_right")
            else:
                april_consumer_stats = april_consumer_stats.group_by("ZOName").agg(
                    (pl.col("ConsumerCount").sum()).alias("ConsumerCount_start")
                )
                current_consumer_stats = current_consumer_stats.group_by("ZOName").agg(
                    (pl.col("ConsumerCount").sum()).alias("ConsumerCount_current")
                )
                avg_consumer_count = (
                    april_consumer_stats.join(
                        current_consumer_stats, on="ZOName", how="outer"
                    )
                    .with_columns(
                        (
                            (
                ...
                lpg_cdcms_pcc_sales = lpg_cdcms_pcc_sales.group_by("ZOName").agg(
                    (pl.col("TotalRefillSales").sum()).alias("TotalRefillSales")
                )
                pcc = avg_consumer_count.join(
                    lpg_cdcms_pcc_sales, on="ZOName", how="outer"
                ).drop("ZOName_right")
            ...
            overall_num = pcc.group_by("temp").agg(
                (pl.col("TotalRefillSales").sum()).alias("TotalRefillSales"),
                (pl.col("avg_consumer_count").sum()).alias("avg_consumer_count"),
            )
            ...
            overall_num = overall_num.with_columns(
                (pl.col("pcc") * 365 / days_in_fy_till_yesterday).alias("pcc_prorated")
            ).drop("pcc")
            overall_num = overall_num.with_columns(
                pl.col("pcc_prorated").round(2).alias("pcc_prorated")
            )
            ...
            return {
                "status": True,
                "message": "success",
                "overal_number": overall_num["pcc_prorated"][-1],
                "data": pcc.to_dicts(),
            }
        except Exception as e:

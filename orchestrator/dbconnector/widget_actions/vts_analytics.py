...
async def get_shortage_data(filters, cross_filters, bu_ic, violation_types):
    ...
    shortage_query = VTSAnalyticsActions.apply_conditions_to_query(
        shortage_query, conditions
    )
    df_shortage = await VTSAnalyticsActions.execute_query(
        shortage_query, engine="polars"
    )
...
def create_shortage_detail(
    df_shortage: pl.DataFrame, bu_ic: bool, violation_types
) -> pl.DataFrame:
    ...
    per_mat = df_clean.group_by(
        ["tl_number", "invoice_match_key", "material_group_nm"]
    ...
    df_shortage_grouped = per_mat_labeled.group_by(
...
def merge_shortage_with_violations(
    ...
    if df_shortage.is_empty():
        if base_is_shortage:
            ...
            if not df_truck_master.is_empty() and not df_violations.is_empty():
                ...
                truck_lookup = df_truck_master.rename(
                    {"truck_no": "tl_number"}
                ).select(  # align join key name
                ...
                needs_fill = (
                    df_violations["transporter_name"].null_count() > 0
                )  # cheap metadata op [[Missing data](https://docs.pola.rs/user-guide/expressions/missing-data/#missing-data)]

                if needs_fill:
                    # Join on tl_number to bring in transporter_name from truck master
                    df_violations = (
                        df_violations.join(
                            truck_lookup, on="tl_number", how="left", suffix="_truck"
                        )
                        .with_columns(
    ...
    if not df_violations.is_empty() and "invoice_number" in df_violations.columns:
        ...
        df_violations = df_violations.with_columns(
            invoice_match_key=pl.when(cleaned_col_expr.is_null())
            .then(pl.lit(None))
            .when(cleaned_col_expr.str.contains("-"))
            .then(cleaned_col_expr.str.split("-").list.get(0))
            .otherwise(cleaned_col_expr)
        )
    ...
    if aggregate:
        ...
        df_shortage_agg = df_shortage.group_by(["tl_number", "invoice_match_key"]).agg(
        ...
        if base_is_shortage:
            ...
            if (
                df_violations.is_empty()
                or "invoice_number" not in df_violations.columns
            ):
        ...
        if "location_name" not in df_merged.columns:
            ...
            df_merged = df_merged.with_columns(
                location_name=(
                    pl.col("location_name").fill_null(pl.col("plant_nm")).fill_null("")
                    if "plant_nm" in df_merged.columns
                    else pl.col("location_name").fill_null("")
                )
            )
        ...
        if "zone" not in df_merged.columns:
            ...
            df_merged = df_merged.with_columns(
                zone=(
                    pl.col("zone").fill_null(pl.col("zone_nm")).fill_null("")
                    if "zone_nm" in df_merged.columns
                    else pl.col("zone").fill_null("")
                )
            )
        ...
        if "created_at" not in df_merged.columns:
            ...
            "created_at" in df_merged.columns
            and df_merged["created_at"].null_count() > 0
        ):
        ...
        df_merged = df_merged.with_columns(
            ...
            qty_shortage=(
                pl.col("qty_shortage").cast(pl.Float64, strict=False).fill_null(0)
            ),
        )
        ...
        # Get metadata
        df_shortage_meta = df_shortage.group_by(["tl_number", "invoice_match_key"]).agg(
            pl.col("plant_nm").first(),
            pl.col("zone_nm").first(),
            pl.col("invoice_number").first(),
            pl.col("invoice_date").first(),
        ...
        if "location_name" not in df_merged.columns:
            ...
            df_merged = df_merged.with_columns(
                location_name=(
                    pl.col("location_name").fill_null(pl.col("plant_nm"))
                    if "plant_nm" in df_merged.columns
                    else pl.col("location_name").fill_null("")
                )
            )
        ...
        if "zone" not in df_merged.columns:
            ...
            df_merged = df_merged.with_columns(
                zone=(
                    pl.col("zone").fill_null(pl.col("zone_nm"))
                    if "zone_nm" in df_merged.columns
                    else pl.col("zone").fill_null("")
                )
            )
        ...
        if "created_at" not in df_merged.columns:
            if "violation_date" in df_merged.columns:
                df_merged = df_merged.with_columns(
                    created_at=pl.col("violation_date").fill_null(
                        pl.col("invoice_date")
                        if "invoice_date" in df_merged.columns
                        else pl.lit("")
                    )
                )
            else:
        ...
        df_merged = df_merged.with_columns(
            ...
            qty_shortage_detail=(
                pl.col("qty_shortage_detail").cast(pl.String).fill_null("")
            ),
        )
    ...
    if not df_truck_master.is_empty() and not df_merged.is_empty():
        ...
        truck_lookup = df_truck_master.rename({"truck_no": "tl_number"}).select(
            ["tl_number", "transporter_name", "location_name", "zone"]
        )
        ...
        df_merged = df_merged.with_columns(
            transporter_name=pl.coalesce("transporter_name", "transporter_name_truck"),
            location_name=pl.coalesce("location_name", "location_name_truck"),
            zone=pl.coalesce("zone", "zone_truck"),
        )
...
async def download_streaming_data(df: pl.DataFrame, filename="violations"):
    ...
    df = df.with_columns(cs.datetime(time_zone="*").dt.replace_time_zone(None))
...
class VTSAnalyticsActions:
    ...
    def transform_key(key, query=None):
        ...
        if (
            query
            and "sales_trips_till_date" in query.lower()
            and key.lower() in ("transporter_code", "bu")
        ):
            return None
    ...
    def build_filter_conditions(filters, cross_filters, query):
        ...
        if filters:
            for rec in filters:
                ...
                if (
                    key.lower() == "bu"
                    and "sales_trips_till_date" not in query.lower()
                    and rec.value.upper() == "I&C"
                ):
                    continue
    ...
    def add_alert_type_conditions(conditions, alert_type):
        ...
        if alert_type.lower() == "blocked":
            conditions.append(
                "alert_status = 'Open' and vehicle_unblocked_date is null"
            )
        if alert_type.lower() == "acceptance_close":
            conditions.append(
                "alert_status = 'Close' and vehicle_unblocked_date is null"
            )
        elif alert_type.lower() == "auto_unblock":
            conditions.append(
                "alert_status = 'Close' AND mark_as_false = false AND vehicle_unblocked_date is not null"
            )
        elif alert_type.lower() == "manual_unblock":
            conditions.append(
                "alert_status = 'Close' AND mark_as_false = true and vehicle_unblocked_date is not null"
            )
        else:
    ...
    async def vts_dashboard_card_download(filters, cross_filters, drill_state, payload):
        ...
        try:
            ...
            df = df.with_columns([pl.col("alert_history").fill_null([])])
                ...
                [
                    ...
                    .then(
                        ...
                        .list.eval(
                            ...
                            .struct.field("employee_id")
                            .filter(
                                pl.element().struct.field("action_type")
                                == "Justification"
                            )
                        )
                        .list.first()
    ...
    async def pagination_df(df, payload):
        ...
        if str(payload.get("download", "")).lower() == "true":
            ...
            for col in merged_df.select_dtypes(
            ...
            merged_df = merged_df.loc[
                :, (merged_df.astype(str).apply(lambda x: x.str.strip() != "").any())
            ]
    ...
    async def vts_insite(filters, cross_filters, drill_state, payload):
        try:
            ...
            conditions = VTSAnalyticsActions.add_alert_type_conditions(
                conditions, alert_type
            )
            vts_insite_query = VTSAnalyticsActions.apply_conditions_to_query(
                query, conditions
            )
            print(vts_insite_query)
            df1 = await VTSAnalyticsActions.execute_query(
                vts_insite_query, engine="polars"
            )

            truck_master_query = (
                """SELECT truck_no, transporter_name FROM vts_truck_master"""
            )
            df_truck_master = await VTSAnalyticsActions.execute_query(
                truck_master_query, engine="polars"
            )
    ...
    async def vts_insite_violation(filters, cross_filters, drill_state, payload):
        ...
        try:
            ...
            bu_lpg = any(
                getattr(f, "key") == "bu" and getattr(f, "value") == "LPG"
                for f in filters
            )
            bu_tas = any(
                getattr(f, "key") == "bu" and getattr(f, "value") == "TAS"
                for f in filters
            )
            ...
            df_shortage = await get_shortage_data(
                filters, cross_filters, bu_ic, violation_types
            )
            ...
            final_df = final_df.with_columns(
                total_violations=pl.sum_horizontal(pl.col(violation_cols))
            )
            ...
            if final_df.is_empty():
                return {
                    "status": True,
                    "message": "No violation data found",
                    "data": [],
                }
            ...
            if group_by_col and group_by_col in final_df.columns:
                violation_cols = [v for v in all_violations if v in final_df.columns]
                # violation_cols.append('qty_shortage')
                agg_df = final_df.group_by(group_by_col).agg(
                    [pl.col(violation_cols).sum()]
                )
                ...
                return {"status": True, "message": "success", "data": agg_df.to_dicts()}
            ...
            if qlick_view == "zone" and not click_value:
                ...
                agg_df = final_df.group_by("zone").agg([pl.col(violation_cols).sum()])
                agg_df = agg_df.with_columns(
                    total_count=pl.sum_horizontal(pl.col(violation_cols))
                )
                return {
            ...
            if qlick_view == "zone" and click_value:
                ...
                agg_df = final_df.group_by("location_name").agg(
                    [pl.col(violation_cols).sum()]
                )
                agg_df = agg_df.with_columns(
                    total_count=pl.sum_horizontal(pl.col(violation_cols))
            ...
            # LOCATION -> TRANSPORTER DRILL
                ...
                agg_df = final_df.group_by("transporter_name").agg(
                    [pl.col(violation_cols).sum()]
                )
                agg_df = agg_df.with_columns(
                    total_count=pl.sum_horizontal(pl.col(violation_cols))
            ...
            # TRANSPORTER -> VEHICLE DRILL
                ...
                agg_df = final_df.group_by("tl_number").agg(
                    [pl.col(violation_cols).sum()]
                )
                agg_df = agg_df.with_columns(
                    total_count=pl.sum_horizontal(pl.col(violation_cols))
            ...
            # VEHICLE -> INVOICE DRILL
                ...
                if "invoice_number" in final_df.columns:
                    ...
                    agg_df = final_df.group_by(group_cols).agg(
                        [pl.col(violation_cols).sum()]
                    )
                    agg_df = agg_df.with_columns(
                        total_count=pl.sum_horizontal(pl.col(violation_cols))
    ...
    async def violation_percentages(filters, cross_filters, drill_state, payload):
        try:
            ...
            conditions = VTSAnalyticsActions.build_filter_conditions(
                filters, cross_filters, base_query
            )
            ...
            violation_counts = {
                col: int(row.get(col, 0) or 0) for col in violation_cols
            }
            ...
            emlock_data = await VTSAnalyticsActions.execute_query(
                emlock_query, engine="dict"
            )
            ...
            shortage_count = (
                shortage_result.get("trip_count", 0)
            ...
            percentages = {
                key: (
                    round(100 - (value * 100) / total_trip_count, 2)
    ...
    async def total_count_shortage(filters, cross_filters, drill_state, payload):
        try:
            ...
            if location_df.empty:
                return {
                    "status": False,
                    "message": "Location master data not available",
                    "data": [],
                }
            ...
            return {"status": True, "message": "Success", "trip_count": total_count}
    ...
    async def vts_drill_down_violation(filters, cross_filters, drill_state, payload):
        ...
        try:
            ...
            sql_parts = VTSAnalyticsActions._format_vts_drill_down_query_parts(
                payload, violation_type, filters, cross_filters
            )

            # Format the query with all parts
            formatted_query = base_query.format(**sql_parts)
            #  Execute query
            result_df = await VTSAnalyticsActions.execute_query(
                formatted_query, engine="polars"
            )
            ...
            return {"status": True, "message": "Success", "data": result_df.to_dicts()}
    ...
    @staticmethod
    def _format_vts_drill_down_query_parts(
        payload, violation_type, filters=None, cross_filters=None
    ):
        """
        ...
        zone_filter = (
            zone_filter_from_filters
            if zone_filter_from_filters
            else (f"AND vah.zone = '{zone}'" if zone and not zone_is_true else "")
        )
        location_filter = f"AND vah.location_name = '{location}'" if location else ""
        transporter_filter = (
            f"AND vtm.transporter_name = '{transporter}'" if transporter else ""
        )
        ...
        if date_wise:
            ...
            additional_select_part = (
                (", ".join(additional_select_cols) + ",")
                if additional_select_cols
                else ""
            )

            group_drill_part = f", {drill_col}" if drill_col else ""
            additional_group_part = (
                (", " + ", ".join(additional_group_cols))
            ...
            additional_group_part = (
                (", ".join(additional_group_cols)) if additional_group_cols else ""
            )
        ...
        try:
            ...
            if payload.get("download"):
                ...
                with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                    ...
                    df2["swipeoutl1"] = (
                        df2["swipeoutl1"].fillna("").astype(str).str.lower()
                    )
                    df2["swipeoutl2"] = (
                        df2["swipeoutl2"].fillna("").astype(str).str.lower()
                    )
                    # swipe_out_l1 = df2[df2["swipeoutl1"] == "false"]
                    # swipe_out_l2 = df2[df2["swipeoutl2"] == "false"]

                    final_df_pending = df2[
                        (df2["swipeoutl1"] == "false") | (df2["swipeoutl2"] == "false")
                    ].copy()
                    ...
                    if final_df_pending.empty:
                        pd.DataFrame(
                            [{"message": "No data found for emlock_open"}]
                        ).to_excel(writer, index=False, sheet_name="emlock_open")
                    else:
                        final_df_pending.to_excel(
                    ...
                    shortage_df = await VTSAnalyticsActions.execute_query(
                        shortage_query
                    )

                    if shortage_df is None or shortage_df.empty:
                        pd.DataFrame(
                            [{"message": "No data found for shortage_count"}]
                        ).to_excel(writer, index=False, sheet_name="shortage_count")
                    else:
                    ...
                    melt_df = melt_df.drop_duplicates(
                        subset=["invoice_number", "violation_type"], keep="first"
                    )
            ...
            vts_drill_query = VTSAnalyticsActions.apply_conditions_to_query(
                query, conditions
            )
            ...
            transporter_query = (
                """SELECT distinct truck_no, transporter_name FROM vts_truck_master"""
            )
            ...
            if violation_type and violation_type != "all":
                ...
                violation_filtered_df = merged_df[
                    merged_df[violation_type].fillna(0) != 0
                ].copy()
                violation_filtered_df = violation_filtered_df.sort_values(
    ...
    async def non_reporting_devices(filters, cross_filters, drill_state, payload):
        try:
            ...
            if status_filter == "live":
                conditions.append(
                    "completed_trip = 'open' AND completed_trip_auto_dc = 'open'"
                )
            elif status_filter == "closed":
                # conditions.append("'closed' IN (completed_trip, completed_trip_auto_dc)")
                conditions.append(
                    "completed_trip='closed' OR completed_trip_auto_dc='closed'"
                )
        ...
        try:
            ...
            if selected_tt:
                ...
                trip_df = df.with_columns(
                    pl.coalesce(
                        [pl.col("event_start_datetime"), pl.col("event_end_datetime")]
                    )
                    .cast(pl.Date)
                    .cast(str)
                    .alias("created_at")
            ...
            if payload.get("download") == "true":
                ...
                if "violation_type" in df.columns:
                    df = df.with_columns(
                        pl.col("violation_type").replace(
                            violation_mapping, default=pl.col("violation_type")
                        )
                    )
    ...
    async def safety_compliance(filters, cross_filters, drill_state, payload):
        try:
            if payload.get("download"):
                print(
                    "Download requested — generating Excel with multiple sheets (Polars only)"
                )
                table_names = [
                    tbl.strip() for tbl in drill_state.split(",") if tbl.strip()
                ]
                if not table_names:
                    return JSONResponse(
                        {"error": "Missing drill_state (table names)"}, status_code=400
                    )
                output = io.BytesIO()
                for table_name in table_names:
                    ...
                    df = await VTSAnalyticsActions.execute_query(
                        final_query, engine="polars"
                    )
            ...
            if is_date_wise:
                ...
                df = (
                    df.with_columns(pl.col("event_date").dt.date().alias("created_at"))
                    .group_by(group_cols)
                    .agg(
                        pl.col("invoice_no").n_unique().alias("invoice_count"),
                        pl.col("tt_number").n_unique().alias("vehicle_count"),
    ...
    @staticmethod
    async def safety_compliance_percentage(
        filters, cross_filters, drill_state, payload
    ):

        total_trips_count = vts_query.vts_query.get("total_trips")
        conditions = VTSAnalyticsActions.build_filter_conditions(
            filters, cross_filters, total_trips_count
        )
        total_trips_count = VTSAnalyticsActions.apply_conditions_to_query(
            total_trips_count, conditions
        )
        ...
        try:
            ...
            percentages = {
                k: round(100 - (v / total_length) * 100, 2) for k, v in counts.items()
            }
            return {
                "status": True,
                "message": "Success",
    ...
    @staticmethod
    async def location_level_voilation_breakup(
        filters, cross_filters, drill_state, payload
    ):
        try:
            ...
            for _, row in loc_master_df.iterrows():
                loc_map[row["sap_id"]] = {"zone": row["zone"], "name": row["name"]}
    ...
    async def vts_alerts_violations(filters, cross_filters, drill_state, payload):
        try:
            ...
            alerts_query = VTSAnalyticsActions.apply_conditions_to_query(
                base_query, conditions
            )

            # Execute queries
            alerts_df = await VTSAnalyticsActions.execute_query(
                alerts_query, engine="polars"
            )
            ...
            grouped = alerts_df.group_by([group_by_column, "violation_type"]).agg(
    ...
    async def violation_trends_over_time(filters, cross_filters, drill_state, payload):
        try:
            ...
            alerts_df = alerts_df.filter(
                ...
                & (pl.col("period").cast(pl.Utf8).str.strip_chars() != "")
            )

            if alerts_df.height == 0:
                return {"status": True, "message": "success", "data": []}

            grouped = alerts_df.group_by(["period", "violation_type"]).agg(
                pl.len().alias("count")
            )
    ...
    async def violation_details(filters, cross_filters, drill_state, payload):
        try:
            ...
            query = base_query.format(
                period_expr=period_expr, violation_type=violation_type
            )
            final_query = VTSAnalyticsActions.apply_conditions_to_query(
                query, conditions
            )
            ...
            for col, total_count in overall_instance_totals.items():
                instance_breakup[col] = {
                    "total_count": total_count,
                    "percentage": (
                        round((total_count / grand_total) * 100, 2)
                        if grand_total > 0
                        else 0
                    ),
                }
            ...
            for _, row in df.iterrows():
                ...
                formatted_date = VTSAnalyticsActions.format_date(
                    row["period"], drill_state
                )

                period_data.append(
                    {"date": formatted_date, "value": {"counts": counts}}
                )
    ...
    async def alert_summary(filters, cross_filters, drill_state, payload):
        try:
            ...
            formatted_query = query.format(
                group_by_column=group_by_column, violation_type=violation_type
            )
            final_query = VTSAnalyticsActions.apply_conditions_to_query(
                formatted_query, conditions
            )
    ...
    async def card_chart_shortage(filters, cross_filters, drill_state, payload):
        try:
            ...
            shortage_resp = await VTSAnalyticsActions.execute_tibco_query(
                conn, shortage_tibco_query
            )
            ...
            vts_history["invoice_prefix"] = vts_history["invoice_number"].apply(
                lambda x: x.split("-")[0]
            )
            ...
            violation_cols = [
                col
                for col in vts_history.columns
                if col not in ["invoice_number", "invoice_prefix"]
            ]
            total_violation_count = vts_history[violation_cols].sum().sum()

            shortage_percentage = (
                round((total_qty_shortage / total_violation_count) * 100, 2)
    ...
    async def integrate_shortage_trips(filters, cross_filters, drill_state, payload):
        ...
        transporter_filter = next(
            (f for f in trips_filters if getattr(f, "key") == "transporter_name"), None
        )
        trips_query = f"""
            SELECT *     
            FROM 
        ...
        """
        sql_filters = [
            f for f in filters if getattr(f, "key", None) != "transporter_name"
        ]
        conditions = VTSAnalyticsActions.build_filter_conditions(
            sql_filters, cross_filters, trips_query
        )
        trips_query = VTSAnalyticsActions.apply_conditions_to_query(
            trips_query, conditions
        )
            ...
            if key and val and key.lower() == "transporter_name":
                ...
                if cond == "equals":
                    filtered_trips_df = filtered_trips_df[
                        filtered_trips_df[df_col] == val
                    ]
                elif cond == "in":
                    if not isinstance(val, list):
                        val = [val]
                    filtered_trips_df = filtered_trips_df[
                        filtered_trips_df[df_col].isin(val)
                    ]
        ...
        if "invoice_date" in filtered_trips_df.columns:
            filtered_trips_df["invoice_date"] = pd.to_datetime(
            ...
            if filtered_trips_df["invoice_date"].dt.tz is None:
                ...
                filtered_trips_df["invoice_date"] = filtered_trips_df[
                    "invoice_date"
                ].dt.tz_convert(ist)
        ...
        def compute_group_summary(df, group_cols):
            ...
            for keys, group in df.groupby(current_col, dropna=False):
                ...
                if (
                    "material_group_nm" in group.columns
                    and "qty_shortage" in group.columns
                ):
                    bif_df = (
                        group.groupby("material_group_nm", dropna=False)["qty_shortage"]
                        .sum()
                        .reset_index()
                    )
        ...
        if "vehicle_id" in filter_keys:
            ...
            group_cols = ["zone_nm"]
        if (
            "material_group_nm" not in filtered_trips_df.columns
            and "item_no" in filtered_trips_df.columns
        ):
            filtered_trips_df.rename(
                columns={"item_no": "material_group_nm"}, inplace=True
            )
        ...
        if date_wise is True or date_wise == "true":
            ...
            date_wise_df = (
                filtered_trips_df.groupby("created_at", as_index=False)
                .agg(
        ...
        if payload.get("table") == "true":
            filtered_trips_df = filtered_trips_df.rename(
                columns={
                    "material_group_nm": "product_bifurcation",
            ...
            table_df = filtered_trips_df.groupby(
                ["vehicle_id", "invoice_no"], as_index=False
            ).agg(
                {
        ...
        filtered_trips_df = filtered_trips_df.replace(
            [float("inf"), float("-inf")], None
        )
        filtered_trips_df = filtered_trips_df.where(pd.notnull(filtered_trips_df), None)
        print(
            "TOTAL SHORTAGE BEFORE GROUPING =",
            filtered_trips_df["qty_shortage"].astype(float).sum(),
        )
    ...
    async def get_unblock_ageing(filters, cross_filters, drill_state, payload):
        try:
            ...
            closed_query = await widget_actions.WidgetActions.apply_filter_drilldown(
                closed_query, access_filters, drill_state
            )
            ...
            # Execute queries
            resp = await urdhva_base.BasePostgresModel.get_aggr_data(
                query=closed_query, limit=0
            )
            shortage_resp = await urdhva_base.BasePostgresModel.get_aggr_data(
                query=shortage_query, limit=0
            )
            ...
            df["vehicle_blocked_end_date"] = pd.to_datetime(
                df["vehicle_blocked_end_date"]
            ).dt.tz_localize(None)
            df["vehicle_blocked_start_date"] = pd.to_datetime(
                df["vehicle_blocked_start_date"]
            ).dt.tz_localize(None)
            ...
            df["unblocking_days"] = (
                (df["vehicle_unblocked_date"] - df["created_at"]).dt.total_seconds()
                / 86400
            ).clip(lower=0)
            # Violation counts
            violation_counts = df.pivot_table(
            ...
            avg_unblocking["average_unblocking"] = (
                avg_unblocking["total_unblocking_days"] / avg_unblocking["total_alerts"]
            ).round(2)
            ...
            if drill_state:
                ...
                if filters:
                    ...
                    if "zone" in filter_keys and "location_name" not in filter_keys:
                        ...
                        and "transporter_code" not in filter_keys
                    ):
                        group_by_keys = ["zone", "location_name", "transporter_code"]
                    elif (
                        "zone" in filter_keys
                        and "location_name" in filter_keys
                        and "transporter_code" in filter_keys
                        and "tt_number" not in filter_keys
                    ):
                        group_by_keys = [
                            "zone",
                            "location_name",
            ...
            if str(payload.get("download", "")).lower() == "true":
                return await download_streaming_data(
                    pl_df, filename="unblock_ageing_data"
                )
    ...
    async def get_emlock_open_data(filters, cross_filters, drill_state, payload):
        ...
        try:
            ...
            dedup_cols = [
                c
                for c in ["invoice_number", "trucknumber", "zone", "location_name"]
                if c in df.columns
            ]
            if dedup_cols:
            ...
            base = df.group_by(base_cols).agg(
                [
            ...
            filtered = base.filter(pl.col("has_swipeoutl1") | pl.col("has_swipeoutl2"))
            ...
            grouped = filtered.group_by(group_by).agg(
                [
            ...
            return {
                ...
                "swipe_out_l1_count": int(
                    filtered.filter(pl.col("has_swipeoutl1"))[
                        "invoice_number"
                    ].n_unique()
                ),
                "swipe_out_l2_count": int(
                    filtered.filter(pl.col("has_swipeoutl2"))[
                        "invoice_number"
                    ].n_unique()
                ),
    ...
    async def power_disconnection(filters, cross_filters, drill_state, payload):
        try:
            ...
            merged_df = vts_df.merge(
                df_transporter, left_on="tl_number", right_on="truck_no", how="left"
            )
            ...
            violation_filtered_df = merged_df[
                merged_df[violation_type].fillna(0) >= 6
            ].copy()
            ...
            for key in ["zone", "location_name", "transporter_name"]:
                if payload.get(key):
                    violation_filtered_df = violation_filtered_df[
                        violation_filtered_df[key] == payload[key]
                    ]
            # --- DATE-WISE AGGREGATION (move here!) ---
            date_wise = payload.get("date_wise") or payload.get("payload", {}).get(
                "date_wise"
            )
            if date_wise is True or date_wise == "true":
                violation_filtered_df["created_at"] = pd.to_datetime(
                    violation_filtered_df["created_at"]
                )
                ist = pytz.timezone("Asia/Kolkata")
                if violation_filtered_df["created_at"].dt.tz is None:
                    ...
                    violation_filtered_df["created_at"] = violation_filtered_df[
                        "created_at"
                    ].dt.tz_convert(ist)

                violation_filtered_df["date"] = violation_filtered_df[
                    "created_at"
                ].dt.strftime("%Y-%m-%d")
            ...
            if selected_tl:
                violation_filtered_df = violation_filtered_df[
                    violation_filtered_df["tl_number"] == selected_tl
                ]
                ...
                invoice_df = violation_filtered_df.sort_values(
                    by="created_at", ascending=True
                )
                invoice_df = invoice_df[
                    ["invoice_number", "created_at", violation_type]
                ]
                ...
                .agg(
                    {
                        ...
                        violation_type: [
                            "count",
                            "sum",
                        ],  # count of records >= 6, and sum of actual values
                    }
                )
                .reset_index()
            )
            ...
            if group_col != "tl_number":
                summary_df["vehicle_count"] = (
                    violation_filtered_df.groupby(group_col)["tl_number"]
                    .nunique()
                    .values
                )
    ...
    async def risk_score(filters, cross_filters, drill_state, payload):
        ...
        try:
            ...
            limit = (
                0
                if payload.get("download") == "true"
                else payload.get("page_size", 100)
            )
            conditions = []
            ...
            if not table_name:
                return {
                    "status": False,
                    "message": "table_name not provided in payload",
                    "data": [],
                }
            ...
            if action == "risk_score_trends":
                ...
                if table_name not in TRENDS_CONFIG:
                    return {
                        "status": False,
                        "message": f"Table {table_name} not supported for trends",
                        "data": [],
                    }
                ...
                if not clicked_value:
                    return {
                        "status": False,
                        "message": f"{config['payload_key']} not provided in payload",
                        "data": [],
                    }
                ...
                if not violation_query_template:
                    return {
                        "status": False,
                        "message": f"Query template not found for {config['query_key']}",
                        "data": [],
                    }
                ...
                response = await urdhva_base.BasePostgresModel.get_aggr_data(
                    query=violation_query, limit=0, skip_total=True
                )
                ...
                if violation_data:
                    ...
                    violation_columns_map = vts_query.vts_query.get(
                        "violation_columns_map", {}
                    )

                    # Convert violation_date to date format and format for display
                    df = df.with_columns(
                        pl.col("violation_date")
                        .cast(pl.Date)
                        .cast(pl.Utf8)
                        .str.replace_all(r"(\d{4})-(\d{2})-(\d{2})", r"$2-$3-$1")
                        .alias("violation_date_formatted")
                    )

                    # Get all violation type columns that exist
                    violation_types = [
                        col for col in violation_columns_map.keys() if col in df.columns
                    ]
                    ...
                    grouped_df = (
                        df.group_by("violation_date_formatted")
                        .agg(agg_cols)
                    ...
                    for row in grouped_df.iter_rows(named=True):
                        ...
                        violations = [
                            {
                                "violation_type": k,
                                "count": int(row[k]) if row[k] is not None else 0,
                            }
                ...
                if not grouped_data:
                    return {
                        "status": True,
                        "message": f"No violation trends found for {clicked_value}",
                        "data": [],
                    }
            ...
            if table_name in CLICK_HANDLERS:
                ...
                if clicked_value:
                    ...
                    response = await urdhva_base.BasePostgresModel.get_aggr_data(
                        query=query, limit=0, skip_total=True
                    )
                    ...
                    if loc_val and data:
                        if loc_val is True:
                            ...
                            if "location_name" in df.columns:
                                grouped = df.group_by("location_name").agg(
                                    pl.len().alias("count")
                                )
                                data = grouped.to_dicts()
                            ...
                            data = [
                                row
                                for row in data
                                if row.get("location_name") == loc_val
                            ]
                    ...
                    if payload.get("download") == "true":
                        ...
                        if not df.empty:
                            ...
                            pl_df = pl.from_pandas(df)
                            return await download_streaming_data(
                                pl_df, filename=f"{table_name}_{clicked_value}"
                            )
            ...
            if column_filters and isinstance(column_filters, dict):
                for col, search_val in column_filters.items():
                    if search_val:  # Ensure there is a value to search for
                        ...
                        conditions.append(
                            f"CAST(\"{col}\" AS TEXT) ILIKE '%{search_val}%'"
                        )
            ...
            if not is_download:
                ...
                filtered_count_query = VTSAnalyticsActions.apply_conditions_to_query(
                ...
                total_records = (
                    count_resp["data"][0]["count"] if count_resp.get("data") else 0
                )
            ...
            filtered_data_query = VTSAnalyticsActions.apply_conditions_to_query(
                base_query, conditions
            )
            resp = None
            ...
            if is_download:
                ...
                if table_name == "cluster_master":
                    ...
                    all_master_df = strip_tz(
                        pd.DataFrame(
                            (
                                await urdhva_base.BasePostgresModel.get_aggr_data(
                                    query=VTSAnalyticsActions.apply_conditions_to_query(
                                        base_query, conditions
                                    ),
                                    limit=0,
                                    skip_total=True,
                    ...
                    if cluster_ids:
                        ...
                        ev_conds = VTSAnalyticsActions.build_filter_conditions(
                            filters, cross_filters, ev_base
                        )
                        vd = payload.get("version_date", "")
                        ...
                        if not event_df.empty and "cluster_id" in event_df.columns:
                            event_df["cluster_id"] = event_df["cluster_id"].apply(
                                norm_id
                            )
                    ...
                    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                        wb = writer.book
                        lnk = wb.add_format(
                            {"font_color": "#1155CC", "underline": True}
                        )
                        bck = wb.add_format(
                            {
                                "font_color": "#C00000",
                        ...
                        if "cluster_id" in cols:
                            ...
                            for r, v in enumerate(all_master_df["cluster_id"], 1):
                                sn = norm_id(v)[:31]
                                sw.write_url(
                                    r, ci, f"internal:'{sn}'!A1", lnk, norm_id(v)
                                )
                        ...
                        if not event_df.empty and "cluster_id" in event_df.columns:
                            for cid, sdf in event_df.groupby("cluster_id", sort=True):
                                sn = str(cid)[:31]
                                sdf.to_excel(
                                    writer, index=False, sheet_name=sn, startrow=1
                                )
                    ...
                    async def stream_csv():
                        ...
                        try:
                            result = await session.stream(
                                text(filtered_data_query).execution_options(
                                    yield_per=1000
                                )
                            )
                            cols_fetched = False
                            async for row in result:
                                ...
                                for v in row:
                                    if (
                                        hasattr(v, "tzinfo")
                                        and getattr(v, "tzinfo") is not None
                                    ):
                ...
                # BRANCH 3: all other tables - single-sheet XLSX (small data, BytesIO)
                    ...
                    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                        strip_tz(pd.DataFrame(resp["data"])).dropna(
                            axis=1, how="all"
    ...
    async def risk_score_cluster_map(filters, cross_filters, drill_state, payload):
        ...
        try:
            ...
            event_lat_lon = payload.get("clicked_event_lat_lon") or payload.get(
                "event_lat_lon"
            )
            ...
            if not cluster_id and not version_date:
                return {
                    "status": False,
                    "message": "Either cluster_id or version_date must be provided in payload",
                    "data": [],
                }
            ...
            if filters:
                ...
                built_conds = VTSAnalyticsActions.build_filter_conditions(
                    filters, cross_filters, "SELECT * FROM public.cluster_master"
                )
            ...
            if cluster_id:
                ...
                master_query = vts_query.vts_query.get(
                    "cluster_map_master_drilldown"
                ).format(master_cond=master_cond, filter_str=filter_str)

                event_cond = (
                    f" AND e.event_lat_lon = '{str(event_lat_lon).replace(chr(39), chr(39)+chr(39))}'"
                    if event_lat_lon
                    else ""
                )
                event_query = vts_query.vts_query.get(
                ...
                master_resp = await urdhva_base.BasePostgresModel.get_aggr_data(
                    query=master_query, limit=0, skip_total=True
                )
                ...
                event_resp = await urdhva_base.BasePostgresModel.get_aggr_data(
                    query=event_query, limit=0, skip_total=True
                )
                ...
                resp_msg = (
                    f"Cluster coordinate events for {event_lat_lon}"
                    if event_lat_lon
                    else f"Cluster map details for cluster_id {cluster_id}"
                )
                return {
                    "status": True,
                    "message": resp_msg,
                ...
                ).format(safe_date=safe_date, filter_str=filter_str)
                ...
                count_query = vts_query.vts_query.get(
                ...
                count_resp = await urdhva_base.BasePostgresModel.get_aggr_data(
                    query=count_query, limit=0, skip_total=True
                )
                ...
                counts_map = {
                    str(c.get("c_id")): c.get("event_count", 0)
                    for c in count_data
                    if c.get("c_id")
                }
                coords_map = {
                    str(c.get("c_id")): c.get("event_coords", [])
                    for c in count_data
                    if c.get("c_id")
                }
                ...
                return {
                    ...
                    + risk_bands[
                        "Critical"
                    ],  # Optionally combine depending on map thresholds
    ...
    async def cluster_wise_daily_trends(filters, cross_filters, drill_state, payload):
        try:
            ...
            base_conds = VTSAnalyticsActions.build_filter_conditions(
                filters, cross_filters, "SELECT * FROM public.clusterwise_event"
            )
            if base_conds:
            ...
            query = vts_query.vts_query.get("cluster_wise_daily_trends").format(
                filter_sql=filter_sql
            )
            resp = await urdhva_base.BasePostgresModel.get_aggr_data(
                query=query, skip_total=True
            )
    ...
    async def risk_score_max_date(filters, cross_filters, drill_state, payload):
        try:
            resp = await urdhva_base.BasePostgresModel.get_aggr_data(
                query="SELECT MAX(version_date)::date AS max_date FROM public.tt_risk_score",
                skip_total=True,
            )
            max_date = (
                str(resp["data"][0]["max_date"])[:10]
                if resp.get("data") and resp["data"][0].get("max_date")
                else None
            )
            return {
                "status": bool(max_date),
                "message": (
                    "Last updated date fetched successfully"
                    if max_date
                    else "No data found"
                ),
                "data": max_date,
    ...
    async def adding_device(filters, cross_filters, drill_state, payload):
        try:
            ...
            if truck_df.height == 0:
                return {"status": False, "message": "trucks is blocked", "data": {}}
            ...
            # Join and rename
            final_df = truck_df.join(
                location_df, left_on="LOCN_CODE", right_on="sap_id", how="inner"
            )

            if final_df.height == 0:
                return {
                    "status": False,
                    "message": "No valid truck-location mapping found",
                    "data": {},
                }
            ...
            shortage_data = await VTSAnalyticsActions.execute_query(
                shortage_tibco_query
            )
    ...
    async def vts_accept_and_block(filters, cross_filters, drill_state, payload):
        try:
            ...
            condition = VTSAnalyticsActions.build_filter_conditions(
                filters, cross_filters, base_query
            )
            ...
            final_condition = ""
            if condition:
                condition = condition.replace("bu", "a.bu").replace(
            ...
            merged_df = await VTSAnalyticsActions.execute_query(
                final_query, engine="polars"
            )
            ...
            system_ids = (
                merged_df.filter(pl.col("doc_type") == "System Generated")
                .select("alert_id")
                .unique()
            )

            user_ids = (
                merged_df.filter(pl.col("doc_type") == "User Created")
                .select("alert_id")
                .unique()
            )
            ...
            system_only_df = merged_df.join(
                system_only_ids, on="alert_id", how="inner"
            ).unique(subset=["alert_id"])

            both_ids = system_ids.join(user_ids, on="alert_id", how="inner")
            both_df = merged_df.join(both_ids, on="alert_id", how="inner").unique(
                subset=["alert_id"]
            )

            system_only_df, both_df = (
                system_only_df.drop("file_path", "report_type", strict=False),
                both_df.drop("file_path", "report_type", strict=False),
            )
            ...
            if payload.get("download") == "true":
                ...
                return await download_streaming_data(
                    combined, filename="Show_Cause_Notice"
                )
            ...
            return {
                ...
                "message": "success",
                "data": {
                    "system_only": system_only_df.height,
                    "system_and_user": both_ids.height,
                },
            }
        except Exception as e:

...
class VTSAlertManager(alert_factory.AlertFactory):
    ...
    async def create_bu_alert(
        ...
        if alert_data["base_location_id"].startswith("4"):
            ...
            vts_location_data = await hpcl_ceg_model.LocationMaster.get_aggr_data(
                query, limit=0
            )
            if vts_location_data.get("data", []):
                location_details = vts_location_data["data"][0]
        else:
            status, location_details = await alert_helper.get_location_details(
                alert_data["location_type"], alert_data["base_location_id"]
            )
            if not status:
                logger.info(
                    f"Error in finding location {alert_data['base_location_id']} "
                    f"for bu {alert_data['location_type']} - {location_details}"
                )
                location_details = {}

        instance_data, violation_name, vts_alert_history_ids = (
            await vts_analysis.get_vts_instance(
                alert_data["tl_number"],
                alert_data["base_location_id"],
                alert_data["location_type"],
                alert_data.get("tt_type", ""),
            )
        )
        ...
        interlock_name, instance = helpers.get_interlock_name_and_instance_name_vts(
            interlock_details.get("interlock_name"), instance_count + 1
        )
        vts_alert_data.update(interlock_details)
        vts_alert_data["location_data"] = (
            location_details.__dict__
        ...
        vts_alert_data["vehicle_blocked_start_date"] = (
            urdhva_base.utilities.get_present_time().isoformat()
        )
        days_to_add = (
            instance_data["block_duration"]
            if not await vts_analysis.is_vehicle_blacklisted(alert_data["tl_number"])
            else 1826
        )
        vts_alert_data[
            "vehicle_blocked_end_date"
        ] = urdhva_base.utilities.get_present_time() + datetime.timedelta(
            days=days_to_add
        )
        ...
        if instance_data["instance"] == "Instance - 1":
            query = (
                f"update vts_truck_details set instance_1 = 1, truck_status = 'BLOCKED', block_start_datetime = '{vts_alert_data['vehicle_blocked_start_date']}', block_end_datetime = '{vts_alert_data['vehicle_blocked_end_date']}' "
        ...
        if instance_data["instance"] == "Instance - 2":
            ...
            query = (
                f"update vts_truck_details set instance_2 = 1, truck_status = 'BLOCKED', block_start_datetime = '{vts_alert_data['vehicle_blocked_start_date']}', block_end_datetime = '{vts_alert_data['vehicle_blocked_end_date']}' "
                f"where truck_regno = '{tl_number}'"
            )
        if instance_data["instance"] == "Instance - 3":
            ...
            query = (
                f"update vts_truck_details set instance_3 = 1, truck_status = 'BLOCKED', block_start_datetime = '{vts_alert_data['vehicle_blocked_start_date']}', block_end_datetime = '{vts_alert_data['vehicle_blocked_end_date']}' "
                f"where truck_regno = '{tl_number}'"
            )
        if query:
    ...
    async def create_bu_alert_old(
        ...
        try:
            ...
            status, location_details = await alert_helper.get_location_details(
                alert_data["location_type"], alert_data["location_id"]
            )
            if not status:
                logger.info(
                    f"Error in finding location {alert_data['location_id']} "
                    f"for bu {alert_data['location_type']} - {location_details}"
                )
            ...
            if isinstance(alert_data, dict):
                alert_records = [
                    alert_data
                ]  # Wrap single record in a list for uniform processing
            elif isinstance(alert_data, list):
                alert_records = alert_data  # Use directly if it's already a list
            else:
                logger.error(
                    "Invalid alert_data format. Expected dict or list of dicts."
                )
                return
            ...
            for record in alert_records:
                try:
                    ...
                    for key, details in vts_mapping.vts_interlock_mapping.items():
                        ...
                        alert_count = await check_violation_count.CheckViolationCount().check_violation_count(
                            record["location_id"],
                        ...
                        maintenance_time = datetime.datetime.strptime(
                            maintenance_time, "%Y-%m-%d"
                        )
                        ...
                        if alert_count["count"]:
                            ...
                            if last_created_at > maintenance_time:
                                logger.info(
                                    f"Instance Already created for this for {key} vehicle_number:{record['tl_number']} bu:{record['location_type']} sap_id:{record['location_id']}"
                                )
                                ...
                                query = (
                                    f"location_id='{record['location_id']}' and tl_number='{record['tl_number']}' "
                                    f"and {key}>=1 and created_at::DATE>='{maintenance_time}' and location_type='{record['location_type']}' "
                                    f"and auto_unblock!='false'"
                                )
                                data = await hpcl_ceg_model.VtsAlertHistory.get_all(
                        ...
                        if data["count"] > details["alert_threshold"]:
                            ...
                            if alert_count["count"]:
                                for i in range(alert_count["count"]):
                                    ...
                                    entry = {
                                        "alert_type": previous_data[i][
                                            "interlock_name"
                                        ],
                                        "alert_id": previous_data[i]["unique_id"],
                                        "created_at": previous_data[i][
                                            "created_at"
                                        ].strftime("%Y-%m-%d %H:%M:%S %p"),
                                        "vehicle_number": previous_data[i][
                                            "vehicle_number"
                                        ],
                                    }
                            ...
                            interlock_details = (
                                utilities.interlock_mapping.get_interlock_name(
                                    record["location_type"],
                                    details["alerting_rules"][str(altcount)][
                                        "interlock_name"
                                    ],
                                )
                            )
                            ...
                            # continue
                            tripinterlockname = await check_violation_count.CheckViolationCount().checktripcount(
                                record["location_id"],
                                record["location_type"],
                                record["tl_number"],
                                key,
                            ...
                            if (
                                tripinterlockname
                                and tripinterlockname["violation_type"] == key
                            ):
                                logger.info(
                                    f"alert already exists for bu: {vts_alert_data["bu"]} sap_id:{vts_alert_data["sap_id"]} for {key} vehicle_number:{vts_alert_data["vehicle_number"]}"
                                )
                                continue
                            # checking if alert already is in active or not based on interlock_name if already in active
                            # then continue.
                            interlocknamecheck = await check_violation_count.CheckViolationCount().check_interlock(
                                record["location_id"],
                                record["location_type"],
                                record["tl_number"],
                            ...
                            if (
                                interlocknamecheck
                                and interlocknamecheck["interlock_name"]
                                == interlock_details["interlock_name"]
                            ):
                                logger.info(
                                    f"alert already exists for bu: {vts_alert_data["bu"]} sap_id:{vts_alert_data["sap_id"]} for {key} vehicle_number:{vts_alert_data["vehicle_number"]}"
                                )
                            ...
                            vts_alert_data["alert_section"] = "VTS"
                            vts_alert_data["alert_history"] = alert_history
                            vts_alert_data["clear_count"] = details["alerting_rules"][
                                str(altcount)
                            ]["clear_count"]
                            vts_alert_data["severity"] = details["severity"]
                            vts_alert_data["alert_timestamp"] = (
                                datetime.datetime.strptime(
                                    record["report_duration"], "%Y-%m-%d %H:%M:%S"
                                ).isoformat()
                            )
                            vts_alert_data["vts_alert_history_ids"] = (
                                vts_alert_history_ids
                            )
                            vts_alert_data["transporter_name"] = ""
                            vts_alert_data["transporter_code"] = alert_data["vendor_id"]
                            vts_alert_data["device_id"] = (
                                f"Instance {altcount+1}: {key}"
                            )
                            vts_alert_data["device_name"] = (
                                f"Instance {altcount+1}: {key}"
                            )
                            vts_alert_data["vehicle_blocked_start_date"] = (
                                recv_time.isoformat()
                            ...
                            vts_alert_data["vehicle_blocked_end_date"] = (
                                helpers.get_time_stamp_by_delta(
                                    days=details["alerting_rules"][str(altcount)][
                                        "block_duration"
                                    ],
                                    with_month_start_day=False,
                                    ascending=True,
                                ...
                                "data": [
                                    {
                                        ...
                                        "vehicle_blocked_start_date": recv_time.strftime(
                                            "%Y-%m-%d"
                                        ),
                                        "vehicle_blocked_end_date": helpers.get_time_stamp_by_delta(
                                            days=details["alerting_rules"][
                                                str(altcount)
                                            ]["block_duration"],
                                            with_month_start_day=False,
                                            ascending=True,
                    ...
                    logger.error(
                        f"Exception in processing alert data {e}, Traceback "

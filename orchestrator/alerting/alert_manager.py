...
async def get_function_mapping():
    function_map = {
        "Justification": "is_justify",
        "Rejected": "is_rejected",
        "Approved": "is_approved",
        "Override": "is_override",
        "interLockOk": "is_interlockok",
        "excApprovalTimeExp": "is_exc_approval_time_exp",
        "Message": "is_message",
        "Raised": "is_raised",
        ...
        "Tripped": "is_tripped",
        "VTS": "is_vts",
        "AcceptClose": "is_accept",
        "InvalidAlert": "is_invalid",
        "FalseAlert": "is_false",
        "ValidAlert": "is_valid",
        "Blocked": "is_blocked",
        "UnBlocked": "is_unblocked",
        "Interrupt": "is_interrupt",
        "Request": "is_extra_days",
    }
    return function_map
...
async def close_vts_alert(alert_data, input_data):
    ...
    if not input_data.get("doc_link", ""):
        input_data["doc_link"] = await get_doc_link_from_alert_history(alert_data)
    un_block_datetime = (
        str(alert_data["vehicle_blocked_end_date"].strftime("%Y-%m-%d %H:%M:%S"))
        if input_data.get("is_autoblock", False)
        else str(urdhva_base.utilities.get_present_time().strftime("%Y-%m-%d %H:%M:%S"))
    )
    approved_datetime = await get_approved_remarks(
        alert_data, is_approved=False, get_approved_time=True
    )
    params = {
        ...
        "BlockStartDate": str(
            alert_data["vehicle_blocked_start_date"].strftime("%Y-%m-%d %H:%M:%S")
        ),
        "BlockEndDate": str(
            alert_data["vehicle_blocked_end_date"].strftime("%Y-%m-%d %H:%M:%S")
        ),
        "WaivedOff": 0,
...
async def close_emlock_alert(alert_data, input_data):
    ...
    if not isinstance(alert_data, dict):
        alert_data = alert_data.__dict__
    meta_data = {}
    if alert_data["violation_type"] in [
        "FanNotGenerated",
    ...
    params = {
        ...
        "acknowledgedTime": urdhva_base.utilities.get_present_time().strftime(
            "%Y-%m-%d %H:%M:%S"
        ),
...
async def get_approved_remarks(
    alert_data: dict, is_approved=False, get_approved_time=False
):
                ...
                if alert_history["action_type"] == "Approved":
                    try:
                        _date = datetime.datetime.strptime(
                            alert_history["processed_time"], "%Y-%m-%dT%H:%M:%S.%f"
                        ).strftime("%Y-%m-%d %H:%M:%S")
                        return _date
                    except Exception:
                        return str(
                            urdhva_base.utilities.get_present_time().strftime(
                                "%Y-%m-%d %H:%M:%S"
                            )
                        )
        return str(
            urdhva_base.utilities.get_present_time().strftime("%Y-%m-%d %H:%M:%S")
        )
    for alert_history in alert_data.get("alert_history", []):
        if is_approved:
            if alert_history.get("is_approved", False):
            ...
            if alert_history["action_type"] not in [
                "Created",
                "Approved",
                "VTS",
                "Active",
...
async def vts_alert_closer(alert_data, input_data):
    ...
    notify_email.publish_message(
        **{
            ...
            "body": read_template(
                "/opt/ceg/algo/orchestrator/notification_templates/interlock_alert_closure.html",
                data=data,
            ),
            "html_content": True,
        }
...
async def emlock_alert_closer(alert_data, input_data):
    ...
    await close_emlock_alert(alert_data, input_data)
...
async def _is_close_alert(input_data):
    ...
    """
    action_data = connection_mapping.alert_action.get(input_data["bu"])[
        input_data["alert_section"]
    ]
...
async def _is_create_workflow(input_data):
    ...
    for key, values in action_data["actions"].items():
        if values["name"] == input_data["action_type"]:
            return values.get("create_workflow", False), action_data.get(
                "create_workflow_func", ""
            )
    return False, ""
...
class AlertAction:
    ...
    async def update_alert_data(cls, input_data):
        ...
        function_map = {
            "Justification": "justify_alert",
            "Rejected": "reject_alert",
            "Approved": "approve_alert",
            "Override": "override_alert",
            "interLockOk": "interlock_ok_alert",
            "excApprovalTimeExp": "exc_approval_time_exp_alert",
            "Message": "message_alert",
            "Raised": "raised_alert",
            "Cancelled": "cancel_alert",
            "Allocated": "allocate_alert",
            "SentToSap": "sent_to_sap_alert",
            "OrderPlaced": "order_placed_alert",
            "Created": "created_alert",
            "Tripped": "tripped_alert",
            "VTS": "vts_alert",
            "AcceptClose": "accept_close",
            "InvalidAlert": "invalid_alert",
            "FalseAlert": "false_alert",
            "ValidAlert": "valid_alert",
            "Blocked": "block_alert",
            "UnBlocked": "unblock_alert",
            "Interrupt": "interrupt_alert",
            "Request": "request_alert",
            "Maintenance": "maintenance_alert",
            "SendItBack": "send_it_back",
        }
        event_tag_map = {
            "Justification": "is_justify",
            "Approved": "is_approved",
            "AcceptClose": "accept",
            "InvalidAlert": "invalid",
        }
        ...
        if function_name:
            if getattr(alert_data, "alert_section", None) == "VTS":
                ...
                if assigned_roles:
                    ...
                    input_data["action_msg"] = (
                        f"{existing_msg}{prefix}initiated by {', '.join(assigned_roles)}"
                    )
            ...
            if input_data["action_type"] == "Approved" and input_data["bu"] == "RO":
                ...
                await hpcl_ceg_model.RoInterlockDisableCreate(
                    **interlock_disable_data
                ).create()
            # call the function
    ...
    async def update_alert_data_vts(cls, input_data):
        ...
        function_map = {
            ...
            "Maintenance": "maintenance_alert",
            "SendItBack": "send_it_back",
            "FalseViolation": "false_violation",
            "AcceptViolation": "accept_violation",
        }
        event_tag_map = {
            "Justification": "is_justify",
            "Approved": "is_approved",
    ...
    async def update_alert_history_vts(cls, input_data, alert_data):
        ...
        if isinstance(alert_data, dict):
            ...
            allocated_time = (
                ...
                if hasattr(alert_data, "updated_at")
        ...
        if alert_data["alert_section"] in ["VTS"]:
            input_data["action_msg"] = input_data.get("action_msg", "") + (
                (" " if input_data.get("action_msg") else "")
                + "Remarks initiated by "
                + ", ".join(alert_data.get("assigned_user_roles", []))
                if alert_data.get("assigned_user_roles")
                else ""
            )
        ...
        alert_history.append(
            {
                "allocated_time": (
                    allocated_time.isoformat()
                    if isinstance(allocated_time, datetime.datetime)
                    else allocated_time
                ),
                "processed_time": processed_time.isoformat(),
                "action_type": input_data["action_type"],
                "action_msg": input_data["action_msg"],
                "mail_sent_to": "",
                ...
                "maintenance_exception": event_tags.get(
                ...
                "is_exc_approval_time_exp": event_tags.get(
        ...
        alert_id = (
            alert_data.get("id")
            if isinstance(alert_data, dict)
            else getattr(alert_data, "id", None)
        )
        ...
        # Modify the alert with the updated alert_history
        await hpcl_ceg_model.ViolationHistoryVts(
            **{"id": alert_id, "alert_history": alert_history}
        ).modify()
    ...
    async def update_alert_history(cls, input_data, alert_data):
        ...
        if alert_data["alert_section"] in ["VTS"] and input_data["action_type"] in [
            "Justification"
        ]:
            await hpcl_ceg_model.Alerts(
                **{"id": alert_id, "device_msg": "request_raised_for_unblock"}
            ).modify()
    ...
    def get_exception_message(cls, exception):
        ...
        key_map = {
            "is_maintenance_exception": {
                "name": "isMaintenanceException",
                "type": "Boolean",
            },
            ...
            "is_exc_approval_time_exp": {
                "name": "isExcApprovalTimeExp",
                "type": "Boolean",
            },
        ...
        return {
            value["name"]: {"type": "Boolean", "value": exception.get(key, False)}
    ...
    async def publish_to_camunda(cls, input_data, alert_data, action_type, msg=None):
        ...
        if int(r.status_code / 100) != 2:
            print(
                f"Error while sending message to camunda: {r.status_code} - {r.text} - {alert_data.unique_id}"
            )
        else:
    ...
    async def maintenance_alert(cls, input_data, alert_data):
        ...
        if input_data.get("alert_section", "") in ["TAS"] and input_data.get("days", 0):
            ...
            await hpcl_ceg_model.Alerts(
                **{"id": alert_id, "maintenance_time": maintenance_time}
            ).modify()
    ...
    async def exc_approval_time_exp_alert(cls, input_data, alert_data):
        ...
        :return:
        """
        return await cls.publish_to_camunda(
            input_data, alert_data, "excApprovalTimeExp"
        )
    ...
    async def generate_show_cause_notice(cls, alert_data):
        try:
            ...
            vts_alert_history_query_resp = (
                await urdhva_base.BasePostgresModel.get_aggr_data(
                    vts_alert_history_query, limit=0
                )
            )
            vts_alert_history_query_resp = pd.DataFrame(
                vts_alert_history_query_resp.get("data", [])
            )
            ...
            vts_alert_history_query_resp["Latitude"] = vts_alert_history_query_resp[
                "Latitude"
            ].fillna("")
            vts_alert_history_query_resp["Longitude"] = vts_alert_history_query_resp[
                "Longitude"
            ].fillna("")
            ...
            trasporter_details = await urdhva_base.BasePostgresModel.get_aggr_data(
                query
            )
            ...
            if resp:
                template_data = {
                    "violation_stat_date": resp[0]["violation_stat_date"].strftime(
                        "%d.%m.%Y"
                    ),
                    "violation_end_date": resp[0]["violation_end_date"].strftime(
                        "%d.%m.%Y"
                    ),
                    "transporter_code": transporter_code,
                    "transporter_name": trasporter_details.get("transporter_name", ""),
                    "transporter_address": trasporter_details.get(
                        "transporter_address", ""
                    ),
                    "date": urdhva_base.utilities.get_present_time().strftime(
                        "%d/%m/%Y"
                    ),
                ...
                if alert_data.device_id in ["Instance - 2", "Instance - 3"]:
                    template_data[
                        "black_list_conditions"
                    ] = f"""Please note, that every case of blacklisting of a TT inducted with the Transporter shall be considered 
            ...
            body = (
                await cls.vts_read_template(interlock_value.value)
                if interlock_value
                else ""
            )
            ...
            status, minio_path = minio_connector.upload_to_minio(
                alert_data.bu, alert_data.alert_section, alert_data.unique_id, pdf_path
            )
            if status:
                query = (
                    f"""select * from notices_vts where alert_id='{alert_data.id}'"""
                )
                notices_data = await hpcl_ceg_model.NoticesVTS.get_aggr_data(
                    query, limit=0
                )
                if notices_data.get("data", []):
                    ...
                    await hpcl_ceg_model.NoticesVTS(
                        **{"id": notices_data["id"]}
                    ).modify()
                else:
                    notices_respose = {
    ...
    async def false_alert(cls, input_data, alert_data):
        ...
        if alert_data.alert_section == "VA":
            await hpcl_ceg_model.Alerts(
                **{"id": input_data["alert_id"], "mark_as_false": True}
            ).modify()

...
class EMLockAlertManager(alert_factory.AlertFactory):
    ...
    async def create_bu_alert(
        ...
        status, loc_dt = await alert_helper.get_location_details(
            bu=alert_data["location_type"], sap_id=alert_data["location_id"]
        )
        if status:
        ...
        emlock_interlock_details = emlock_mapping.emlock_vehicle_mapping[
        ...
        exception_msg = (
            f"Vehicle Number - {alert_data['truck_number']} \n Violation Type - {alert_data['exception_type']} \n"
        ...
        interlock_details.update(
            {
                ...
                "severity": emlock_interlock_details[alert_data["exception_type"]][
                    "severity"
                ],
                ...
                "alert_timestamp": datetime.datetime.strptime(
                    alert_data["created_datetime"], "%Y-%m-%d %H:%M:%S"
                ).isoformat(),
            }
        )
    ...
    async def create_bu_alert_old(
        ...
        for record in alert_data["data"]:
            ...
            status, location_details = await alert_helper.get_location_details(
                record["location_type"], record["location_id"]
            )
            if not status:
                logger.info(
                    f"Error in finding location {record['location_id']} "
                    f"for bu {record['location_type']} - {location_details}"
                )
                continue
            exception_msg = (
                f"Vehicle Number - {record['vehicle_number']}, Violation Type - {record['exception_type']}"
                f", Approved By - {record['approved_by']}, "
                f"Exception Date - {recv_time}"
            )
            query = (
                f"sap_id='{record['location_id']}' and vehicle_number='{record['vehicle_number']}' "
                f"and status='Open' and violation_type='{record['exception_type']}'"
            )

            data = await hpcl_ceg_model.EMLock.get_all(
            ...
            if len(data["data"]):
                ...
                if em_lock_record["violation_count"] > 10:
                    ...
                    em_lock_record["interlock_name"] = (
                        emlock_mapping.emlock_vehicle_mapping[record["exception_type"]][
                            "interlock_name"
                        ]
                    )
                    # Interlock name should be respective of voilation type
                    em_lock_record.update(
                        interlock_mapping.get_interlock_name(
                            em_lock_record["bu"], em_lock_record["interlock_name"], ""
                        )
                    )

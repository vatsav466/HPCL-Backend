        ...
        try:
            ...
            if not location_data:
                ...
                for attempt in range(retries):
                    ...
                    print(
                        f"Retrying to fetch location data for {bu} {sap_id}... Attempt {attempt + 1}/{retries}"
                    )
            ...
            base_data.update(
                {
                    ...
                    for key in ["device_id", "device_type", "device_name"]
                }
            )
            base_data.update(
                {
            ...
            async with httpx.AsyncClient(verify=False) as client:
                ...
                resp = await client.get(
                    f"{base_url}/api_cache/v1/get_unique_alert_id",
                    params={"bu": bu, "sap_id": sap_id, "sop_id": sop_id},
                )
            ...
            if alert_data.get("alert_section", "") == "VTS":
                async with httpx.AsyncClient(verify=False) as client:
                    ...
                    resp = await client.get(
                        f"{base_url}/api_cache/v1/get_unique_alert_id",
                        params={
                            "bu": alert_data.get("alert_section"),
                            "sap_id": sap_id,
                            "sop_id": sop_id,
                        },
                    )
            ...
            elif alert_data.get("alert_section", bu) == "RO" and interlock_name not in [
            ...
            if "alert_timestamp" in alert_data.keys():
                try:
                    alert_data["alert_timestamp"] = alert_data[
                        "alert_timestamp"
                    ].replace(tzinfo=None)
                except:
                    ...
            ...
            alert_resp = await hpcl_ceg_model.AlertsCreate(
                **{
                    **base_data,
                    "severity": (
                        alert_data.get("severity").capitalize()
                    ...
                    "unique_id": unique_id,
                    "alert_section": alert_data.get("alert_section", bu),
                    "external_id": alert_data.get(
                        "vendor_alert_id", str(alert_data["alert_id"])
                    ),
                    ...
                    "servicing_plant_name": str(
                        alert_data.get("servicing_plant_name", "")
                    ),
                    "progress_rate": 1,
                    "transporter_name": alert_data.get("transporter_name", ""),
                    "transporter_code": alert_data.get("transporter_code", ""),
                    "vehicle_blocked_start_date": alert_data.get(
                        "vehicle_blocked_start_date", None
                    ),
                    "vehicle_blocked_end_date": alert_data.get(
                        "vehicle_blocked_end_date", None
                    ),
                    "dry_out_start_time": alert_data.get("dry_out_start_time", None),
                    "intra_day_dry_out_start_time": alert_data.get(
                        "intra_day_dry_out_start_time", None
                    ),
                    "vehicle_unblocked_date": alert_data.get(
                        "vehicle_unblocked_date", None
                    ),
                    "dry_out_end_time": alert_data.get("dry_out_end_time", None),
                    "intra_day_dry_out_end_time": alert_data.get(
                        "intra_day_dry_out_end_time", None
                    ),
                    ...
                    "external_timestamp": alert_data.get(
                        "alert_timestamp",
                        urdhva_base.utilities.get_present_time()
                        .replace(tzinfo=None)
                        .isoformat(),
                    ...
                    "vts_alert_history_ids": alert_data.get(
            ...
            if alert_data.get("alert_section", "") in ["VA"]:
                if alert_data.get("alert_section", "") == "VA":
                    if alert_data.get("bu", "") in ["LPG"] and alert_data.get(
                        "interlock_name", ""
                    ) not in ["LPG Leakages", "LPG Leakages thru Filling Gun"]:
                        alert_level = "level - 1"
                    else:
            ...
            elif alert_data.get("alert_section", "") in ["LPG"] and alert_data.get(
                "interlock_name", ""
            ) in [
            ...
            elif alert_data.get("alert_section", "") == "RO" and alert_data.get(
                "interlock_name", ""
            ) not in ["Restroom Cleaning Evidence Missing"]:
                alert_level = await ro_analysis.get_ro_levels(
                    bu=base_data["bu"],
                    violation_type=alert_data.get("violation_type", ""),
                    sap_id=str(base_data["sap_id"]),
                )
            elif alert_data.get("alert_section", "") in ["VTS"] and alert_data.get(
                "interlock_name", ""
            ) not in ["No VTS No Load", "Itdg Admin Blocked"]:
                ...
                await redis_ins.hset(
                    "alert_mapping", alert_data["alert_id"], alert_resp["id"]
                )
            payload = {
                "businessKey": unique_id,
                "variables": {
                    "alert_id": {"value": alert_resp["id"], "type": "String"},
                    ...
                    "dealer_id": {
                        "value": alert_data.get("dealer_id", ""),
                        "type": "String",
                    },
                    "product_code": {
                    ...
                    "indent_no": {
                        "value": alert_data.get("indent_no", ""),
                        "type": "String",
                    },
                    "indent_raised_date": {
                    ...
                    "tas_device_name": {
                        "value": alert_data.get("tas_device_name", ""),
                        "type": "String",
                    },  # Added for TAS use
                },
            }
            ...
            if alert_data.get("interlock_name") == "Itdg Admin Blocked":
                payload["variables"].update(
                    {
                        "waitTime": {
                            "value": alert_data.get("waitTime", ""),
                            "type": "String",
                        },
                        "bu": {"value": alert_data.get("bu", ""), "type": "String"},
                        "auto_unblock": {
                            "value": alert_data.get("auto_unblock", ""),
                            "type": "String",
                        },
                        "vehicle_number": {
                            "value": alert_data.get("vehicle_number", ""),
                            "type": "String",
                        },
                        "checkTicketClose": {
                            "value": alert_data.get("checkTicketClose", False),
                            "type": "Boolean",
                        },
                    }
                )
            ...
            if alert_data.get("interlock_name") == "Truck Contract Validity Status":
                payload["variables"].update(
                    {
                        "contract_valid_upto": {
                            "value": alert_data.get("contract_valid_upto", ""),
                            "type": "String",
                        },
                        "bu": {"value": alert_data.get("bu", ""), "type": "String"},
                        "vehicle_number": {
                        ...
                        "device_installation_id": {
                            "value": alert_data.get("device_installation_id", ""),
                            "type": "String",
                        },
                        "before30days": {
                            "value": alert_data.get("before30days", False),
                            "type": "Boolean",
                        },
                        "waitTime": {
                            "value": alert_data.get("waitTime", ""),
                        ...
                        "days_remaining": {
                            "value": alert_data.get("days_remaining", ""),
                            "type": "String",
                        },
                    }
                )
            ...
            if interlock_name:
                # Create Interlock
                interlock = await hpcl_ceg_model.InterlockCreate(
                    **{
                        **base_data,
                        "interlock_name": interlock_name,
                ...
                await hpcl_ceg_model.Alerts(**alert_data_dict).modify()

                payload["variables"]["interlock_id"] = {
                    "value": interlock["id"],
                    "type": "String",
                }
                ...
                if alert_data_dict.get(
                    "alert_section"
                ) == "VTS" and alert_data_dict.get("interlock_name", "") not in [
                    "No VTS No Load"
                ]:
                    await vts_analysis.update_alert_id_to_vts_history(
                        alert_id=str(alert_resp["id"]),
                        vts_alert_id=alert_data_dict.get("vts_alert_history_ids", []),
                    )
                    blocked_tt_data = dict()
                    blocked_tt_data["TT_No"] = alert_data_dict.get("vehicle_number", "")
                    blocked_tt_data["BlockStartDate"] = alert_data_dict[
                        "vehicle_blocked_start_date"
                    ].strftime("%d/%b/%Y 00:00")
                    blocked_tt_data["BlockEndDate"] = alert_data_dict[
                ...
                if alert_data_dict.get("alert_section") in [
                    "VA"
                ] and alert_data_dict.get("bu") in ["RO"]:
                    return True, "alert created"
                if alert_data_dict.get("alert_section") in [
                    "RO"
                ] and interlock_name.get("interlock_name") not in [
                    "Dry Out Each Indent Wise MainFlow",
                    "Restroom Cleaning Evidence Missing",
                ]:
                ...
                if (
                    alert_data_dict.get("alert_section") in ["TAS"]
                    and interlock_name.get("interlock_name") in Analog_data
                ):
                    print("Workflow Skipped for TAS No Workflow ILs")
                    logger.info(
                        f"Workflow Skipped for TAS No Workflow ILs: {interlock_name.get('interlock_name')}"
                    )
                    return True, "alert created"
                ...
                await redis_ins.hset(
                    "alert_camunda_url", str(alert_resp["id"]), camunda_url
                )
            else:
                print(
                    f"Unable to find Camunda workflow for interlock: {interlock_name}, BU: {bu}"
                )
                logger.info(
                    f"Unable to find Camunda workflow for interlock: {interlock_name}, BU: {bu}"
                )
            ...
            logger.error(
                f"Error in alert creation {e}, traceback {traceback.format_exc()}"
            )
    ...
    async def close_alert(cls, alert_data):
        ...
        try:
            ...
            if not bu:
                return False, {
                    "status": "Error",
                    "message": "Business unit not provided",
                }
            ...
            if "interlock_id" in alert_data:
                ...
                if alert_data.get("interlock_id", ""):
                    try:
                        il_data = await hpcl_ceg_model.Interlock.get(
                            alert_data["interlock_id"]
                        )
                        ...
                        il_data["interlock_status"] = (
                            hpcl_ceg_enum.AlertStatus.Close.value
                        )
                        data_obj = hpcl_ceg_model.Interlock(**il_data)
                        await data_obj.modify()
                        logger.info(
                            f"Closed interlock with ID: {alert_data['interlock_id']}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to close interlock with ID {alert_data['interlock_id']}: {e}"
                        )
                else:
                ...
                try:
                    ...
                    try:
                        if await redis_ins.hexists(
                            "alert_mapping", al_data.get("external_id", "")
                        ):
                            await redis_ins.hdel(
                                "alert_mapping", al_data["external_id"]
                            )
                    ...
                    print(traceback.format_exc())
                    logger.error(
                        f"Failed to close alert with ID {alert_data['alert_id']}: {e}"
                    )
                    return False, {
                        "status": "Error",
                        "message": f"Error closing alert: {str(e)}",
                    }
            ...
            # Handle case with interlock_name (lookup by query)
                ...
                try:
                    ...
                    if (
                        il_resp
                        and hasattr(il_resp, "__dict__")
                        and il_resp.__dict__.get("body")
                    ):
                ...
                try:
                    ...
                    if (
                        alert_resp
                        and hasattr(alert_resp, "__dict__")
                        and alert_resp.__dict__.get("body")
                    ):
                ...
                if not alert_data_list and not il_data:
                    ...
                    logger.warning(
                        "Both Interlock and Alert records not found. Skipping closure."
                    )
                    return False, {
                        "status": "Error",
                        "message": "No matching interlock or alert records found",
                    }
                ...
                if il_data:
                    try:
                        ...
                        interlock_closed = True
                        logger.info(
                            f"Closed interlock with name: {alert_data['interlock_name']}"
                        )
                    except Exception as e:
                        ...
                        for alert in alert_data_list:
                            if alert_data.get("alert_history"):
                                if not alert.get("alert_history"):
                                    ...
                                    alert_data["alert_history"]["allocated_time"] = (
                                        alert["alert_history"][-1].get("processed_time")
                                    )
                                    alert["alert_history"].append(
                                        alert_data["alert_history"]
                                    )

                            alert["severity"] = alert_data.get(
                                "severity", "Medium"
                            ).capitalize()
                            alert["alert_status"] = (
                                hpcl_ceg_enum.AlertStatus.Close.value
                            ...
                            alert["alert_state"] = (
                                hpcl_ceg_enum.AlertState.Resolved.value
                            )
                            alert["interlock_name"] = alert_data.get(
                                "interlock_name", ""
                            )
                            ...
                            try:
                                await redis_ins.hdel(
                                    "alert_camunda_url", str(alert["id"])
                                )
                            except Exception:
                                print("Failed to del in redis")
                            alert_closed = True
                            logger.info(
                                f"Closed alert with external ID: {alert_data['alert_id']}"
                            )
                    except Exception as e:
                        print(traceback.format_exc())
                ...
                if interlock_closed or alert_closed:
                    ...
                    return False, {
                        "status": "Warning",
                        "message": "Failed to close any alerts or interlocks",
                    }
            ...
            return False, {
                "status": "Error",
                "message": f"Error in alert closure: {str(e)}",
            }

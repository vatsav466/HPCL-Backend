    ...
    async def create_bu_alert(
        ...
        try:
            ...
            if not location_id:
                ...
                return {
                    "status": False,
                    "message": "Invalid data format: location_id is missing",
                    "alert_data": alert_data,
                }

            # Retrieve necessary fields from the alert_data
            status, loc_dt = await alert_helper.get_location_details(
                bu=alert_data["location_type"], sap_id=location_id
            )
            if not status:
                logger.info(
                    f"Error in finding location {location_id} "
            ...
            for record in alert_records:
                try:
                    ...
                    va_alert_data = va_alert_mapping.VA_Alert_Mapping[
                        record["location_type"]
                    ].get(record["alert_section"], {})
                    ...
                    interlock_details = utilities.interlock_mapping.get_interlock_name(
                        record["location_type"], va_alert_data["name"]
                    )

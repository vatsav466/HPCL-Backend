        ...
        for interlock_name in interlock_names:
            ...
            for record in resp1.get("data", []):
                ...
                if alert_count > 2:
                    ...
                    mapped_interlock_name = INTERLOCK_NAME_MAPPING.get(
                    ...
                    if is_duplicate:
                        print(
                            f"Duplicate alert found for {interlock_name} on device {device_name}. Skipping alert creation."
                        )
                        continue

                    # Check alert history
                    alert_history = await duplicates_check.alert_history_check(
                        alert_data, month_check=True
                    )
                    if alert_history:
                        print(
                            f"Alert history exists for {interlock_name} on device {device_name}. Skipping alert creation."
                        )
                        continue
                    # Create the alert
                    ...
                    if success:
                        print(
                            f"Weekly summary alert created successfully for {interlock_name} on device {device_name}."
                        )
                    else:
                        print(
                            f"Failed to create weekly summary alert for {interlock_name} on device {device_name}: {msg}"
                        )
    except Exception:
    ...
    try:
        ...
        for record in resp1.get("data", []):
            ...
            if alert_count > fan_printed:
                ...
                if is_duplicate:
                    print(
                        f"Duplicate alert found for Bay Reassignment on sap_id {sap_id}. Skipping alert creation."
                    )
                    continue
                ...
                if success:
                    print(
                        f"Bay reassignment alert created successfully for sap_id {sap_id}."
                    )
                else:
                    print(
                        f"Failed to create Bay reassignment alert for sap_id {sap_id}: {msg}"
                    )
    except Exception:

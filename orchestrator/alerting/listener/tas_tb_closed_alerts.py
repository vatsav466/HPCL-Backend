        ...
        if tb_status == "CLEARED_UNACK":
            if not workflow_instance_id:
                ...
                try:
                    ...
                    print(
                        f"Closed alert in DB with external_id: {external_id} | DB id: {alert_update['id']} | Result: {result}"
                    )
                except Exception as e:
                    print(
                        f"Error closing alert in DB with external_id: {external_id} | Exception: {e}"
                    )
                ...
                try:
                    ...
                    print(
                        f"Closed alert with external_id: {external_id} | Success: {success}"
                    )
                except Exception as e:
                    print(
                        f"Error closing alert with external_id: {external_id} | Exception: {e}"
                    )
                    print(traceback.format_exc())

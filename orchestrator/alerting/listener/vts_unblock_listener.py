    ...
    async def process_task(self, task):
        ...
        if task.get("task_type") == "approve":
            for alert_id in task["alert_ids"]:
                ...
                if alert_data["bu"] in ["TAS"]:
                    try:
                        payload = [
                            {
                                ...
                                "blockingFrom": alert_data[
                                    "vehicle_blocked_start_date"
                                ].strftime("%Y%m%d"),
                                "blockingTo": alert_data[
                        ...
                        resp = requests.post(
                            urdhva_base.settings.post_to_ims_url,
                            json=payload,
                            headers=headers,
                            timeout=30,
                        )
                        ...
                        vehicle_unblocked_date = datetime.datetime.now(
                        ...
                        if closed_at:
                            ...
                            alert_data["action_msg"] = (
                                f"Approved unblock request by {employee_id}"
                            )
                        ...
                        print(
                            "workflow_deletion Status code:",
                            delete_response.status_code,
                        )
                        print("workflow_deletion Response body:", delete_response.text)
                    except requests.exceptions.HTTPError as e:
                        print(
                            "HTTP error:", e, "Body:", getattr(e.response, "text", None)
                        )
                    except requests.exceptions.RequestException as e:
            ...
            for alert_id in task["alert_ids"]:
                ...
                if alert_data["bu"] in ["TAS"]:
                    (
                        alert_data["assigned_user_roles"].remove("Creator SOD")
                        if "Creator SOD" in alert_data["assigned_user_roles"]
                        else alert_data["assigned_user_roles"]
                    )
                    alert_data["assigned_user_roles"].append("Approver SOD")
                elif alert_data["bu"] in ["LPG"]:
                    (
                        alert_data["assigned_user_roles"].remove("Creator LPG")
                        if "Creator LPG" in alert_data["assigned_user_roles"]
                        else alert_data["assigned_user_roles"]
                    )

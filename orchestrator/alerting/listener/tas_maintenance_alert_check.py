...
async def maintenance_alert_check(alert_data):
    ...
    try:
        ...
        if re.match(
            r"^[A-Z]+[- ][A-Z0-9]+_", original_device_name
        ) and not original_device_name.startswith("TK-"):
            tas_device_name = original_device_name.split("_", 1)[1]
        else:
            tas_device_name = original_device_name

        if alert_data["interlock_name"] in [
            "ESD ROSOV_Close Status_Fail",
            "ESD MOV_Close Status_Fail",
        ]:
            print(
                f"Creating alert for exception interlock_name: {alert_data['interlock_name']}"
            )
            return False
        ...
        if not interlock_name.endswith("Maintenance"):
            ...
            logger.info(
                f"Checking for Tank_Under_Maintenance alert for device: {tas_device_name_for_query}"
            )
            ...
            tank_maintenance_params = urdhva_base.queryparams.QueryParams(
                q=tank_maintenance_query
            )
            tank_maintenance_resp = await hpcl_ceg_model.Alerts.get_all(
                tank_maintenance_params, resp_type="plain"
            )

            if tank_maintenance_resp["data"]:
                logger.info(
                    f"Device '{tas_device_name_for_query}' has an active Tank_Under_Maintenance alert - skipping new alert for equipment '{current_equipment_name}'"
                )
                return True

            logger.info(
                f"No Tank_Under_Maintenance alert found for device: {tas_device_name_for_query} - proceeding to equipment level check"
            )

            if interlock_name in [
                "MOV_Close Status",
                "ROSOV_Close Status",
                ...
                logger.info(
                    f"Interlock '{interlock_name}' is MOV/ROSOV type - checking if triggered due to equipment '{effect_due_to}' under maintenance for device: {tas_device_name_for_query}"
                )
                ...
                logger.info(
                    f"MOV/ROSOV effect due to maintenance check query: {effect_close_query}"
                )

                effect_params = urdhva_base.queryparams.QueryParams(
                    q=effect_close_query
                )
                effect_resp = await hpcl_ceg_model.Alerts.get_all(
                    effect_params, resp_type="plain"
                )

                if effect_resp["data"]:
                    logger.info(
                        f"Interlock '{interlock_name}' on device '{tas_device_name_for_query}' is triggered due to equipment '{effect_due_to}' being under maintenance - skipping new alert"
                    )
                    return True

                logger.info(
                    f"No maintenance alert found for effect equipment '{effect_due_to}' on device '{tas_device_name_for_query}' - proceeding to equipment level check"
                )

            # Step 2 - Check equipment specific maintenance
            if (
                current_equipment_name is not None
                and current_equipment_name in related_equipment_names
            ):
                logger.info(
                    f"Checking equipment specific maintenance alert for equipment: {current_equipment_name}, device: {tas_device_name_for_query}"
                )
                ...
                logger.info(
                    f"Equipment maintenance check query: {equipment_maintenance_query}"
                )

                equipment_maintenance_params = urdhva_base.queryparams.QueryParams(
                    q=equipment_maintenance_query
                )
                equipment_maintenance_resp = await hpcl_ceg_model.Alerts.get_all(
                    equipment_maintenance_params, resp_type="plain"
                )

                if equipment_maintenance_resp["data"]:
                    logger.info(
                        f"Equipment '{current_equipment_name}' has an active maintenance alert for device '{tas_device_name_for_query}' - skipping new alert"
                    )
                    return True

                logger.info(
                    f"No active maintenance alert found for equipment '{current_equipment_name}' on device '{tas_device_name_for_query}' - alert will be created"
                )

            else:
                logger.info(
                    f"Equipment '{current_equipment_name}' is not in related equipment list - no equipment level maintenance check needed"
                )

        logger.info(
            f"No blocking conditions found - alert will be created for equipment '{current_equipment_name}' on device '{tas_device_name_for_query}'"
        )
        return False
...
async def close_tas_workflow(alert_data, message_type="Message"):
    ...
    logger.info(
        f"Closing camunda workflow for alert_id: {alert_data['id']} {alert_data}"
    )
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if r.status_code // 100 == 2:
                    ...
                    logger.error(
                        f"Attempt {attempt}: Error sending message to camunda: "
                        f"{r.status_code} - {r.text} - {alert_data['unique_id']}"
                    )
            except Exception as e:
                logger.error(
                    f"Attempt {attempt}: Exception in closing camunda flow {e} "
                    f"for alert_id {alert_data['id']}, business_key {alert_data['unique_id']}"
                )
            ...
            if attempt < max_retries:
                ...
                logger.error(
                    f"Failed to send message to camunda after {max_retries} attempts"
                )
...
async def create_under_maintenance_alert(alert_data):
    if alert_data["interlock_name"] == "Tank_Under Maintenance":
        ...
        logger.info("*" * 100)
        logger.info(
            f"Processing Tank_Under Maintenance alert for: {alert_data['tas_device_name']}"
        )
        logger.info("*" * 100)
        ...
        logger.info(
            f"maintenance_query --> {json.dumps(maintenance_params, default=str)}"
        )
    ...
    if alert_data["interlock_name"] != "Tank_Under Maintenance":
        logger.info("*" * 100)
        logger.info(
            f"Checking for existing 'Tank_Under Maintenance' alert for device_id: {alert_data.get('device_id', '')}"
        )
        logger.info("*" * 100)
        ...
        if maintenance_resp and maintenance_resp.get("data"):
            logger.info("*" * 100)
            logger.info(
                f"Not creating alert; existing Tank Under Maintenance alert found: {maintenance_resp['data']}"
            )
            logger.info("*" * 100)
        else:
            await create_alert(alert_data=alert_data)

...
class DryoutCollector:
    ...
    async def get_dry_out_data(cls):
        try:
            ...
            records = cls.assign_values_to_dataframe(
            ...
            for _dry in records:
                ...
                alert_data["product_code"] = _mapping.get(
                    _dry["item_name"], _dry["product_no"]
                )
            ...
            dry_out_history = await hpcl_ceg_model.DryOutHistory.get_aggr_data(
                query, limit=50000
            )
            dry_out_hist_data = {
                f"{rec['sap_id']}_{rec['product_no']}": rec
                for rec in dry_out_history["data"]
            }
            dry_out_alert = {
                f"{rec['rosapcode']}_{rec['product_no']}": rec for rec in records
            }
            closed_alerts = list(
            ...
            for index in range(0, len(closed_ids), 1000):
                ...
                for cid in batch_ids:
                    ...
                    key = next(
                        k for k, v in dry_out_hist_data.items() if v["id"] == cid
                    )
            ...
            for i in dry_ou_days:
                total_dryouts = (
                    dry_out_data.filter(pl.col("status") == i)
                ...
                ms_dryouts = (
                    dry_out_data.filter(
                        (pl.col("status") == i) & (pl.col("item_name").is_in(["MS"]))
                    )
                    .select(pl.col("rosapcode").n_unique())
                    .item()
                )
                ...
                hsd_dryouts = (
                    dry_out_data.filter(
                        (pl.col("status") == i) & (pl.col("item_name").is_in(["HSD"]))
                    )
                    .select(pl.col("rosapcode").n_unique())
                    .item()
                )
                ...
                turbo_dryouts = (
                    dry_out_data.filter(
                        (pl.col("status") == i) & (pl.col("item_name").is_in(["TURBO"]))
                    )
                    .select(pl.col("rosapcode").n_unique())
                    .item()
                )
                ...
                e20_dryouts = (
                    dry_out_data.filter(
                        (pl.col("status") == i) & (pl.col("item_name").is_in(["E20"]))
                    )
                    .select(pl.col("rosapcode").n_unique())
                    .item()
                )
                ...
                dry_out_df = dry_out_data.filter(pl.col("status") == i).select(
                    [
                ...
                status, minio_path = minio_connector.upload_to_minio(
                    "RO", "DryOut", str(sync_date.split("-")[0]), path
                )
                ...
                dryout_records = {
                    ...
                    "power99_dryouts": (
                        power99_dryouts if power99_dryouts else power99_dryouts
                    ),
                    "power100_dryouts": (
                        power100_dryouts if power100_dryouts else power100_dryouts
                    ),

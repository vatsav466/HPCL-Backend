        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{API_BASE_URL}/api/tas/get_agent_service_status", json=payload
                ) as resp:
                    if resp.status == 200:
        ...
        while True:
            try:
                ...
                for sap_id, name in location_map.items():
                    ...
                    last_ping_str = await redis_client.hget(
                        "tas_agent_up_status", sap_id
                    )
                    if last_ping_str:
                        ...
                        if not first_cycle and now_ist - last_ping_time > timedelta(
                            minutes=CHECK_PERIOD_MINUTES
                        ):

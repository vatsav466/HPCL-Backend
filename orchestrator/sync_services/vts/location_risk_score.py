...
def load_alerts_data():
    ...
    for key, df in df_dict.items():
        ...
        if "EVENT_DATETIME" in df.columns:
            df["EVENT_DATETIME"] = pd.to_datetime(
                df["EVENT_DATETIME"], errors="coerce", format="mixed"
            )
    ...
    def generate_invoice_summary(df, name):
        ...
        return {
            ...
            "Total_Unique_Invoice": (
                df["INVOICE_NO"].nunique() if "INVOICE_NO" in df.columns else 0
            ),
            "Loaded_Unique_Invoice": (
                df.loc[
                    df["TRIP_STATUS"].str.lower() == "loaded", "INVOICE_NO"
                ].nunique()
                if "TRIP_STATUS" in df.columns
                else 0
            ),
            "Unloaded_Unique_Invoice": (
                df.loc[
                    df["TRIP_STATUS"].str.lower() == "un loaded", "INVOICE_NO"
                ].nunique()
                if "TRIP_STATUS" in df.columns
                else 0
            ),
...
shortage_df = shortage_df.rename(
    columns={
        "sap_id": "LOCATION",
        "inv_ref": "INVOICE_NO",
...
shortage_df["invoice_date"] = pd.to_datetime(
    shortage_df["invoice_date"], format="%Y%m%d"
)
...
summary = {
    ...
    "Total_Unique_Invoice": (
        shortage_df["INVOICE_NO"].nunique()
        if "INVOICE_NO" in shortage_df.columns
        else 0
    ),
}
...
# DATE RANGE of DATA
# =====================================================
alerts["SCHEDULED_DATETIME"] = pd.to_datetime(
    alerts["SCHEDULED_DATETIME"], errors="coerce"
)
...
shortage_df["invoice_date"] = shortage_df["invoice_date"] + pd.Timedelta(
    hours=23, minutes=59
)
...
def extract_trip_summary_from_df(completed_Trips_df):
    ...
    trips_summary = (
        completed_Trips_df.groupby("LOCATION")["INVOICE_NO"]
        .nunique()
        .reset_index()
...
def compute_vss_score(alert_ratio):
    ...
    # ---------------------------------------------------------
    category_mapping = [
        {"Category": "Route", "Files": ["RD", "ST"], "Weight": 0.35},  # 0.25
        {"Category": "Device Integrity", "Files": ["PD", "DR"], "Weight": 0.45},  # 0.30
        {
            "Category": "Safety",
            "Files": ["SV", "HA", "HB", "HT", "CD", "PANIC"],
            "Weight": 0.20,
        },  # 0.10
        {
            "Category": "Governance",
            "Files": ["Manual_Unblock", "Trip_Not_Closed"],
            "Weight": 0.00,
        },  # 0.15
        {"Category": "EM Lock Events", "Files": ["EMLock"], "Weight": 0.00},  # 0.20
    ]
...
def compute_total_location_sensitivity_fast(
    ...
    event_data["alert_sensitivity"] = (
        event_data["alert_sensitivity_value"].rank(pct=True) * 100
    )
    ...
    alerts_Sensitivity["alert_sensitivity"] = alerts_Sensitivity[
        "alert_sensitivity"
    ].round(2)
...
alert_type_weights = {"DR": 0.4, "RD": 0.3, "PD": 0.2, "ST": 0.1}
...
alert_data_sen = compute_total_location_sensitivity_fast(
    alert_data, cluster_weights_per_alert, alert_type_weights, radius_m=INFLUENCE_RADIUS
)
...
def compute_shortage_score(shortage_df, trips_summary, allowable_threshold):
    ...
    shortage_df["QTY_SHORTAGE(L)"] = pd.to_numeric(
        shortage_df["QTY_SHORTAGE(L)"], errors="coerce"
    ).fillna(0)

    # Shortage flag
    shortage_df["SHORTAGE_FLAG"] = (
        shortage_df["QTY_SHORTAGE(L)"] > shortage_df["allowable_variation"]
    )
    ...
    loc_shortage[["shortage_trips", "Avg_shortage_L"]] = loc_shortage[
        ["shortage_trips", "Avg_shortage_L"]
    ].fillna(0)

    # ---------------------------------------------------------
    # FREQUENCY SCORE
    # ---------------------------------------------------------
    loc_shortage["shortage_rate"] = (
        loc_shortage["shortage_trips"] / loc_shortage["TOTAL_TRIPS"]
    )

    # Smooth sigmoid (your original)
    loc_shortage["freq_score"] = 100 / (
        1 + np.exp(-10 * (loc_shortage["shortage_rate"] - 0.10))
    )
    ...
    iqr = loc_shortage["Avg_shortage_L"].quantile(0.75) - loc_shortage[
        "Avg_shortage_L"
    ].quantile(0.25)
    iqr = max(iqr, 1)
    ...
    loc_shortage["Shortage_score"] = (
        0.4 * loc_shortage["freq_score"] + 0.6 * loc_shortage["sev_score"]
    ).round(2)
...
shortage_score_1day = compute_shortage_score(
    shortage_1day, trips_summary_1day, allowable_threshold
)
shortage_score = compute_shortage_score(shortage_df, trips_summary, allowable_threshold)


def calculate_location_risk_score(
    VSS_score, alerts_Sensitivity_df, shortage_score, master_df, app_db_engine
):

    print("🔹 Merging all component scores...")

    VSS_score["LOCATION"] = VSS_score["LOCATION"].astype(str).str.strip()
    alerts_Sensitivity_df["LOCATION"] = (
        alerts_Sensitivity_df["LOCATION"].astype(str).str.strip()
    )
    ...
    merged_df = merged_df.drop(columns=["TOTAL_TRIPS_y"]).rename(
        columns={"TOTAL_TRIPS_x": "TOTAL_TRIPS"}
    )
    ...
    missing_locations = (
        merged_df[merged_df["LOCATION_NAME"].isna()]["LOCATION"].unique().tolist()
    )
    if missing_locations:
        print(
            f"Fetching {len(missing_locations)} missing location names from location_master..."
        )
        location_master_df = pd.read_sql_query(
            "SELECT sap_id, name FROM location_master", app_db_engine
        )
        location_master_df["sap_id"] = (
            location_master_df["sap_id"].astype(str).str.strip()
        )
        location_master_map = dict(
            zip(location_master_df["sap_id"], location_master_df["name"])
        )
        ...
        merged_df["LOCATION_NAME"] = merged_df.apply(
            lambda row: (
                ...
                else location_master_map.get(row["LOCATION"])
            ),
            axis=1,
        )
...
Final_LRS_1day = calculate_location_risk_score(
    VSS_score_1day, alert_data_sen_1day, shortage_score_1day, master_df, app_db_engine
)
Final_LRS = calculate_location_risk_score(
    VSS_score, alert_data_sen, shortage_score, master_df, app_db_engine
)

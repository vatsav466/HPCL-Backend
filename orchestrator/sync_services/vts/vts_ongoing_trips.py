...
def insertToDB(data, table_name, indexing_col=()):
    ...
    if missing_in_data:
        print(
            f"Columns in DB but not in incoming data (will be NULL): {missing_in_data}"
        )
...
def get_ongoing_trip_data(
    table_name, params, max_date=None, master_data=pl.DataFrame()
):
    _datecol = "EVENT_DATE"
    ...
    if not max_date:
        max_date = (
            f""" select max({_datecol}) as max_date from vts_{table_name.lower()} """
        )
        cursor = None
        ...
        if not data.is_empty():
            ...
            max_date = (datetime.datetime.now() - relativedelta(days=10)).strftime(
                "%Y-%m-%d"
            )
    ...
    data = data.rename(
        {key: value for key, value in rename_mapper.items() if key in data.columns}
    )
    ...
    for col in ["route_id", "sec_route_id"]:
        if col in data.columns:
            data = data.with_columns(
                pl.col(col).fill_null(0).cast(pl.Float64).cast(pl.Int64).alias(col)
            )

    print("Before Location Master Mappings :", len(data))
    if not master_data.is_empty():
        data = data.join(
            master_data.select(["bu", "sap_id", "name", "zone", "region"]),
            on="sap_id",
            how="left",
        )
...
def main():
    ...
    for table_name in [
        ...
        get_ongoing_trip_data(
            table_name=table_name, params=params, max_date=None, master_data=master_data
        )

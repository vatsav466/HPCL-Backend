...
def get_and_insert_data(cursor, query, params=None):
    ...
    data.loc[(data['ORGSBUCD'] == '4000')&(~(df['MATERIAL_GROUP_CD'].isin(['031','038'])))&(df['DISTRIBUTION_CHANNEL_CD'] == '014'),'ProductName'] = 'Lubes-Exports'
    data.loc[(data['ORGSBUCD'] == '4000')&(df['MATERIAL_GROUP_CD'] =='038')&(df['DISTRIBUTION_CHANNEL_CD'].isin(['011','012'])),'ProductName'] = 'HP DEF-Retail'
    data = pl.from_pandas(data)

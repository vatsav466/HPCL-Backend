...
def insertToDB(data, table_name, indexing_col=()):
    ...
    apr_data = pl.read_csv('/tmp/april_updated.csv',infer_schema_length=0)
    ...
    print(data.columns)
    #print(jun_data.columns)
    print(len(data))
    #print(len(jun_data))
    print(data.schema)
    #jun_data=jun_data.with_columns(pl.col('NETWEIGHT_KG').cast(float).round(2).alias('NETWEIGHT_KG')) 
    #jun_data=jun_data.with_columns(pl.col('NETWEIGHT_TMT').cast(float).round(2).alias('NETWEIGHT_TMT')) 
    #print(jun_data.schema)
    #jun_data = jun_data.select(data.columns)
    #data = jun_data
    print(len(data))
    #data = apr_data
    #data.write_csv('/tmp/data_apr.csv')
    print(len(data)) 
    data = data.unique(['engine_id'])
    print(len(data))
    #data = pl.concat([data,apr_data])
    print(len(data))
    ...
    '''
    cur.execute(f"""
select * FROM   "MOM_LEVEL_FINAL_DATA" where "month_name"in  ('Apr','May','Jun') and "fiscal_year" = '2023-2024'
    """
    ...
    data.to_csv('/tmp/hisdata.csv',index = False)
...
def get_and_insert_data(cursor, query, params=None):
    ...
    '''
    cursor.execute(f"""
    select * FROM PS.EDW_PRIM_SLS_FCT where SLS_TRNS_DT BETWEEN '2023-06-01' AND '2023-06-30';
    ...
    data.to_csv('/tmp/his_data.csv',index = False)
    exit()
    '''
    print(len(data))
...
if __name__ == "__main__":
    params = {
        ...
        "port": 3306,
        "table_name": "MOM_LEVEL_FINAL_DATA",
...
NETWEIGHT_UOM,
NETWEIGHT AS NETWEIGHT_KG,
CASE WHEN ORGSBUCD = '7000' THEN NETWEIGHT/1000000
...
E1.WEIGHT_UNIT
 ...
 ORDER BY 1,2,3,4
    ...
    get_and_insert_data(cursor, query_check, params=params)

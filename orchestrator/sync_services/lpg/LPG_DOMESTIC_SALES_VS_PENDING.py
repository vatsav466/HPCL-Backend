...
def insertToDB(data, table_name, indexing_col=()):
    ...
    for col, dty in col_dtype.items():
        ...
        if pg_type is None:
            dty_str = str(dty)
            if (
                dty_str.startswith("Datetime")
                or dty_str.startswith("Date")
                or dty in (pl.Datetime, pl.Date)
            ):
                pg_type = "timestamp"
            else:
                pg_type = "text"
...
def _merge_data(
    left_df, right_df, left_on, right_on, how, suffixes="_right", indicator=False
):
    """
    ...
    left_df = left_df.with_columns(
        pl.col(x).alias(y) for x, y in zip(left_on, new_names)
    )
    right_df = right_df.with_columns(
        pl.col(x).alias(y) for x, y in zip(right_on, new_names)
    )

    left_df = left_df.with_columns(left_merge=pl.lit("Left"))
    right_df = right_df.with_columns(right_merge=pl.lit("Right"))

    final_df = left_df.join(right_df, on=new_names, how=how, suffix=suffixes).drop(
        new_names
    )
    if indicator:
        final_df = final_df.with_columns(
            _merge=pl.when(
                (pl.col("left_merge").is_not_null()) & (pl.col("right_merge").is_null())
            )
            .then(pl.lit("left_only"))
            .when(
                (pl.col("left_merge").is_null()) & (pl.col("right_merge").is_not_null())
            )
...
def calculate_ageing(df):
    ...
    for col in df.columns:
        if col.startswith("Pending"):
            df = df.with_columns(
                pl.col("Total_Pending")
                + pl.col(col).fill_null(0).cast(pl.Int64).alias("Total_Pending")
            )
...
def get_pending_vs_delivered_data():
    ...
    data = data.with_columns(
        pl.col("System_Idx").map_elements(lambda x: str(uuid.uuid4().hex))
    )
    ...
    tblDistributorMaster = tblDistributorMaster.with_columns(
        pl.col("JDEDistributorCode")
        .fill_null(0)
        .cast(pl.Int64)
        .alias("JDEDistributorCode")
    )
    data = data.with_columns(
        pl.col("JDEDistributorCode")
        .fill_null(0)
    ...
    data = _merge_data(
        left_df=data,
        right_df=tblDistributorMaster.select(
            [
                "JDEDistributorCode",
                "DistributorName",
                "SACode",
    ...
    data = data.with_columns(
        pl.col("ZOName").str.strip_chars().replace(zoneMap).alias("ZOName")
    )
    data = data.with_columns(
        pl.lit(
            (datetime.datetime.now(ZoneInfo("Asia/Kolkata")) - relativedelta(days=1))
        ).alias("Execution_Date")
    )
    data = data.with_columns(
        pl.col("Execution_Date").dt.strftime("%B").alias("Execution_Month")
    )
    data = data.with_columns(
        pl.col("Execution_Date").dt.strftime("%Y").alias("Execution_Year")
    )
    data = data.with_columns(
        pl.col("Execution_Date").dt.strftime("%b-%Y").alias("Month-Year")
    )
    ...
    data = data.with_columns(
        pl.when(pl.col("CylType").fill_null("") == "C142")
        .then(pl.col("TotalSalesYesterday") * 14.2)
        .when(pl.col("CylType").fill_null("") == "C5")
        .then(pl.col("TotalSalesYesterday") * 5)
        .alias("sales_volume")
    )
    ...
    data = data.with_columns(
        ...
        .when(pl.col("CylType").fill_null("") == "C5")
        .then(pl.col("BookingReceivedYesterday") * 5)
        .alias("bookings_volume")
    )
    ...
    data = data.with_columns(
        ...
        .when(pl.col("CylType").fill_null("") == "C5")
        .then(pl.col("Total_Pending") * 5)
        .alias("pendings_volume")
    )
    ...
    data = data.with_columns(
        pl.col("Execution_Month").replace(month_map).alias("Month_Number")
    )
    data = data.with_columns(
        [
    ...
    try:
        data = data.with_columns(
            pl.col("ROName").str.replace("REGIONAL OFFICE", "RO").alias("ROName")
        )
    except Exception:
    ...
    insertToDB(
        ...
        indexing_col=["ZOName", "Financial_Year", "Month", "Execution_Date"],
    )
    ...
    monthly_data = monthly_data.with_columns(
        pl.col("Month").replace(month_to_quarter).alias("Quarter")
    )
    insertToDB(
        monthly_data,
        "lpg_monthly_cdcms_sales_summary",
        indexing_col=["Month", "ZOName"],
    )
...
def process_subsidy_data(data, cursor, code=""):
    ...
    data = _merge_data(
        left_df=data,
        right_df=tblRefillResponseBatchHeader.select(
            ["PFMS_Batch_ID", "Bank_Debit_Date"]
        ),
    ...
    data = data.with_columns(
        pl.col("Distributor_Code").fill_null(0).cast(pl.Int64).alias("Distributor_Code")
    )
    # Getting SACode
    data = _merge_data(
        left_df=data,
        right_df=tblDistributorMaster.select(
            ["JDEDistributorCode", "SACode", "StateCode"]
        ),
    ...
    data = data.with_columns(
        pl.col("Booking_Date")
        .cast(pl.Utf8)
        .map_elements(lambda x: datetime.datetime.strptime(x, "%Y-%m-%d"))
        .alias("Booking_Date")
    )
    return data
...
def get_subsidy_failure_statistics():
    ...
    insertToDB(
        exception_data,
        "lpg_domestic_subsidy_exception",
        indexing_col=("Consumer_Scheme", "Booking_Date", "Product_Code"),
    )
    insertToDB(
        payment_error_data,
        "lpg_domestic_subsidy_payment_failure",
        indexing_col=("Consumer_Scheme", "Booking_Date", "Product_Code"),
    )
...
def get_subsidy_central_stats():
    ...
    data = data.with_columns(
        pl.col("Month_Name").replace(month_order).alias("month_number")
    )
    ...
    insertToDB(
        ...
        indexing_col=("ZOName", "Financial_Year", "Month"),
    )
...
def process_subsidy_data(data, cursor, table_name, code="payment"):
    ...
    data = _merge_data(
        left_df=data,
        right_df=tblDistributorMaster.select(
            ["JDEDistributorCode", "DistributorName", "SACode", "StateCode"]
        ),
        left_on=["Distributor_Code"],
        right_on=["JDEDistributorCode"],
        how="left",
...
def get_subsidy_exception_failure_data():
    ...
    process_subsidy_data(
        exception, cursor, "subsidy_exception_statistics", code="exception"
    )
...
def get_new_connection_data():
    ...
    data = data.with_columns(
        pl.col("MonthYear").str.split("-").list.get(0).alias("Month")
    )
    data = data.with_columns(
        pl.col("MonthYear").str.split("-").list.get(1).alias("Year")
    )
...
def get_dbc_enrollment():
    ...
    fetch_data(cursor, trunc_query, getData=False, params=params)
    insertToDB(
        data, "lpg_cdcms_dbc_enrollment_data", indexing_col=("ZOName", "MonthYear")
    )
...
def get_consumer_statistics():
    ...
    data = data.with_columns(
        pl.col("DistributorCode").fill_null(0).cast(pl.Int64).alias("DistributorCode")
    )
    # Getting SACode
    data = _merge_data(
        left_df=data,
    ...
    data = data.with_columns(
        (
            pl.col("ConsumerCount").fill_null(0).cast(pl.Int64)
            - pl.col("eKYCCompleted").fill_null(0).cast(pl.Int64)
        ).alias("eKYCPending")
    )
    ...
    insertToDB(
        ...
        indexing_col=(
            "ZOName",
            "RelationshipStatus",
            "RelationshipSubStatus",
            "ConsumerCategory",
            "CylinderType",
        ),
    )
    data = data.with_columns(
        pl.lit(datetime.datetime.now()).alias("Execution_Datetime")
    )
    data = data.with_columns(
        pl.col("Execution_Datetime")
        .dt.strftime("%Y-%m-%d")
        .map_elements(lambda x: datetime.datetime.strptime(x, "%Y-%m-%d"))
        .alias("Execution_Date")
    )

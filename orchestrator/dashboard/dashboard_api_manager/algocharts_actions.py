...
async def algocharts_get_columns(data: Algocharts_Get_ColumnsParams):
    ...
    async with session() as db:
        try:
            columns_list = await charts_functions.getColumns(
                db, data.schema, data.table
            )
            if not columns_list:
                return {
                    "status": False,
                    "message": "check the table or schema name",
                    "data": [],
                }
        except Exception as e:
            return {"status": False, "message": str(e), "data": []}
...
async def algocharts_get_unique_values(data: Algocharts_Get_Unique_ValuesParams):
    ...
    async with session() as db:
        try:
            columns_mapping = await charts_functions.getUniqueValues(
                db, data.schema, data.table, data.column
            )
            if not columns_mapping:
                return {
                    "status": False,
                    "message": "check the table or column name",
                    "data": [],
                }
        except Exception as e:
            return {"status": False, "message": str(e), "data": []}

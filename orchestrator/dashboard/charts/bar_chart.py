        ...
        try:
            ...
            if metrics_data.type == "Query":
                ...
                if "join" not in user_query.lower():
                    resp["query"] = await charts_functions.get_not_join_query(
                        user_query, metrics_data.organization_id
                    )
                else:
                    resp["query"] = await charts_functions.get_join_query(
                        user_query, metrics_data.organization_id
                    )
                ...
                grp_by_col = (
                    []
                )  # group_by column details in dictionary {'name':'','label':''}
                ...
                if not metrics_data.params.form_data.x_axis.label:
                    metrics_data.params.form_data.x_axis.label = (
                        metrics_data.params.form_data.x_axis.name
                    )
                column_labels[metrics_data.params.form_data.x_axis.name] = (
                    metrics_data.params.form_data.x_axis.label
                )
            ...
            x_axis = (
                metrics_data.params.form_data.x_axis.name
                if not metrics_data.params.form_data.x_axis.label
                else metrics_data.params.form_data.x_axis.label
            )
            if metrics_data.type == "Query":
            ...
            for row in query_results.fetchall():
                ...
                if x_axis_name and x_axis_name in row_dict:
                    row_dict[x_axis_name] = await charts_functions.timestamp_formatting(
                        row_dict[x_axis_name], x_axis_time_format
                    )
            ...
            sorted_data = sorted(
                final_list, key=lambda x: x[x_axis], reverse=x_axis_sorting
            )

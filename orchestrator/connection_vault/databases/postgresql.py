...
class Postgresql(BaseAction):
    ...
    async def get_connection(self):
        if "connection_name" in self.params.keys() and self.params["connection_name"]:
            ...
            self.params = {
                "host": db.host,
                "port": db.port,
                "user_name": db.query.split("&")[0].split("=")[-1],
                "password": db.query.split("&")[1].split("=")[-1],
    ...
    async def test_connection(self):
        ...
        try:
            ...
            return {"status": True, "message": "Connected to PostgresSQL", "data": []}
        except asyncpg.PostgresConnectionError as err:
            print(err, traceback.format_exc())
            # traceback.print_exc(file=sys.stdout)
            return {
                "status": False,
                "message": "Unable to connect to PostgresSQL",
                "data": [],
            }
    ...
    async def get_databases(self, debug=False, **kwargs):
        ...
        try:
            ...
            return {
                "status": True,
                "message": "Connected to PostgresSQL",
    ...
    async def query_builder(
        self, schema_name, table_name, condition=None, columns=None, limit=None
    ):
        """
        @description:
        :param schema_name:
        ...
        if columns:
            query += ", ".join(
                f'"{item["value"]}" AS "{item["label"]}"' for item in columns
            )
        else:
            query += "*"
        if schema_name:
    ...
    async def create_table(
        ...
        **kwargs,
        ...
        dtype_dict = {
            ...
            "Decimal(precision=8, scale=3)": "numeric(10,3)",
            "Decimal(precision=5, scale=2)": "numeric(10,2)",
            "Decimal(precision=10, scale=4)": "numeric(10,4)",
        ...
        if unique_key:
            ...
            constraint_query = (
                f"CONSTRAINT uk_{schema_name}_{table_name} UNIQUE ({result_string}),"
            )
        if constraint_query:
    ...
    async def write_data_from_csv(
        self, records, create_table_name, schema_name="public", **kwargs
    ):
        """
        @description:
        :param records:
        ...
        await self.create_table(schema_name, create_table_name, records.head(10))
        ...
        # await connection.close()
        await self.close_connection(connection)
        return {"status": True, "message": "Data inserted Successfully", "data": []}
        ...
        **kwargs,
        ...
        if table_name in [
            ...
            if data and table_name == "industry_performance":
                data = [
                    rec
                    for rec in data
                    if rec["key"] not in ["SalesArea_Name", "Region_Name"]
                ]
            if data:
                where_clause = data
        else:
        ...
        try:
            ...
            for column in column_name:
                    ...
                    if isinstance(where_clause, list):
                        for condition in where_clause:
                            if isinstance(
                                ...
                                condition, str
                            ):  # Handle shorthand single condition (string format)
                ...
                # Collect data
                columns_mapping[column] = [
                    record[column] for record in data if record.get(column)
                ]
    ...
    async def upsert_data(
        ...
        **kwargs,
        ...
        if not update_columns:
            update_columns = [
                col for col in records.columns if col not in conflict_columns
            ]
        ...
        result = await self.create_table(
            schema_name, table_name, records.head(10), unique_key=conflict_columns
        )
        if schema_name:
            table_name = f'{schema_name}"."{table_name}'
        sql_query = f"""
...
class QueryBuilder:
    ...
    async def is_num_val(self, val) -> str:
        ...
        if isinstance(val, list):
            query_val = ", ".join(
                f"'{val}'" if self.isfloat(val) else f"'{v}'" for v in val
            )
        else:
    ...
    async def add_metric_to_col(
        ...
        for json_condition in join_conditions:
            if json_condition.get("source_table", "") and json_condition.get(
                "target_table", ""
            ):
                ...
                for json_col in json_condition.get("join_where_clause", []):
                    map_columns[json_condition.get("source_table", "")].update(
                        {
                            f'"{json_col["source_column"]}"': f'"{json_col["source_label"]}"'
                        }
                    )
                    map_columns[json_condition.get("target_table", "")].update(
                        {
                            f'"{json_col["target_column"]}"': f'"{json_col["target_label"]}"'
                        }
                    )
                    group_by_map[json_condition.get("source_table", "")].append(
                        f'"{json_col["source_column"]}"'
                    )
                    group_by_map[json_condition.get("target_table", "")].append(
                        f'"{json_col["target_column"]}"'
                    )

                for column, alias_col in json_condition.get(
                    "select_columns", {}
                ).items():
                    map_columns[json_condition.get("target_table", "")].update(
                        {f'"{column}"': f'"{alias_col}"'}
                    )
                    group_by_map[json_condition.get("target_table", "")].append(
                        f'"{column}"'
                    )
        ...
        for table_name, columns_dict in columns_dicts.items():
            query_columns = ", ".join(
                (
                    ...
                    if column.split("(")[0]
                    not in [op.value for op in AggregationOperator]
    ...
    async def from_clause(self, table: str, table_mapping: typing.Dict) -> str:
        """
    ...
    async def join_query_builder(
        ...
        for each_join_query in join_query_json:
            ...
            for filters in each_join_query["filters"]:
                filters["dtype"] = filters.get(
                    "dtype", "character varying"
                )  # To Do need to get datatype from table
                where_cond = await self.where_clause(
                    filters, target_table, table_mappings
                )
            ...
            if each_join_query.get("select_columns", {}):
                for column, alias_column in each_join_query.get(
                    "select_columns", {}
                ).items():
            ...
            join_cond += (
                f' FROM "{target_table}" AS {table_mappings.get(target_table, "a")}'
            )
            ...
            for each_cond in each_join_query["join_where_clause"]:
                source_cond = (
                    f"""'{each_cond["source_column"]}'"""
                    if each_cond["textSSBox"]
                    else f'''{table_mappings.get(each_join_query["source_table"], "a")}."{each_cond['source_column']}"'''
                )
                target_cond = (
                    f"""'{each_cond["target_column"]}'"""
                    if each_cond["textSTBox"]
                    else f'''{table_mappings.get(each_join_query["target_table"], "a")}."{each_cond['target_column']}"'''
                )
            ...
            join_query = (
                f'{join_cond} AS {table_mappings.get(target_table, "a")} {join_query}'
            )
    ...
    async def where_clause(
        self, filter_cond: typing.Dict, table: str, table_mapping: typing.Dict
    ) -> str:
        """
        ...
        if op == "TEMPORAL_RANGE":
            ...
            if match:
                ...
                where_clause_cond += (
                    f"""{select_column} BETWEEN '{start_date}' AND '{end_date}' """
                )
    ...
    async def group_by_query_builder(
        ...
        for group in group_by_json:
            select_column = await self.select_col(
                "", group["column"], table_mapping.get(table_name, "a")
            )
    ...
    async def having_query_builder(
        ...
        for filters in having_query_json:
            ...
            where_cond = await self.where_clause(
                filters, table=table_name, table_mapping=table_mapping
            )
            if count == 1:
    ...
    async def order_by_query_builder(
        ...
        for filters in order_by_query_json:
            select_column = await self.select_col(
                filters["agg"], filters["column"], table_mapping.get(table_name, "a")
            )
    ...
    async def limit_query_builder(self, limit: int) -> str:
        """
    ...
    async def offset_query_builder(self, offset: int) -> str:
        """
    ...
    async def generate_query(
        ...
        table_mappings = await self.map_alias_name_to_table(
            [query_context["table_name"]] + query_context["join_tables"]
        )
        ...
        if query_context.get("map_column", {}):
            table_column_map = await self.get_select_columns(
                query_context["map_column"], [{}], table_mappings
            )
        ...
        if query_context.get("join_tables", []):
            query += await self.join_query_builder(
                query_context.get("join_conditions", {}), table_mappings
            )
        ...
        if query_context.get("metrics", []):
            ...
            for table, column in group_by_map.items():
                for each_col in column:
                    if count == 1:
                        query += (
                            f'GROUP BY "{table_mappings.get(table, "a")}".{each_col} '
                        )
                    else:

...
async def fetch_va_score(bu):
    ...
    if resp["status"]:
        va_score = {
            rec.get("LOCATION_ID", "").split(",")[-1].strip(): rec
...
"""
    ...
    while True:
        resp = await hpcl_ceg_model.LocationMaster.get_aggr_data(
    ...
    fallback_scores = await validate_locations_from_history(bu, location_id, va_data)
    ...
    performance_score = {}
    present_timestamp = datetime.datetime.now(tz=datetime.timezone.utc).strftime(
        "%Y-%m-%d"
    )
    ...
    for sap_id, rec in performance_score.items():
        performance_score[sap_id]["rank"] = int(
            df[df["sap_id"] == sap_id]["rank"].values[0]
        )
    ...
    await hpcl_ceg_model.PerformanceScore.bulk_update(
        performance_score.copy(), upsert=True
    )
    await hpcl_ceg_model.PerformanceScoreHistory.bulk_update(
        performance_score, upsert=True
    )
  ...
  python performance_score_generator.py --bu TAS --location-ids "1216 1221 1334"
    ...
    if args.location_ids:
        ...
        if "," in args.location_ids:
            location_ids = [
                lid.strip() for lid in args.location_ids.split(",") if lid.strip()
            ]
        else:
            location_ids = [
                lid.strip() for lid in args.location_ids.split() if lid.strip()

...
def get_priority_level(
    score_gap: float, weightage: float, max_total_score: float = 100
) -> str:
    """
    ...
    gap_percentage_of_weightage = float(
        round((score_gap / weightage * 100) if weightage > 0 else 0, 2)
    )
...
def generate_reason_for_score(
    score: float, weightage: float, msg: str = "", details: Optional[Dict] = None
) -> Dict:
    """
    ...
    percentage_achieved = float(
        round((score / weightage * 100) if weightage > 0 else 0, 2)
    )
...
def generate_actions_for_improvement(
    module_name: str,
    rule_name: str,
    score: float,
    weightage: float,
    ...
    if module_type == "open_alerts":
        ...
        or module_type == "gantry_interlocks"
        or module_type == "process_interlocks"
    ):
    ...
    if not actions:
        ...
        global_impact_pct = float(
            round((score_gap / 100) * 100, 2) if score_gap > 0 else 0
        )
        actions.append(
            {
...
def generate_summary_insights(
    module_results: Dict, parent_weightage: float = 0, category_weightage: float = 0
) -> Dict:
    """
    ...
    if parent_weightage > 0:
        ...
        module_actual_contribution = (
            (module_score / module_weightage) * module_max_contribution
            if module_weightage > 0
            else 0
        )
    ...
    if not results:
        return {
            ...
            "contribution_to_overall_score": float(
                round(module_actual_contribution, 2)
            ),
            "max_contribution_to_overall_score": float(
                round(module_max_contribution, 2)
            ),
            "module_score": float(round(module_score, 2)),
            "module_weightage": float(round(module_weightage, 2)),
            "contribution_percentage": float(
                round((module_actual_contribution / 100) * 100, 4)
            ),
            "max_contribution_percentage": float(
                round((module_max_contribution / 100) * 100, 4)
            ),
            "critical_issues": [],
    ...
    effective_parent_weightage = (
        parent_weightage if parent_weightage > 0 else category_weightage
    )
    if effective_parent_weightage == 0:
        effective_parent_weightage = (
            module_weightage  # Use module's own weightage as fallback
        )
            ...
            critical_issues.append(
                {
                    ...
                    "contribution_to_overall_score": float(
                        result_nested_contribution["actual_contribution_to_overall"]
                    ),
                    "max_contribution_to_overall_score": float(
                        result_nested_contribution["max_contribution_to_overall"]
                    ),
                    "contribution_percentage": float(
                        result_nested_contribution["contribution_percentage"]
                    ),
                    "max_contribution_percentage": float(
                        result_nested_contribution["max_contribution_percentage"]
                    ),
                    "contribution_gap": float(
                        result_nested_contribution["contribution_gap"]
                    ),
                    "contribution_gap_percentage": float(
                        result_nested_contribution["contribution_gap_percentage"]
                    ),
            ...
            quick_wins.append(
                {
                    ...
                    "action": (
                        actions[0].get("action") if actions else "Review and improve"
                    ),
                }
            )
...
def calculate_nested_contribution(
    score: float,
    weightage: float,
    parent_weightage: float,
    category_weightage: float,
    max_total_score: float = 100,
) -> Dict:
    """
    ...
    return {
        ...
        "contribution_percentage": float(
            round((actual_contribution / max_total_score) * 100, 2)
        ),
        "max_contribution_percentage": float(
            round((max_contribution / max_total_score) * 100, 2)
        ),
        "contribution_gap": float(round(max_gap_contribution, 2)),
        "contribution_gap_percentage": float(
            round((max_gap_contribution / max_total_score) * 100, 2)
        ),
    }
...
def extract_results_from_category(category: Dict) -> List[Dict]:
    ...
    for result in direct_results:
        ...
        if "results" in result and isinstance(result["results"], list):
            ...
            result["parent_weightage"] = float(
                0
            )  # No parent, so use category weightage directly
...
def generate_overall_insights(performance_score: Dict) -> Dict:
        ...
        module_gaps.append(
            {
                ...
                "contribution_to_overall_score": float(
                    round(category_score, 2)
                ),  # Actual contribution to overall score (out of 100)
                "contribution_percentage": float(
                    round((category_score / max_possible) * 100, 2)
    ...
    for result in all_results:
        ...
        if "reason" not in result or "actions" not in result:
            ...
            parent_result_name = (
                f"{result['parent_result']}".lower()
                if result.get("parent_result")
                else ""
            )
        ...
        if priority == "CRITICAL" and gap > 0:
            ...
            if parent_result:
                ...
                full_name = (
                    f"{parent_module} > {rule_name}" if parent_module else rule_name
                )
            ...
            all_critical.append(
                {
                    ...
                    "section_weightage": float(
                        round(weightage, 2)
                    ),  # Weightage within its section
                    "contribution_to_overall_score": float(
                        nested_contribution["actual_contribution_to_overall"]
                    ),  # Actual contribution to overall score (out of 100)
                    "max_contribution_to_overall_score": float(
                        nested_contribution["max_contribution_to_overall"]
                    ),  # Max possible contribution
                    "contribution_percentage": float(
                        nested_contribution["contribution_percentage"]
                    ),  # Percentage of total score (actual)
                    "max_contribution_percentage": float(
                        nested_contribution["max_contribution_percentage"]
                    ),  # Max percentage possible
                    "contribution_gap": float(
                        nested_contribution["contribution_gap"]
                    ),  # Gap in contribution to overall score
                    "contribution_gap_percentage": float(
                        nested_contribution["contribution_gap_percentage"]
                    ),  # Gap as percentage of total
                    "percentage_achieved": float(
                        reason_obj.get(
                            "percentage_achieved", round((score / weightage * 100), 2)
                        )
                    ),
                    "explanation": result.get(
                        ...
                        reason_obj.get(
                            "explanation",
                            f"Score is {float(round(score, 2))} out of {float(round(weightage, 2))}",
                        ),
                    ),
                    "actions": [
                        a.get("action") for a in result.get("actions", [])[:2]
                    ],  # Top 2 actions
                }
            )
        ...
        if priority in ["CRITICAL", "HIGH"] and gap > 0:
            ...
            for action in actions:
                ...
                if "0 " in description and (
                    "vehicles have active" in description
                    or "devices have active" in description
                ):
                    # Check if this is actually a valid case (score gap exists but no alerts)
                    if (
                        "configuration" in description.lower()
                        or "calculation" in description.lower()
                    ):
            ...
            if valid_actions:
                for action in valid_actions[:2]:  # Top 2 actions per result
                    ...
                    if action_priority in ["CRITICAL", "HIGH"]:
                        ...
                        action_copy["module"] = result.get(
                            "parent_module", result.get("module", "")
                        )
                        ...
                        action_copy["global_impact"] = float(
                            round(gap, 2)
                        )  # Same value, but clearer naming
                        action_copy["global_impact_percentage"] = float(
                            round((gap / 100) * 100, 2)
                        )  # Percentage of total score
                        # Add contribution fields
                        action_copy["contribution_to_overall_score"] = float(
                            nested_contribution["actual_contribution_to_overall"]
                        )
                        action_copy["contribution_gap"] = float(
                            nested_contribution["contribution_gap"]
                        )
                        ...
                        if "impact" in action_copy:
                            if (
                                "points" in action_copy["impact"]
                                and "overall score" not in action_copy["impact"]
                            ):
                                action_copy["impact"] = action_copy["impact"].replace(
                                    "points", "points on overall score"
                                )
    ...
    def action_sort_key(action):
        ...
        return (
            priority_order.get(action.get("priority", "LOW"), 99),
            -action.get("score_gap", 0),
        )
    ...
    for mg in module_gaps[:3]:  # Top 3 modules
        ...
        module_results = [
            ...
            if r.get("parent_module") == module_name or r.get("module") == module_name
        ]
        module_results.sort(
            key=lambda x: calculate_score_gap(
                float(x.get("score", 0)), float(x.get("weightage", 0))
            ),
            reverse=True,
        )
        ...
        for result in module_results[:5]:  # Top 5 issues in this module
            # Convert numpy types in result
            result = convert_numpy_types(result)
            result_gap = calculate_score_gap(
                float(result.get("score", 0)), float(result.get("weightage", 0))
            )
            if result_gap > 0:
                ...
                result_nested_contribution = calculate_nested_contribution(
                    result_score,
                    result.get("weightage", 0),
                    result_parent_weightage,
                    result_category_weightage,
                    max_possible,
                )
                ...
                top_issues.append(
                    {
                        ...
                        "section_score": float(
                            round(result_score, 2)
                        ),  # Score within its section
                        "section_weightage": float(
                            round(result.get("weightage", 0), 2)
                        ),  # Weightage within its section
                        "contribution_to_overall_score": float(
                            result_nested_contribution["actual_contribution_to_overall"]
                        ),  # Actual contribution to overall score (out of 100)
                        ...
                        "priority": result.get(
                            "priority",
                            get_priority_level(result_gap, result.get("weightage", 0)),
                        ),
                        "explanation": result.get(
                            "msg", result.get("reason", {}).get("explanation", "")
                        ),
                        "top_action": (
                            result.get("actions", [{}])[0].get("action", "")
                            if result.get("actions")
                            else ""
                        ),
                        "action_impact": (
                            result.get("actions", [{}])[0].get("impact", "")
                            if result.get("actions")
                            else ""
                        ),
                    }
        ...
        for issue in top_issues[:3]:  # Top 3 issues
            ...
            matching_result = next(
                (r for r in module_results if r.get("name") == issue["rule"]), None
            )
            if matching_result and matching_result.get("actions"):
                for action in matching_result["actions"][
                    :1
        ...
        focus_areas.append(
            {
                ...
                "contribution_percentage": float(
                    round((mg["score"] / max_possible) * 100, 2)
                ),  # Percentage of total score
    ...
    for category in categories:
        ...
        for module_result in category_results:
            ...
            if "results" in module_result and isinstance(
                module_result.get("results"), list
            ):
                ...
                if "insights" in module_result:
                    ...
                    insights["contribution_to_overall_score"] = float(
                        round(module_actual_contribution, 4)
                    )
                    insights["max_contribution_to_overall_score"] = float(
                        round(module_max_contribution, 4)
                    )
                    insights["contribution_percentage"] = float(
                        round((module_actual_contribution / max_possible) * 100, 4)
                    )
                    insights["max_contribution_percentage"] = float(
                        round((module_max_contribution / max_possible) * 100, 4)
                    )
                    ...
                    for issue in insights.get("critical_issues", []):
                        ...
                        matching_result = next(
                            (
                                r
                                for r in module_result.get("results", [])
                        ...
                        if matching_result:
                            ...
                            issue["contribution_to_overall_score"] = float(
                                rule_nested_contribution[
                                    "actual_contribution_to_overall"
                                ]
                            )
                            issue["max_contribution_to_overall_score"] = float(
                                rule_nested_contribution["max_contribution_to_overall"]
                            )
                            issue["contribution_percentage"] = float(
                                rule_nested_contribution["contribution_percentage"]
                            )
                            issue["max_contribution_percentage"] = float(
                                rule_nested_contribution["max_contribution_percentage"]
                            )
                            issue["contribution_gap"] = float(
                                rule_nested_contribution["contribution_gap"]
                            )
                            issue["contribution_gap_percentage"] = float(
                                rule_nested_contribution["contribution_gap_percentage"]
                            )
                    ...
                    for quick_win in insights.get("quick_wins", []):
                        ...
                        if matching_result:
                            ...
                            quick_win["contribution_to_overall_score"] = float(
                                rule_nested_contribution[
                                    "actual_contribution_to_overall"
                                ]
                            )
                            quick_win["max_contribution_to_overall_score"] = float(
                                rule_nested_contribution["max_contribution_to_overall"]
                            )
                            quick_win["contribution_percentage"] = float(
                                rule_nested_contribution["contribution_percentage"]
                            )
                            quick_win["contribution_gap"] = float(
                                rule_nested_contribution["contribution_gap"]
                            )
                            quick_win["section_score"] = float(round(rule_score, 2))
                            quick_win["section_weightage"] = float(
                                round(rule_weightage, 2)
                            )
    ...
    return {
        ...
        "critical_issues": all_critical[
            :10
        ],  # Top 10 critical issues across all modules
        "recommended_actions": all_high_priority_actions[
            :20

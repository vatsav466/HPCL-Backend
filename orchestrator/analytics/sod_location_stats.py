...
async def generate_sod_engineering_location_stats(sap_id):
    ...
    try:
        query = (
            f"sap_id = '{location_data['sap_id']}' and device_type in {device_types}"
        )
        params = urdhva_base.queryparams.QueryParams()
        params.fields = []
        params.q = query
            ...
            for equip in equipment_names:
                ...
                if maintenance_item["alert_category"] in ("Safety", "Process"):
                    if maintenance_item["interlock_name"] not in global_interlock:
                        global_interlock.add(maintenance_item["interlock_name"])
                        interlocks.append(maintenance_item["interlock_name"])
                        maintenance_map.setdefault(equip, set()).add(
                            maintenance_item["interlock_name"]
                        )
            ...
            for equip in equipment_names:
                ...
                if fault_item["alert_category"] == "Safety":
                    if fault_item["interlock_name"] not in global_interlock:
                        global_interlock.add(fault_item["interlock_name"])
                        interlocks.append(fault_item["interlock_name"])
                        fault_map.setdefault(equip, set()).add(
                            fault_item["interlock_name"]
                        )
        ...
        LRC_NAME_MAP = {"lrca": "lrca", "lrc1": "lrca", "lrcb": "lrcb", "lrc2": "lrcb"}
        if resp.get("data"):
            df = pl.DataFrame(resp["data"]).select(["active_server_name", "status"])
            master_record = df.filter(pl.col("status").cast(pl.Int32) == 1).select(
                "active_server_name"
            )
            if master_record.height > 0:
                ...
                for server in hardcoded_status:
                    server["status"] = (
                        "master" if server["id"] == master_id else "slave"
                    )
        ...
        for _, row in grouped_df.iterrows():
            ...
            maintenance_device_names, total_maintenance_count = (
                await get_alert_count_for_interlock_set(
                    maintenance_interlocks, mapped_name, sap_id
                )
            )
            fault_device_names, total_fault_count = (
                await get_alert_count_for_interlock_set(
                    fault_interlocks, mapped_name, sap_id
                )
            )
        ...
        matching_plcs = [
            entry for entry in plc_status_data if entry["sap_id"] == str(sap_id)
        ]
        ...
        for plc in matching_plcs:
            ...
            if plc_type:
                ...
                if plc_status_dict[a_key]["status"] != "master" and plc[
                    "plc_a_status"
                ] in ("master", "slave"):
                    plc_status_dict[a_key]["status"] = plc["plc_a_status"]
                if plc_status_dict[b_key]["status"] != "master" and plc[
                    "plc_b_status"
                ] in ("master", "slave"):
...
async def get_filtered_location_data(
    bu, location_onboard, specific_zone=None, specific_sap_id=None
):
    """
    ...
    return {
        "status": True,
        "message": "Success",
        "data": {"zone": zone_list, "sap_id": sap_id_list},
    }

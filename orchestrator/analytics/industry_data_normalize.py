    ...
    for fis_year in df["FIN_YEAR"].unique().tolist():
        ...
        if start_month:
            filtered_months = filtered_months[
                filtered_months.index(start_month.upper()) :
            ]
        if end_month:
            filtered_months = filtered_months[
                : filtered_months.index(end_month.upper()) + 1
            ]
...
def fetch_industry_raw_data(
    actual=True, history=True, start_month=None, end_month=None
):
    """
    ...
    df = df.melt(
        id_vars=columns, value_vars=Months, var_name="MONTH", value_name="VALUE"
    )
...
def get_group_by_keys(
    by_month=False, by_product=False, by_year=True, by_company=True, comp_type=False
):
    group_keys = []
    if by_year:
...
def get_filtered_data(df, filters):
    ...
    for filter in filters:
        if filter["cond"] in ["=", "equal", "equals"]:
            ...
            df = df[
                df[filter["key"]].isin([k.strip() for k in filter["value"].split(",")])
            ]
    return df


def get_industry_share(
    actual=True,
    history=True,
    start_month=None,
    end_month=None,
    ...
    fiscal_year.FiscalYear.fiscal_year_start_date
    df["FIN_YEAR"].unique().tolist()
    market_size = {
        rec["FIN_YEAR"]: round(rec["VALUE"] / 1000, 2)
        for rec in df.groupby(["FIN_YEAR"])["VALUE"]
        .sum()
        .reset_index()
...
def get_industry_share_old(
    ...
    :return:
    """
    df = filter_data_by_month(
        fetch_industry_raw_data(actual, history), start_month, end_month
    )
    ...
    overall_share = {
        rec["FIN_YEAR"]: {
            "share": rec["VALUE"],
            "avg_share": rec["VALUE"] / len(unique_years),
        }
        for rec in overall_share.to_dict(orient="records")
    }
    psu_share = (
        df[df["Co"].isin(["IOCL", "HPCL", "BPCL"])]
        .groupby(["FIN_YEAR"])["VALUE"]
        .sum()
        .reset_index()
    )
    ...
    for year in unique_years:
        for company in unique_companies:
            share = round(
                (
                    df[(df["COMNAME"] == company) & (df["FIN_YEAR"] == year)][
                        "VALUE"
                    ].sum()
                )
                / 1000,
...
def get_performance_metrics(
    actual=True, history=True, start_month=None, end_month=None, sbu=None
):
    df = filter_data_by_month(
        fetch_industry_raw_data(actual, history), start_month, end_month
    )
    if sbu:
        ...
        df = df[df["SBU"] in sbu]
    company_wise = (
        df.groupby(["FIN_YEAR", "MONTH", "COMNAME", "COMP_TYPE"])["VALUE"]
        .sum()
        .reset_index()
    )
    pivoted_df = company_wise.pivot(
        index=["COMNAME", "COMP_TYPE", "MONTH"], columns="FIN_YEAR", values="VALUE"
    ).reset_index()
    pivoted_df = pivoted_df.rename(
        columns={"2023-2024": "LAST_YEAR", "2024-2025": "PRESENT_YEAR"}
    )
    # Calculate the difference and percentage growth
    pivoted_df["VALUE_DIFF"] = pivoted_df["PRESENT_YEAR"] - pivoted_df["LAST_YEAR"]
    pivoted_df["PERCENT_GROWTH"] = (
        pivoted_df["VALUE_DIFF"] / pivoted_df["LAST_YEAR"]
    ) * 100
    pivoted_df["data_order"] = pivoted_df["MONTH"].map(
        {cond: i for i, cond in enumerate(Months)}
    )

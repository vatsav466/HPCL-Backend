...
class SolarHelpers:
    ...
    @classmethod
    async def enrich_with_location_master(
        cls,
        df: pl.DataFrame,
        join_column: str = "BU Code",
        filters=None,
        drill_state="",
    ):
        """
        ...
        try:
            ...
            if filters:
                ...
                valid_filters = cls.validate_filters_for_query(
                    filters, available_columns=available_columns
                )
                ...
                if valid_filters:
                    query = await widget_actions.WidgetActions.apply_filter_drilldown(
                        query, valid_filters, drill_state
                    )
            ...
            if rows:
                ...
                # Join on sap_id
                df = df.join(
                    location_df.select(["bu", "sap_id", "name", "zone"])
                    .unique(subset=["sap_id"])
                    .with_columns(pl.col("sap_id").alias("sap_id_join")),
    ...
    async def get_solar_master_data(cls, filters=None, drill_state=""):
        ...
        try:
            base_path = os.path.join(
                urdhva_base.settings.base_path,
                "orchestrator",
                "masters",
            ...
            solar = await cls.enrich_with_location_master(
                solar, join_column="BU Code", filters=filters, drill_state=drill_state
            )
        ...
        if getData:
            ...
            if enrich_with_location and "PLANT_CD" in data.columns:
                try:
                    ...
                    data = await cls.enrich_with_location_master(
                        data,
    ...
    def _get_capped_month_range(current_date, months):
        ...
        try:
            ...
            month_ago = current_date.replace(year=year, month=month, day=last_day)
        max_days = months * 31
    ...
    def extract_date_range_from_filters(cls, filters):
        ...
        for filter_item in filters:
            ...
            col = (
                filter_dict.get("key")
                or filter_dict.get("columns")
                or filter_dict.get("column")
                or filter_dict.get("field")
                or filter_dict.get("col")
            )
            op = (
                (
                    filter_dict.get("cond")
                    or filter_dict.get("operator")
                    or filter_dict.get("op")
                    or ""
                )
                .strip()
                .lower()
                ...
                # Handle date_filter and date_range
                    ...
                    if op == "date_range" and isinstance(val, str) and "," in val:
                        ...
                        if isinstance(val, str) and "," in val:
                            ...
                            isinstance(val, str) and len(val) == 10
                        ):  # Single date YYYY-MM-DD
                            try:
                                single_date = datetime.datetime.strptime(
                                    val, "%Y-%m-%d"
                                ).date()
                            ...
                            if val == "t":
                                ...
                                start_date, end_date = cls._get_capped_month_range(
                                    current_date, 1
                                )
                ...
                try:
                    ...
                    if op_lower == "date_filter":
                        ...
                        if df[col].dtype == pl.Utf8:
                            date_col = pl.col(col).str.strptime(
                        ...
                        if val == "24h":
                            ...
                            yesterday = current_date - datetime.timedelta(days=1)
                            df = df.filter(
                                (date_col >= pl.lit(yesterday))
                                & (date_col < pl.lit(current_date))
                            )
                        elif val == "1w":
                            week_ago = current_date - datetime.timedelta(days=6)
                            ...
                            if len(date_range) == 2:
                                ...
                                try:
                                    ...
                                    df = df.filter(
                                        (date_col >= pl.lit(start_date))
                                        & (date_col <= pl.lit(end_date))
                                    )
                                except ValueError:
                                    pass
                            else:
                    ...
                    if (
                        col == "TimestampUTC"
                        and df[col].dtype in [pl.Datetime]
                        and op_lower in ["=", "equals", "equal"]
                    ):
                        if isinstance(val, list):
                            df = df.filter(
                                expr.cast(pl.Date)
                                .cast(pl.Utf8)
                                .is_in([str(v) for v in val])
                            )
                        elif isinstance(val, str) and "," in val:
                            values_list = [v.strip() for v in val.split(",")]
                            df = df.filter(
                                expr.cast(pl.Date).cast(pl.Utf8).is_in(values_list)
                            )
                        else:
                    ...
                    if op_lower in ["=", "equals", "equal"]:
                        if isinstance(val, list):
                            ...
                            df = df.filter(
                                expr.is_not_null()
                                & (expr.cast(pl.Utf8).str.strip_chars() == str(val))
                            )
                        ...
                        if isinstance(val, str) and "," in val:
                            ...
                            for v in values_list:
                                condition = condition | (
                                    expr.cast(pl.Utf8).str.contains(
                                        str(v), literal=True
                                    )
                                )
                            ...
                            df = df.filter(
                                expr.cast(pl.Utf8).str.contains(str(val), literal=True)
                            )
                        ...
                        if isinstance(val, list):
                            ...
                            values_list = [v.strip() for v in val.split(",")]
                            df = df.filter(
                                expr.cast(pl.Utf8).str.strip_chars().is_in(values_list)
                            )
                        else:
                            df = df.filter(
                                expr.cast(pl.Utf8).str.strip_chars() == str(val)
                        ...
                        df = df.filter(
                            expr.cast(pl.Utf8).str.contains(str(val), literal=False)
                        )
                    elif op_lower in [">", "<", ">=", "<="]:
                            ...
                            if isinstance(val, str):
                                try:
                                    if df[col].dtype == pl.Datetime:
                                        ...
                                        try:
                                            parsed_val = datetime.datetime.strptime(
                                                val, "%Y-%m-%d %H:%M:%S"
                                            )
                                        except ValueError:
                                            try:

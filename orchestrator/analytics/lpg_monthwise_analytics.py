...
async def get_monthwise_production(
    ...
    if aggregation_level == "zone":
        ...
        grouped = _aggregate_by_level(
            df, "zone", ["month_date", "month_year", "month_name", "zone"], agg_dict
        )
        ...
        grouped = _aggregate_by_level(
            df, "overall", ["month_date", "month_year", "month_name"], agg_dict
        )
    ...
    avg_monthly_production = (
        float(grouped["total_production_mt"].mean()) if len(grouped) > 0 else 0.0
    )
    max_month_production = (
        float(grouped["total_production_mt"].max()) if len(grouped) > 0 else 0.0
    )
    min_month_production = (
        float(grouped["total_production_mt"].min()) if len(grouped) > 0 else 0.0
    )

    # Find months with max and min production
    max_month_row = (
        grouped.loc[grouped["total_production_mt"].idxmax()]
        if len(grouped) > 0
        else None
    )
    min_month_row = (
        grouped.loc[grouped["total_production_mt"].idxmin()]
        if len(grouped) > 0
        else None
    )
    ...
    summary = {
        ...
        "max_production_month": (
            max_month_row["month_year"] if max_month_row is not None else None
        ),
        "min_production_month": (
            min_month_row["month_year"] if min_month_row is not None else None
        ),
...
async def lpg_operations_monthwise_productivity(
    ...
    def calculate_productivity_and_summary(grouped_df):
        ...
        # Calculate summary statistics
        avg_productivity = (
            float(grouped_df["productivity"].mean()) if len(grouped_df) > 0 else 0.0
        )
        max_productivity = (
            float(grouped_df["productivity"].max()) if len(grouped_df) > 0 else 0.0
        )
        min_productivity = (
            float(grouped_df["productivity"].min()) if len(grouped_df) > 0 else 0.0
        )
        total_production = float(grouped_df["total_production"].sum())
        total_net_hours = float(grouped_df["total_net_hours"].sum())

        max_month_row = (
            grouped_df.loc[grouped_df["productivity"].idxmax()]
            if len(grouped_df) > 0
            else None
        )
        min_month_row = (
            grouped_df.loc[grouped_df["productivity"].idxmin()]
            if len(grouped_df) > 0
            else None
        )
        ...
        if len(grouped_df) > 0 and max_month_row is not None:
            ...
            max_month_year = (
                pd.to_datetime(max_month_date).strftime("%Y-%m")
                if pd.notna(max_month_date)
                else None
            )
        else:
            max_month_year = None
        ...
        if len(grouped_df) > 0 and min_month_row is not None:
            ...
            min_month_year = (
                pd.to_datetime(min_month_date).strftime("%Y-%m")
                if pd.notna(min_month_date)
                else None
            )
        else:
            min_month_year = None
        ...
        summary = {
            ...
            "overall_productivity": (
                round(total_production / total_net_hours, 2)
                if total_net_hours > 0
                else 0.0
            ),
        }
    ...
    overall_grouped, overall_summary = calculate_productivity_and_summary(
        overall_grouped
    )
    # Keep only month_date, total_production, total_net_hours, productivity
    overall_grouped = overall_grouped[
        ["month_date", "total_production", "total_net_hours", "productivity"]
    ]
    ...
    if location_level_req:
        location_grouped = df.groupby(
            ["month_date", "sap_id", "plant", "zone"], as_index=False
        ).agg({"total_production": "sum", "total_net_hours": "sum"})
        location_grouped, location_summary = calculate_productivity_and_summary(
            location_grouped
        )
...
async def lpg_operations_monthwise_rejections(
    ...
    def calculate_rejections_and_summary(grouped_df, rej_type):
        ...
        for col in [
            "cs_rejection_percent",
            "pt_rejection_percent",
            "gd_rejection_percent",
        ]:
        ...
        avg_rejection = (
            float(grouped_df[summary_col].mean()) if len(grouped_df) > 0 else 0.0
        )
        max_rejection = (
            float(grouped_df[summary_col].max()) if len(grouped_df) > 0 else 0.0
        )
        min_rejection = (
            float(grouped_df[summary_col].min()) if len(grouped_df) > 0 else 0.0
        )

        max_month_row = (
            grouped_df.loc[grouped_df[summary_col].idxmax()]
            if len(grouped_df) > 0
            else None
        )
        min_month_row = (
            grouped_df.loc[grouped_df[summary_col].idxmin()]
            if len(grouped_df) > 0
            else None
        )
    ...
    if rejection_type == "cs":
        ...
        overall_grouped = df.groupby(["month_date"], as_index=False).agg(
            {"cs_handled": "sum", "cs_sortout": "sum"}
        )
        overall_grouped, overall_summary = calculate_rejections_and_summary(
            overall_grouped, "cs"
        )
        overall_grouped = overall_grouped[["month_date", "rejection_percent"]]
        overall_grouped = overall_grouped.rename(
            columns={"rejection_percent": "cs_rejection_percent"}
        )

    elif rejection_type == "pt":
        overall_grouped = df.groupby(["month_date"], as_index=False).agg(
            {"pt_handled": "sum", "pt_sortout": "sum"}
        )
        overall_grouped, overall_summary = calculate_rejections_and_summary(
            overall_grouped, "pt"
        )
        overall_grouped = overall_grouped[["month_date", "rejection_percent"]]
        overall_grouped = overall_grouped.rename(
            columns={"rejection_percent": "pt_rejection_percent"}
        )

    elif rejection_type == "gd":
        overall_grouped = df.groupby(["month_date"], as_index=False).agg(
            {"gd_handled": "sum", "gd_sortout": "sum"}
        )
        overall_grouped, overall_summary = calculate_rejections_and_summary(
            overall_grouped, "gd"
        )
        overall_grouped = overall_grouped[["month_date", "rejection_percent"]]
        overall_grouped = overall_grouped.rename(
            columns={"rejection_percent": "gd_rejection_percent"}
        )
    ...
    if zone_level_req:
        if rejection_type == "cs":
            ...
            zone_grouped, zone_summary = calculate_rejections_and_summary(
                zone_grouped, "cs"
            )
            zone_grouped = zone_grouped[["month_date", "zone", "rejection_percent"]]
            zone_grouped = zone_grouped.rename(
                columns={"rejection_percent": "cs_rejection_percent"}
            )
    ...
    if location_level_req:
        if rejection_type == "cs":
            location_grouped = df.groupby(
                ["month_date", "sap_id", "plant", "zone"], as_index=False
            ).agg({"cs_handled": "sum", "cs_sortout": "sum"})
            location_grouped, location_summary = calculate_rejections_and_summary(
                location_grouped, "cs"
            )
            location_grouped = location_grouped.rename(columns={"plant": "location"})
            location_grouped["group_id"] = location_grouped["sap_id"]
            location_grouped = location_grouped[
                ["month_date", "location", "group_id", "zone", "rejection_percent"]
            ]
            location_grouped = location_grouped.rename(
                columns={"rejection_percent": "cs_rejection_percent"}
            )
            ...
            location_grouped = df.groupby(
                ["month_date", "sap_id", "plant", "zone"], as_index=False
            ).agg({"pt_handled": "sum", "pt_sortout": "sum"})
            location_grouped, location_summary = calculate_rejections_and_summary(
                location_grouped, "pt"
            )
            location_grouped = location_grouped.rename(columns={"plant": "location"})
            ...
            location_grouped = df.groupby(
                ["month_date", "sap_id", "plant", "zone"], as_index=False
            ).agg({"gd_handled": "sum", "gd_sortout": "sum"})
            location_grouped, location_summary = calculate_rejections_and_summary(
                location_grouped, "gd"
            )
            location_grouped = location_grouped.rename(columns={"plant": "location"})

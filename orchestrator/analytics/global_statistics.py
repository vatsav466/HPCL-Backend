...
async def get_global_bu_statistics(bus: list):
    ...
    # Execute the query
    query_resp = await hpcl_ceg_model.LocationMasterCreate.get_aggr_data(
        query, limit=1000
    )
    ...
    for record in query_resp[
        "data"
    ]:  # Loop through `query_resp['data']` instead of `query_resp`
        print(record)
        bu = record["bu"]
        count = record["count"]
        if bu not in resp:
    ...
    alert_resp = await hpcl_ceg_model.LocationMasterCreate.get_aggr_data(
        alert_query, limit=1000
    )
    ...
    for record in alert_resp["data"]:
        ...
        severity = record[
            "severity"
        ].lower()  # Assuming severity levels are 'critical', 'high', 'medium', 'low'
        count = record["count"]
        ...
        if bu in resp:
            ...
            if (
                severity in resp[bu]
            ):  # Check if severity level exists in the dictionary keys

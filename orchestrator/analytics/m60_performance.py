...
AllProducts = {
    ...
    "PETCHEM": [
        ...
        "PETCHEM",
        "Solvent 2445",
        "CRUDE- Reporting only",
    ],
}

HistoryKeyMapping = {
    "SBU_Name": '"ORGSBUNAME"',
    "Zone_Name": '"ORGZONENAME"',
    "Region_Name": '"ORGRONAME"',
...
DefaultTable = "Day"
MandateKeys = {
    "actual": "ACTUAL_TMT_SALES",
    "history": "ACTUAL_HISTORY_TMT_SALES",
    "target": "TARGET_TMT_SALES",
    "YTD": "YTD_TMT_SALES",
}


async def get_date_filters(
    start_date, end_date, resp_format="%Y-%m-%d", day_resp_format="%Y%m%d"
):
    """
    ...
    if DefaultTable == "Day":
        return [], [
            [
                dt_parser.parse(start_date).strftime(day_resp_format),
    ...
    if start_dt.month == end_dt.month or (end_dt.month - start_dt.month) == 1:
        ...
        day_filter_dates.append(
            [
                start_dt.strftime(day_resp_format),
        ...
        day_filter_dates.append(
            [
                end_dt.replace(day=1).strftime(day_resp_format),
                end_dt.strftime(day_resp_format),
            ]
        )
        dt = helpers.get_time_stamp_by_delta(
            end_dt, months=1, ascending=False, date_time_format=None
        )
        _, month_days = monthrange(dt.year, dt.month)
        dt = dt.replace(day=month_days)
        filter_dates.append(
            [
                helpers.get_time_stamp_by_delta(
                    start_dt, months=1, ascending=True, date_time_format=None
                ).strftime(resp_format),
...
def calculate_pro_rate(target_data, key, start_month=None, end_month=None):
    ...
    :return:
    """
    if not target_data or not all(
        isinstance(rec, dict) and "month_name" in rec for rec in target_data
    ):
...
async def collect_data(
    ...
    query = await widget_actions.WidgetActions.apply_filter_drilldown(
        query, access_filters, drilldown=""
    )
...
def get_group_by_filter_key(
    ...
    if cumulative:
        ...
        if len(cross_filters) == 1:
            ...
            if (
                len(cross_filters) > 1
                and cross_filters[0]["key"].strip('"') == "month_name"
            ):
                Base_Filters = [
                    '"month_name"',
                    '"SBU_Name"',
    ...
    if cross_filters:
        ...
        if (
            "Aviation" in [x["value"].strip('"') for x in cross_filters]
            or "PETCHEM" in [x["value"].strip('"') for x in cross_filters]
            or "GAS" in [x["value"].strip('"') for x in cross_filters]
        ):
            for key in [rec["key"] for rec in cross_filters]:
            ...
            for key in [rec["key"] for rec in cross_filters]:
                if (
                    key in Base_Filters or key in [x.strip('"') for x in Base_Filters]
                ) and Base_Filters.index(key) > index:
...
async def m60_performance(
    ...
    def get_fiscal_year(date_ui, todays_date, same_year=False, key="YTDPM"):
        ...
        end_date_ = fiscal_year.FiscalDate.fromtimestamp(
            int(urdhva_base.utilities.get_present_time().strftime("%s"))
        )
        if key == "YTDPM":
            if same_year:
            ...
            if same_year:
                ...
                end_date = helpers.get_time_stamp_by_delta(
                    dt_parser.parse(end_date_check),
                    years=1,
                    days=0,
                    with_month_start_day=False,
                    date_time_format=None,
        ...
        # For History
        start_date_history = (
            fiscal_year.FiscalYear.current().prev_fiscal_year.start.strftime(
                "%Y%m%d" if DefaultTable == "Day" else "%Y%m"
            )
        )
        if key == "YTDPM":
            if same_year:
                end_date_history = helpers.get_time_stamp_by_delta(
                    end_date_,
                    years=1,
                    days=1,
                    with_month_start_day=True,
                ...
                if isinstance(end_date_history, str):
                    end_date_history = datetime.datetime.strptime(
                        end_date_history, "%Y-%m-%d"
                    )
                print("end_date_history type", type(end_date_history))
                first_day_of_current_month_his = end_date_history.replace(day=1)
                last_day_of_prev_month_his = (
                    first_day_of_current_month_his - datetime.timedelta(days=1)
                )
                print("last_day_of_prev_month_his", last_day_of_prev_month_his)
                end_date_history = datetime.date(
                    last_day_of_prev_month_his.year,
                    last_day_of_prev_month_his.month,
                    last_day_of_prev_month_his.day,
                )
        ...
        end_date_history = end_date_history.replace(
            year=end_date_history.year
        ).strftime("%Y-%m-%d")

        start_date_history = datetime.datetime.strptime(start_date_history, "%Y%m%d")
        start_date_history = start_date_history.replace(
            year=start_date_history.year - 1
        ).strftime("%Y-%m-%d")
    ...
    if "ProductName" in [x["key"].strip('"') for x in filters]:
        product_name_values = [
            x["value"] for x in filters if x["key"].strip('"') == "ProductName"
        ]
        if product_name_values and product_name_values[0] == "":
            cross_filters = [
                f for f in cross_filters if f.get("key", "").strip('"') != "ProductName"
            ]
    """
    ...
    group_by_filter = get_group_by_filter_key(
        cross_filters,
        Base_Filters,
        resp_format_org,
        cumulative,
    ...
    history = actual = target = start_date = end_date = start_date_history = (
        end_date_history
    ) = ""
    if "fiscal_year" in [x["key"].strip('"') for x in filters]:
        fiscal_year_ui = [
            x["value"] for x in filters if x["key"].strip('"') == "fiscal_year"
        ][0]
        todays_date = str(datetime.date.today())
        if todays_date.split("-")[0] == fiscal_year_ui.split("-")[0]:
            if "YTD" in [x["key"].strip('"') for x in filters]:
                start_date, end_date, start_date_history, end_date_history = (
                    get_fiscal_year(
                        fiscal_year_ui, todays_date, same_year=True, key="YTD"
                    )
                )
            ...
            if "YTD" in [x["key"].strip('"') for x in filters]:
                ...
                start_date, end_date, start_date_history, end_date_history = (
                    get_fiscal_year(
                        fiscal_year_ui, todays_date, same_year=False, key="YTD"
                    )
                )
            else:
                # end_date = fiscal_year.FiscalYear.current().fiscal_year_end_date
                ...
                end_date = (
                    fiscal_year.FiscalYear.current().prev_fiscal_year.end.strftime(
                        "%Y%m%d"
                    )
                )

                start_date_history = helpers.get_time_stamp_by_delta(
                    dt_parser.parse(start_date),
                    years=1,
                    days=0,
                    with_month_start_day=False,
                    date_time_format=None,
        ...
        # For History
        start_date_history = (
            fiscal_year.FiscalYear.current().prev_fiscal_year.start.strftime("%Y%m%d")
        )
        end_date_history = helpers.get_time_stamp_by_delta(
            dt_parser.parse(end_date),
            years=1,
    ...
    cross_filters = [
        rec
        for rec in cross_filters
        if not (rec["key"] == "month_name" and not rec["value"].strip('"'))
    ]
    ...
    for filter in cross_filters:
        ...
        if (
            "cumulative" in [x["key"] for x in cross_filters]
            and len(cross_filters) == 1
        ):
            cross_filters = []
        else:
    ...
    filters_req = [
        condition["key"].strip('"')
        for condition in filters
        if condition["key"].strip('"') in ["A", "H", "T"]
    ]
    ...
    for condition in filters:
        if condition["key"].strip('"') == "A":
        ...
        elif condition["key"].strip('"') == "C" and '"T"' not in [
            x["key"] for x in filters
        ]:
            continue
            ...
            if "fiscal_year" in [x["key"].strip('"') for x in filters]:
                ...
                if todays_date.split("-")[0] == fiscal_year_ui.split("-")[0]:
                    ...
                    if (
                        todays_date.split("-")[1] == "04"
                        and todays_date.split("-")[-1] == "01"
                    ):
                        start_date, end_date, start_date_history, end_date_history = (
                            get_fiscal_year(
                                fiscal_year_ui, todays_date, same_year=True, key="YTD"
            ...
            if "fiscal_year" in [x["key"].strip('"') for x in filters]:
                ...
                if (
                    todays_date.split("-")[1] == "04"
                    or todays_date.split("-")[1] == "4"
                ) and todays_date.split("-")[0] == fiscal_year_ui.split("-")[0]:
                ...
                if todays_date.split("-")[0] == fiscal_year_ui.split("-")[0]:
                    start_date, end_date, start_date_history, end_date_history = (
                        get_fiscal_year(
                            fiscal_year_ui, todays_date, same_year=True, key="YTDPM"
                        )
                    )
                    ...
                    start_date, end_date, start_date_history, end_date_history = (
                        get_fiscal_year(
                            fiscal_year_ui, todays_date, same_year=False, key="YTDPM"
                        )
                    )
                else:
                    start_date, end_date, start_date_history, end_date_history = (
            ...
            if todays_date.split('-')[1] == '04' or todays_date.split('-')[1] == '4':
                ...
                start_date_history = start_date_history.replace(year=start_date_history.year-1).strftime("%Y-%m-%d")
            """
        elif condition["key"].strip('"') == "DATE" and '"FYC"' not in [
            x["key"] for x in filters
        ]:
            ...
            start_date_history = start_date_history.replace(
                year=start_date_history.year - 1
            ).strftime("%Y%m%d" if DefaultTable == "Day" else "%Y%m")
            # For History
            end_date_history = dt_parser.parse(end_date)
            end_date_history = end_date_history.replace(
                year=end_date_history.year - 1
            ).strftime("%Y%m%d" if DefaultTable == "Day" else "%Y%m")
            ...
            if condition["key"] != '"C"':
                ...
                if condition["key"] == "month_name":
                    value = [
                        mnt_name.strip() for mnt_name in condition["value"].split(",")
                    ]
                    ...
                    if condition["key"] == "DATE":
                        ...
                        if "fiscal_year" in condition["key"].strip('"'):
                            ...
                            if "YTD" in [
                                x["key"].strip('"') for x in filters
                            ] or "YTDPM" in [x["key"].strip('"') for x in filters]:
                                continue
                        else:
                            cross_filters.append(condition)
    ...
    where_conditions = []
    clause = await widget_actions.WidgetActions.generate_filter_clause(
        cross_filters.copy()
    )
    if clause:
    ...
    if target:
        ...
        if '"month_name"' not in group_by_filter and '"C"' not in [
            x["key"] for x in filters
        ]:
            group_keys.append("month_name")
        if '"C"' not in [x["key"] for x in filters]:
            target_data = await collect_data(
                [target, "month_name"],
                "M60_LEVEL_METADATA",
                where_conditions + Default_Filters,
                start_date,
                end_date,
                group_keys,
            )
        elif (
            '"C"' in [x["key"] for x in filters]
            and '"YTD"' in [x["key"] for x in filters]
            and '"T"' in [x["key"] for x in filters]
            and (
                len(org_cross_filters) == 0
                or (
                    len(org_cross_filters) == 1
                    and org_cross_filters[0]["key"] == '"sbu_wise"'
                )
            )
        ):
            group_keys.append("month_name")
            ...
            '"C"' in [x["key"] for x in filters]
            and '"YTD"' not in [x["key"] for x in filters]
            and '"T"' in [x["key"] for x in filters]
            and (
                len(org_cross_filters) == 0
                or (
        ...
        if target_data:
            ...
            if (
                '"YTD"' in [x["key"] for x in filters]
                and '"T"' in [x["key"] for x in filters]
                and (len(org_cross_filters) == 0)
                and '"C"' in [x["key"] for x in filters]
                and time_grain != "Monthly"
            ) or (
                '"DATE"' in [x["key"] for x in filters]
                and '"T"' in [x["key"] for x in filters]
            ):
                ...
                target_data = pd.DataFrame(
                    calculate_pro_rate(
                        target_data, "TARGET_TMT_SALES", start_date, end_date
                    )
                )
            ...
            #         return {"status":False,"data":'No Data Present For Current Selection'}
                ...
                '"YTD"' in [x["key"] for x in filters]
                and "month_name" in target_data[0]
            ):
                target_data = pd.DataFrame(
                    calculate_pro_rate(
                        target_data, "TARGET_TMT_SALES", start_date, end_date
            ...
            if group_by_filter and not cumulative:
                target_data = (
                    target_data.groupby(get_group_by_columns(group_by_filter))[
                        "TARGET_TMT_SALES"
                    ]
                    .sum()
                    .reset_index()
                ...
                if not cumulative:
                    target_data = pd.DataFrame(
                        [{"TARGET_TMT_SALES": target_data.sum()["TARGET_TMT_SALES"]}]
                    )
            if '"month_name"' in group_by_filter and "month_name" in target_data:
                target_data["month_name"] = pd.CategoricalIndex(
                    target_data["month_name"], ordered=True, categories=months
                )
            ...
            if "ProductName" in target_data.columns.tolist():
                if "HFHSD" in target_data["ProductName"].unique().tolist():
                    if (
                        int(
                            target_data[target_data["ProductName"] == "HFHSD"][
                                "TARGET_TMT_SALES"
                            ]
                            .unique()
                            .tolist()[0]
            ...
            if "C" in [x["key"].strip('"') for x in filters]:
                ...
                if "month_name" in target_data.columns.tolist():
                    ...
                    if "month_name" not in [x.strip('"') for x in group_by_filter]:
                        if existing_columns:
                            target_data = (
                                target_data.groupby(existing_columns, as_index=False)[
                                    "TARGET_TMT_SALES"
                                ]
                                .sum()
                                .reset_index()
            ...
            if (
                "C" in [x["key"].strip('"') for x in filters]
                and "month_name" not in target_data.columns.tolist()
                and "DATE" in [x["key"].strip('"') for x in filters]
            ):
                if (
                    "TARGET_TMT_SALES" in target_data.columns.tolist()
                    and "ProductName" in target_data.columns.tolist()
                ):
                    target_data = (
                        target_data.groupby("ProductName", as_index=False)[
                            "TARGET_TMT_SALES"
                        ]
                        .sum()
                        .reset_index()
    ...
    if actual:
        ...
        day_filter_dates = []
        if start_date and end_date:
            filter_dates, day_filter_dates = await get_date_filters(
                start_date, end_date
            )
        else:
            if DefaultTable == "Day":
        ...
        if actual_data:
            ...
            if (
                isinstance(actual_data, list)
                and len(actual_data) > 0
                and actual_data[0] == "data"
            ):
                actual_data = []
            else:
                ...
                if group_by_filter:
                    actual_data = (
                        actual_data.groupby(get_group_by_columns(group_by_filter))[
                            "ACTUAL_TMT_SALES"
                        ]
                        .sum()
                        .reset_index()
                    ...
                    actual_data = pd.DataFrame(
                        [{"ACTUAL_TMT_SALES": actual_data.sum()["ACTUAL_TMT_SALES"]}]
                    )
                actual_data["ACTUAL_TMT_SALES"] = actual_data[
                    "ACTUAL_TMT_SALES"
                ].fillna(0)
    ...
    if history:
        ...
        if start_date and end_date:
            filter_dates, day_filter_dates = await get_date_filters(
                start_date_history, end_date_history, "%Y%m"
            )
        else:
            if DefaultTable == "Day":
        ...
        for date_range in filter_dates:
            hist_data.extend(
                await collect_data(
                    [history],
                    "MOM_DAY_LEVEL_DATA",
                    where_conditions_history + Default_Filters,
                    date_range[0],
                    date_range[1],
                    group_by_filter,
                    '"YEARMONTH"',
        ...
        if hist_data:
            ...
            if group_by_filter:
                hist_data = (
                    hist_data.groupby(get_group_by_columns(group_by_filter))[
                        "ACTUAL_HISTORY_TMT_SALES"
                    ]
                    .sum()
                    .reset_index()
                ...
                hist_data = pd.DataFrame(
                    [
                        {
                            "ACTUAL_HISTORY_TMT_SALES": hist_data.sum()[
                                "ACTUAL_HISTORY_TMT_SALES"
                            ]
                        }
                    ]
                )
            hist_data["ACTUAL_HISTORY_TMT_SALES"] = hist_data[
                "ACTUAL_HISTORY_TMT_SALES"
            ].fillna(0)
    ...
    if target_data:
                ...
                if not tgt_result:
                    ...
                    if time_grain == "top_sales_area":
                        tgt_result = [
                            {
                                f"{matched_level}": k,
                                "TARGET_TMT_SALES": round(v, 2),
                                "SalesArea_Name": area_map.get(k),
                            }
                            for k, v in summed_data.items()
                        ]
                    if time_grain == "top_regions":
                        tgt_result = [
                            {
        ...
        if (
            all(k not in target_data[0] for k in non_month_keys)
            and "month_name" in target_data[0]
            and len(tgt_result) == 0
        ):
            tgt_result = target_data
    ...
    if len(df_) > 1:
        for df in df_[1:]:
            if group_by_filter:
                merged_df = pd.merge(
                    merged_df, df, on=get_group_by_columns(group_by_filter), how="outer"
                )  # Outer merge with df2
            else:
    ...
    if not merged_df.empty and (
        '"month_name"' in group_by_filter or '"SBU_Name"' in group_by_filter
    ):
        ...
        merged_df["data_order"] = merged_df[order_key].map(
            {cond: i for i, cond in enumerate(sort_key)}
        )
    ...
    if "ProductName" in merged_df.columns.tolist():
        if any(
            product in merged_df["ProductName"].unique().tolist()
            ...
            for product in ["Lubes", "DEF"]:
                if product in merged_df["ProductName"].unique().tolist():
                    if (
                        int(
                            merged_df[merged_df["ProductName"] == product][
                                "ACTUAL_TMT_SALES"
                            ]
                            .unique()
                            .tolist()[0]
    ...
    if time_grain == "fiscal_year":
        result = await get_sbu_sales_fiscal(
            merged_df, filters, cross_filters, drill_state, time_grain, resp_format
        )
        if result["status"]:
            return result
    ...
    if time_grain == "top_zones":
        ...
        result = await get_top_and_bottom_3_zones_by_year_sbu(
            merged_df, filters, cross_filters, drill_state, time_grain, resp_format
        )
        print("RESULT", result)
        if result["status"]:
    ...
    if time_grain == "top_regions":
        ...
        result = await get_top_and_bottom_7_regions_by_year_sbu(
            merged_df, filters, cross_filters, drill_state, time_grain, resp_format
        )
        print("RESULT", result)
        if result["status"]:
    ...
    if time_grain == "top_sales_area":
        ...
        result = await get_top_and_bottom_10_sales_area_by_year_sbu(
            merged_df, filters, cross_filters, drill_state, time_grain, resp_format
        )
        print("RESULT", result)
        if result["status"]:
    ...
    if (
        len(group_by_filter) <= 1
        and len(where_conditions) == 1
        and "month_name" in where_conditions[0]
        and "IN" in where_conditions[0]
        and "month_name" in merged_df.columns
    ):
    ...
    if len(cross_filters) > 0:
        ...
        sorted_cross_filters = sorted(
            cross_filters,
            key=lambda x: (
                filter_order.index(x["key"])
                if x["key"] in filter_order
                else float("inf")
            ),
        )
        ...
        Charts_Get_Distinct_ValuesParams.connection_id = (
            connection_mapping.connection_mapping.get("hpcl_ceg", "1")
        )
        ...
        for each_filter in sorted_cross_filters:
            ...
            if sorted_cross_filters[-1]["key"] not in sorted_level:
                ...
                sorted_level[sorted_cross_filters[-1]["key"]].append(
                    sorted_cross_filters[-1]["value"]
                )
        month_keys = []
        ...
        if sorted_level and sorted_level.get("month_name") and not cumulative:
            ...
            month_keys = sorted_level["month_name"] = sorted(
                sorted_level["month_name"], key=months.index
            )
        if len(group_by_filter) > 1:
            """
            ...
            """
            final_resp, hist_growth_details, tgt_growth_details = generate_stacked_data(
                drill_state, merged_df, resp_format, month_column="month_name"
            )
        else:
            if not resp_format:
                final_resp = {
                    key: value.to_dict()
                    for key, value in merged_df.to_dict(orient="series").items()
                }
            else:
                final_resp, hist_growth_details, tgt_growth_details = (
        ...
        if (
            isinstance(final_resp, dict)
            and len(final_resp.get("ACTUAL_TMT_SALES", [])) == 1
            and not drill_state
        ):
            ...
            if sbuWise:
                final_resp["cumulative"]["0"] = (
                    sbuName_req.upper() + "_CUMMULATIVE_SALES"
                )
            else:
        ...
        if resp_format == "heat_map":
            ...
            hist_xaxis.extend(
                [
                    "_".join(x["title"].split("_")[:2])
                    for x in hist_growth_details
                    if "hist" in x["title"].lower()
                ]
            )
            li = None
            if len(hist_xaxis) > 1:
            ...
            if '"T"' in [x["key"] for x in filters]:
                tgt_xaxis.extend(
                    [
                        "_".join(x["title"].split("_")[:2])
                        for x in tgt_growth_details
                        if "tgt" in x["title"].lower()
                    ]
                )
                if li:
                    if len(tgt_xaxis) != 2:
            ...
            if len(tgt_xaxis) > 0:
                return {
                    ...
                    "data": {
                        "data": final_resp,
                        "hist_growth_details": hist_growth_details,
                        "tgt_growth_details": tgt_growth_details,
                        "hist_xaxis": hist_xaxis,
                        "tgt_xaxis": tgt_xaxis,
                        "level": sorted_level,
                        "month_name": month_keys,
                        "sales_unit": measure_unit,
                    },
                }
            else:
                return {
        ...
        if isinstance(final_resp, dict):
            if all(not v for v in final_resp.values()):
                ...
                if len(cross_filters) == 1 or len(cross_filters) > 1:
                    ...
                    # print("cross_filters", cross_filters)
                    if (
                        list(set([x["key"] for x in cross_filters]))[0].strip('"')
                        == "SBU_Name"
                        and cross_filters[0]["value"] != ""
                        and resp_format != "stacked"
                    ):
                        # print("insied 2nd if")
                        if (
                            len(cross_filters) == 1
                        ...
                        if (
                            len(cross_filters) > 1
                            and cross_filters[1]["key"].strip('"') != "month_name"
                        ):
                        ...
                        if condition["key"].strip('"') == "SBU_Name":
                            ...
                            if condition["value"] == "I&C":
                                if final_resp.get("ProductName", {}):
                                    ...
                                    indices_to_keep = [
                                        k
                                        for k, v in final_resp["ProductName"].items()
                            ...
                            if (
                                '"ProductName"' in group_by_filter
                                and condition["value"] in productOrders
                            ):
                                sort_order = productOrders[condition["value"]]
                                all_products = AllProducts.get(
                                    condition["value"], sort_order
                                )
                                ...
                                for key in all_products:
                                    if key not in list(
                                        final_resp.get("ProductName", {}).values()
                                    ):
                                        order_num = list(
                                            final_resp["ProductName"].keys()
                                        )
                                        order_num.sort()
                                ...
                                for order_num in list(final_resp["ProductName"].keys()):
                                    if (
                                        final_resp["ProductName"][order_num]
                                        in cleanup_products
                                    ):
                                        for key_ in final_resp:
                                ...
                                if sbu_name == "Retail":
                                    ...
                                    df.loc[
                                        df["ProductName"].isin(Finished_Lubes_Retail),
                                        "ProductName",
                                    ] = "Finished_Lubes_Retail"
                                    df.loc[
                                        df["ProductName"].isin(
                                            Finished_Lubes_Distributor
                                        ),
                                        "ProductName",
                                    ] = "Finished_Lubes_Distributor"
                                    ...
                                    df.loc[
                                        df["ProductName"].isin(Finished_Lubes_Consumer),
                                        "ProductName",
                                    ] = "Finished_Lubes_Consumer"
                                    df.loc[
                                        df["ProductName"].isin(Finished_Lubes),
                                        "ProductName",
                                    ] = "Finished_Lubes"

                                    # Ensure numeric conversion
                                df["ACTUAL_TMT_SALES"] = (
                                    df["ACTUAL_TMT_SALES"].fillna(0).astype(float)
                                )
                                df["ACTUAL_HISTORY_TMT_SALES"] = (
                                    df["ACTUAL_HISTORY_TMT_SALES"]
                                    .fillna(0)
                                    .astype(float)
                                )
        ...
        if isinstance(final_resp, list):
            if len(final_resp) == 0:
                return {
                    "status": False,
                    "message": "Data Not Present for the current selection",
                    "data": {
                        "data": final_resp,
                        "level": sorted_level,
                        "month_name": month_keys,
...
def generate_stacked_data(drill_state, df, resp_format="", month_column=""):
    ...
    if month_column:
        df[month_column] = pd.Categorical(
            df[month_column], categories=months, ordered=True
        )
    ...
    if other_columns:
        ...
        if not resp_format:
            ...
            df.rename(
                columns={
                    value: key
                    for key, value in MandateKeys.items()
            ...
            numeric_cols = [
                key for key, value in MandateKeys.items() if value in numeric_cols
            ]

            # Pivot Data - Creating separate columns for Actual, History, and Target
            df_pivot = df.pivot(
                index=other_columns[0], columns=month_column, values=numeric_cols
            )

            # Flatten MultiIndex Columns and rename them
            df_pivot.columns = [
                f"{month}_{metric.split('_')[0]}" for metric, month in df_pivot.columns
            ]
            ...
            unique_months = sorted(
                df[month_column].unique(), key=lambda x: months.index(x)
            )
            series_data = []
            for zone in df[other_columns[0]].unique():
                ...
                for col in zone_data.columns:
                    if (
                        zone_data[col].dtype == "int64"
                        or zone_data[col].dtype == "np.int64"
                    ):
                ...
                for column in numeric_cols:
                    ...
                    series_data.append(
                        {
                            "name": f"{zone} {column.title()}",
                            "stack": column.title(),
                            "data": [
                                zone_data.loc[m, column] if m in zone_data.index else 0
                                for m in unique_months
                            ],
                        }
                    )
            return (
                {"months": unique_months, "series": series_data},
                hist_growth_details,
                tgt_growth_details,
            )
            ...
            if drill_state.strip('"') == "SBU_Name":
                ...
                if resp_format == "heat_map":
                    cumulative_data = (
                        df[df["month_name"].isin(cumulative_months)]
                        .groupby(["Zone_Name"], as_index=False)[sum_cols]
                        .sum()
                    )
                    # .reset_index(drop=True)
                    ...
                    if non_cumulative_months:
                        non_cumulative_data = df[
                            df["month_name"].isin(non_cumulative_months)
                        ]
                    ...
                    cumulative_data = (
                        df[df["month_name"].isin(cumulative_months)]
                        .groupby("ProductName", as_index=False)[sum_cols]
                        .sum()
                    )
            if drill_state.strip('"') == "Zone_Name":
                ...
                cumulative_data = (
                    df[df["month_name"].isin(cumulative_months)]
                    .groupby("Region_Name", as_index=False)[sum_cols]
                    .sum()
                )
            if drill_state.strip('"') == "Region_Name":
                cumulative_data = (
                    df[df["month_name"].isin(cumulative_months)]
                    .groupby("SalesArea_Name", as_index=False)[sum_cols]
                    .sum()
                )
            ...
            if "month_name" in df.columns.tolist():
                ...
                for i in df["month_name"].unique().tolist():
                    ...
                    if len(df["month_name"].unique().tolist()) > 1:
                        ...
                        df_prev = df[
                            df["month_name"].isin(
                                df["month_name"].unique().tolist()[:-1]
                            )
                        ]
                    if len(df["month_name"].unique().tolist()) >= 3:
                        df_pres = df[
                            df["month_name"] == df["month_name"].unique().tolist()[-1]
                        ]
                    # df_prev = df[df['month_name'] == df['month_name'].unique().tolist()[1]]
                    # df_pres = df[df['month_name'] == df['month_name'].unique().tolist()[-1]]
                if (
                    len(df["month_name"].unique().tolist()) == 1
                    and df["month_name"].unique().tolist()[0] == "Apr"
                ):
                    df_pres = df
                ...
                for idx, df_month in enumerate(df_list):
                    if "ACTUAL_TMT_SALES" in df_month.columns.tolist():
                        df_month["ACTUAL_TMT_SALES"] = (
                            df_month["ACTUAL_TMT_SALES"].fillna(0).astype(float)
                        )
                        curr_value = df_month["ACTUAL_TMT_SALES"].sum()
                    if "ACTUAL_HISTORY_TMT_SALES" in df_month.columns.tolist():
                        df_month["ACTUAL_HISTORY_TMT_SALES"] = (
                            df_month["ACTUAL_HISTORY_TMT_SALES"].fillna(0).astype(float)
                        )
                        prev_value = df_month["ACTUAL_HISTORY_TMT_SALES"].sum()
                    if "TARGET_TMT_SALES" in df_month.columns.tolist():
                        df_month["TARGET_TMT_SALES"] = (
                            df_month["TARGET_TMT_SALES"].fillna(0).astype(float)
                        )
                    ...
                    if curr_value or prev_value:
                        ...
                        if "TARGET_TMT_SALES" in df_month.columns.tolist():
                            if tgt_value != 0:
                                tgt_growth = (
                                    (curr_value - tgt_value) / tgt_value
                                ) * 100
                            else:
                                tgt_growth = 100
                        ...
                        if idx == 0:
                            if len(df_list) != 1:
                                hist_growth_details.append(
                                    {"title": "Cum_Hist_Growth", "value": cum_growth}
                                )
                                if "TARGET_TMT_SALES" in df_month.columns.tolist():
                                    tgt_growth_details.append(
                                        {"title": "Cum_Tgt_Growth", "value": tgt_growth}
                                    )
                            if len(df_list) == 1:
                                hist_growth_details.append(
                                    {"title": "Apr_Hist_Growth", "value": cum_growth}
                                )
                                if "TARGET_TMT_SALES" in df_month.columns.tolist():
                                    tgt_growth_details.append(
                                        {"title": "Apr_Tgt_Growth", "value": tgt_growth}
                                    )

                        if idx == 1:
                            hist_growth_details.append(
                                {
                                    "title": f"{df['month_name'].unique().tolist()[1]}_Hist_Growth",
                                    "value": cum_growth,
                                }
                            )
                            if "TARGET_TMT_SALES" in df_month.columns.tolist():
            ...
            df_pivot.columns = [
                f"{month}_{metric.split(',')[0]}" for metric, month in df_pivot.columns
            ]
            # Reset Index to include 'Zone_Name'
            df_pivot.reset_index(inplace=True)
            # Keeping all nan's as zero's
            ...
            if (
                "YTD_actual" in df_pivot.columns.tolist()
                or "YTD_history" in df_pivot.columns.tolist()
            ):
                if df_pivot["YTD_history"].sum() != 0:
                    ytd_hist_growth = (
                        (df_pivot["YTD_actual"].sum() - df_pivot["YTD_history"].sum())
                        / df_pivot["YTD_history"].sum()
                    ) * 100
                ...
                if "YTD_target" in df_pivot.columns.tolist():
                    if df_pivot["YTD_target"].sum() != 0:
                        ytd_tgt_growth = (
                            (
                                df_pivot["YTD_actual"].sum()
                                - df_pivot["YTD_target"].sum()
                            )
                            / df_pivot["YTD_target"].sum()
                        ) * 100
                ...
                hist_growth_details.append(
                    {"title": "YTD_Hist_Growth", "value": ytd_hist_growth}
                )
                if "YTD_target" in df_pivot.columns.tolist():
                    tgt_growth_details.append(
                        {"title": "YTD_Tgt_Growth", "value": ytd_tgt_growth}
                    )
                    # Convert to Dictionary Format
            return (
                df_pivot.to_dict(orient="records"),
            ...
            return (
                {
                    key: value.to_dict()
                    for key, value in df.to_dict(orient="series").items()
                },
                hist_growth_details,
                tgt_growth_details,
            )
                ...
                [
                    {
                        "month_name": month,
                        **{
                            key: group[key].tolist()
                            for key in numeric_cols + other_columns
                        },
                    }
...
async def get_top_and_bottom_3_zones_by_year_sbu(
    merged_df, filters, cross_filters, drill_state="", time_grain="", resp_format=""
):
    try:
        import pandas as pd
            ...
            if isinstance(filters, list):
                return next(
                    (
                        f.get("value")
                        for f in filters
                        if f.get("field") == field_name or f.get("key") == field_name
                    ),
                    None,
                )
        ...
        sbu_filter = get_filter_value(filters, "sbu") or get_filter_value(
            filters, "SBU_Name"
        )
        ...
        if "Zone_Name" in df.columns:
            ...
            df = df[
                df["Zone_Name"].notna()
                & (df["Zone_Name"].str.strip() != "")
                & (df["Zone_Name"] != "-")
            ]

            # Group and aggregate
            zone_sales = (
                df.groupby(["Zone_Name"])["ACTUAL_TMT_SALES"].sum().reset_index()
            )

            # Sort and get top/bottom 3
            top_3_zones = zone_sales.sort_values(
                "ACTUAL_TMT_SALES", ascending=False
            ).head(3)
            bottom_3_zones = zone_sales.sort_values(
                "ACTUAL_TMT_SALES", ascending=True
            ).head(3)
...
async def get_top_and_bottom_7_regions_by_year_sbu(
    merged_df, filters, cross_filters, drill_state="", time_grain="", resp_format=""
):
    try:
        import pandas as pd
        ...
        if "Region_Name" in df.columns:
            ...
            # Group and aggregate
            region_sales = (
                df.groupby(["Region_Name"])["ACTUAL_TMT_SALES"].sum().reset_index()
            )
            ...
            bottom_regions = region_sales.sort_values(
                "ACTUAL_TMT_SALES", ascending=True
            ).head(num_top_bottom)
...
async def get_top_and_bottom_10_sales_area_by_year_sbu(
    merged_df, filters, cross_filters, drill_state="", time_grain="", resp_format=""
):
    ...
    try:
        ...
        if "SalesArea_Name" in df.columns:
            ...
            df = df[
                df["SalesArea_Name"].notna()
                & (df["SalesArea_Name"].str.strip() != "")
                & (df["SalesArea_Name"] != "-")
            ]
            print("df columns", df.columns)
            # Group and aggregate
            sales_area_sales = (
                df.groupby(["SalesArea_Name"])["ACTUAL_TMT_SALES"].sum().reset_index()
            )

            # Sort and get top/bottom
            top_sales = sales_area_sales.sort_values(
                "ACTUAL_TMT_SALES", ascending=False
            ).head(num_top_bottom)
            bottom_sales_area = sales_area_sales.sort_values(
                "ACTUAL_TMT_SALES", ascending=True
            ).head(num_top_bottom)
    ...
    for idx, months in enumerate(SETS):
        ...
        for row in rows:
            ...
            if sbu is None or sbu.strip() == "" or sbu == "0":
                continue
            set_data.append(
                {"sbu": sbu, "total_netweight": float(row["total_netweight"])}
            )
        results.append({"months": months, "data": set_data})
...
def filter_and_map_sales_area(
    ...
    merged_df = results_df.merge(
        mapping_df, how="left", left_on="icSalesArea", right_on="IC Sales Area Name"
    )
    ...
    additional_df = pd.read_excel(
        second_excel_path, sheet_name=second_sheet_name, header=1
    )
    ...
    matched_df["icSalesArea"] = matched_df["icSalesArea"].apply(
        lambda x: " ".join(x.split()) if "-" not in x else x
    )
    additional_df["IC Sales Area"] = additional_df["IC Sales Area"].str.strip()

    additional_df["IC Sales Area"] = (
        additional_df["IC Sales Area"]
        .fillna("")
        .astype(str)
        .apply(lambda x: x.replace("S/A", "DS SA"))
    )
    additional_df["IC Sales Area"] = additional_df["IC Sales Area"].str.upper()
    additional_df["IC Sales Area"] = additional_df["IC Sales Area"].apply(
        lambda x: " ".join(x.split()) if "-" not in x else x
    )
    ...
    matched_df["icSalesArea"].tolist()
    additional_df["IC Sales Area"].tolist()
    ...
    master_data_df = pd.read_excel(
        second_excel_path, sheet_name=second_sheet_name_second
    )
...
async def top_ic(filters, cross_filters, drill_state, time_grain, resp_formatt):
    ...
    try:
        ...
        cur_start, cur_end, hist_start, hist_end = get_day_id_ranges(
            required_year, required_month, filters
        )
        ...
        for col in ["Zone_Name", "Region_Name", "SalesArea_Name"]:
            for f in filters:
                ...
                if key == col:
                    if cond == "in":
                        val_list = [
                            v.strip() for v in value.split(",") if v not in ["", "All"]
                        ]
                        if val_list:
        ...
        fiscal_year_str = (
            f"FY {required_year}"
            if not required_year.startswith("FY")
            else required_year
        )
        target_query = f"""
            SELECT
        ...
        target_rows = await urdhva_base.BasePostgresModel.get_aggr_data(
            target_query, limit=0, skip=0
        )
        ...
        cumulative_ranges = [
            get_day_id_ranges(required_year, m, filters) for m in cumulative_months
        ]

        cur_conditions = " ".join(
            [
                f"WHEN \"DAY_ID\" BETWEEN '{start}' AND '{end}' THEN \"NETWEIGHT_TMT\""
                for start, end, _, _ in cumulative_ranges
            ]
        )
        his_conditions = " ".join(
            [
                f"WHEN \"DAY_ID\" BETWEEN '{hist_start}' AND '{hist_end}' THEN \"NETWEIGHT_TMT\""
                for _, _, hist_start, hist_end in cumulative_ranges
            ]
        )
        ...
        cumulative_result = await urdhva_base.BasePostgresModel.get_aggr_data(
            cumulative_query, limit=0, skip=0
        )
        ...
        if cumulative_months:
            ...
            cumulative_target_rows = await urdhva_base.BasePostgresModel.get_aggr_data(
                cumulative_target_query, limit=0, skip=0
            )
        ...
        for idx, row in enumerate(data, start=1):
            ...
            monthly_diff_value = (
                round(((cur_sales - his_sales) / his_sales) * 100, 2)
                if his_sales != 0
                else None
            )
            ...
            monthly_target_diff = (
                round((cur_sales / target) * 100, 2) if target != 0 else None
            )
            ...
            cumulative_diff_value = (
                round(((cum_cur - cum_his) / cum_his) * 100, 2)
                if cum_his != 0
                else None
            )
            cumulative_target_diff = (
                round((cum_cur / cum_target) * 100, 2) if cum_target != 0 else None
            )
        ...
        if results:
            ...
            results = results.merge(
                enriched_df[["icSalesArea", "Name_x"]],
                how="left",
                left_on="icSalesArea",
                right_on="icSalesArea",
            )
            ...
            results["icSalesArea"] = (
                results["icSalesArea"].astype(str).str.strip().str.upper()
            )
            ...
            results["month_name"] = results["monthly"].apply(
                lambda x: x.get("month_name", "").strip()
            )
            results["month_column"] = results["month_name"].str.upper()
            # print("Converted month_column to uppercase:\n", results["month_column"].unique())

            monitor_df_clean = pd.read_excel(
                "/home/novex/Copy_IC_SA.xlsx",
                sheet_name="SA_Wise_Monthly_Targets",
                skiprows=1,
            )
            ...
            def get_updated_month_value(row):
                ...
                if not match.empty and month_col in match.columns:
                    ...
                    if new_target != 0:
                        monthly_data["target_achieved"] = round(
                            (cur / new_target) * 100, 2
                        )
                ...
                if not match.empty:
                    ...
                    if selected_month in month_order:
                        ...
                        try:
                            new_cum_target = sum(
                                (
                                    float(match.iloc[0][m])
                                    if m in match.columns
                                    and pd.notnull(match.iloc[0][m])
                                    else 0.0
                                )
                        ...
                        if new_cum_target != 0:
                            cumulative_data["target_achieved"] = round(
                                (cum_cur / new_cum_target) * 100, 2
                            )
            ...
            results = results.astype(
                {col: str for col in results.select_dtypes(include="category").columns}
            )
            results = results.fillna("")

...
def generate_group_by_conditions(
    filters,
    cross_filters,
    cumulative=False,
    drill_state="",
...
async def get_zones_and_regions(
    ...
    try:
        ...
        def build_query(fiscal_year, group_by_col):
            ...
            valid_clause = (
                f"AND {group_by_col} IS NOT NULL AND TRIM({group_by_col}) <> '' AND TRIM({group_by_col}) <> '-'"
        ...
        query_tasks = [
            ...
            function(
                query=f"SELECT ROUND(COALESCE(SUM(netweight_tmt) / 1000, 0), 2) AS total_sales FROM industry_performance WHERE {base_where} {product_sql} AND fiscal_year = '{curr_year}'"
            ),
            function(
                query=f"SELECT ROUND(COALESCE(SUM(netweight_tmt) / 1000, 0), 2) AS total_sales FROM industry_performance WHERE {base_where} {product_sql} AND fiscal_year = '{his_year}'"
            ),
        ]
        ...
        (
            ...
            districts_curr_resp,
            districts_his_resp,
            grand_total_curr_resp,
            grand_total_his_resp,
        ) = await asyncio.gather(*query_tasks)

        grand_total_curr = (
            float(grand_total_curr_resp[0]["total_sales"])
            if grand_total_curr_resp and grand_total_curr_resp[0]
            else 0
        )
        grand_total_his = (
            float(grand_total_his_resp[0]["total_sales"])
            if grand_total_his_resp and grand_total_his_resp[0]
            else 0
        )

        # 5. Process and merge data
        def process_data(
            curr_data, his_data, name_col, grand_total_curr, grand_total_his
        ):
            curr_df = (
                pd.DataFrame(curr_data)
                if curr_data
                else pd.DataFrame(columns=[name_col, "total_sales"])
            )
            his_df = (
                pd.DataFrame(his_data)
                if his_data
                else pd.DataFrame(columns=[name_col, "total_sales"])
            )

            merged_df = pd.merge(
                curr_df, his_df, on=name_col, how="outer", suffixes=("_curr", "_his")
            ).fillna(0)
            ...
            merged_df["curr_mkt"] = (
                (merged_df["total_sales_curr"] / grand_total_curr * 100).round(2)
                if grand_total_curr > 0
                else 0
            )
            merged_df["his_mkt"] = (
                (merged_df["total_sales_his"] / grand_total_his * 100).round(2)
                if grand_total_his > 0
                else 0
            )

            merged_df["gain_loss"] = (
                merged_df["curr_mkt"] - merged_df["his_mkt"]
            ).round(2)
            merged_df["growth"] = (
                (
                    (merged_df["total_sales_curr"] - merged_df["total_sales_his"])
                    / merged_df["total_sales_his"]
                    * 100
                )
        ...
        regions_merged = process_data(
            regions_curr_resp, regions_his_resp, "ro", grand_total_curr, grand_total_his
        )
        districts_merged = process_data(
            districts_curr_resp,
            districts_his_resp,
            "distname",
            grand_total_curr,
        ...
        def format_output(df, name_col):
            return df.rename(
                ...
                [name_col, "total_sales", "curr_mkt", "his_mkt", "growth", "gain_loss"]
            ].sort_values(
                by="gain_loss", ascending=sort_ascending
            )

        zones_output = format_output(zones_merged, "zone_name")
        regions_output = (
            format_output(regions_merged, "ro")
            .rename(columns={"ro": "region_name"})
            .head(10)
        )
        ...
        combined_df = pd.concat(
            [zones_output, regions_output, districts_output], axis=0, ignore_index=True
        )
        ...
        if not upload_status:
            raise HTTPException(
                status_code=500, detail=f"MinIO upload failed: {upload_response}"
            )
...
async def get_fiscal_sales(filters: list, cross_filters: list, resp_format: str):
    ...
    for f in filters:
        ...
        if isinstance(values, list) and cond == "in":
            values_str = ",".join(
                [f"'{str(v)}'" for v in values if v not in ["", None]]
            )
    ...
    month_totals_all_zones = (
        df_all.groupby(["fiscal_year", "month_name"])["total_sales"].sum().to_dict()
    )
    ...
    for year, year_df in df.groupby("fiscal_year"):
        ...
        for month in ALL_MONTHS:
            ...
            month_sales = (
                float(month_df["total_sales"].sum()) if not month_df.empty else 0.0
            )
            month_market_share_percentage = (
                (month_sales / total_sales * 100) if total_sales > 0 else 0.0
            )

            float(month_totals_all_zones.get((year, month), 0.0))
            ...
            zones_list = []
            month_all_zones_df = df_all[
                (df_all["fiscal_year"] == year) & (df_all["month_name"] == month)
            ]
            ...
            for zone, zone_df in month_df.groupby("zone_name"):
                ...
                zone_market_share_percentage = (
                    ...
                    if month_total_sales_all_zones > 0
                    else 0.0
                )
                ...
                zones_list.append(
                    {
                        ...
                        "market_share_percentage": round(
                            zone_market_share_percentage, 2
                        ),
                    }
                )
            ...
            month_diff = (
                month_sales - prev_month_sales if prev_month_sales is not None else 0.0
            )
        ...
        year_diff = (
            total_sales - prev_year_total if prev_year_total is not None else 0.0
        )
...
def get_date_filters(filters, months_list=None, cumulative=None, resp_type="months"):
    ...
    end_date_history = helpers.get_time_stamp_by_delta(
        dt_parser.parse(end_date), years=1, days=0, with_month_start_day=False
    )
    months = []
    for condition in filters:
        if condition["key"].strip('"') == "A":
            fiscal_year_pre = (
                f"{fiscal_year.FiscalYear.current().start.year}-"
                f"{fiscal_year.FiscalYear.current().end.year}"
            )
        elif condition["key"].strip('"') == "H":
            fiscal_year_last = (
                f"{fiscal_year.FiscalYear.current().prev_fiscal_year.start.year}-"
            ...
            end_date = helpers.get_time_stamp_by_delta(
                months=1, with_month_start_day=False, with_month_end_day=True
            )
            end_date_history = helpers.get_time_stamp_by_delta(
                dt_parser.parse(end_date), years=1, days=0, with_month_start_day=False
            )
            ...
            end_date_history = end_date_history.replace(
                day=end_date_history.day - 1, year=end_date_history.year - 1
            )
            end_date_history = helpers.get_time_stamp_by_delta(
                end_date_history, with_month_start_day=False, with_month_end_day=True
            )
            ...
            months = [
                mnt_name.strip()
                for mnt_name in condition["value"].split(",")
                if mnt_name.strip()
            ]
    ...
    filters = [
        condition
        for condition in filters
        if condition["key"].strip('"') not in discard_filters
    ]
    print("months", months)
    if not months:
        months = (
    ...
    if "table_month" in [x["key"].strip('"') for x in org_filters]:
        ...
        if not cumulative:
            months = [
                x["value"] for x in org_filters if x["key"].strip('"') == "table_month"
            ]
...
def calculate_market_share(
    ...
    unique_companies = (
        list(df["coname"].unique()) if resp_format == "company_level" else ["HPCL"]
    )
    summary = total_sales
    ...
    if resp_format == "company_level":
        for company in unique_companies:
            ...
            company_share = (
                df[df["coname"] == company]
                .groupby(group_by)["sales"]
                .sum()
                .reset_index()
            )
            company_share.rename(
                columns={"sales": f"{company.lower()}_share"}, inplace=True
            )
            # Merge results
        ...
        hpcl_sales_per_year = (
            df[df["coname"] == "HPCL"].groupby(group_by)["sales"].sum().reset_index()
        )
    ...
    if not drill_state:
        ...
        if time_grain == "Monthly":
            group_items = [
                Base_Filters[Base_Filters.index(drill_state) + 1].strip('"'),
                "month_name",
            ]
        else:
    ...
    if '"ind_sbu_cumulative"' in [x["key"] for x in filters]:
        ...
        for each_filter in filters:
            if (
                each_filter["key"] == '"company_name"'
                or each_filter["key"] == "company_name"
            ):
                ...
                req_df = req_df[
                    [
                        ...
                        if any(name.lower() in col.lower() for name in com_name)
                    ]
                    + ["sbu_name"]
                ]
                cum_df = cum_df[
                ...
                for index, row in df_sbu.iterrows():
                    for company in com_name:
                        ...
                        if column_name in df_sbu.columns:
                            sample[row["month_name"] + f"_{company.lower()}_cum"] = row[
                                column_name
                            ]
        ...
        if data and cumulative_data:
            return {
                "message": "Industry month cumulative",
                "data": data,
    ...
    if '"ind_analytics"' in [x["key"] for x in filters]:
        ...
        company_columns = [
            ...
            if col.endswith("_share") and col not in ["market_share"]
        ]
        ...
        for entry in data:
            ...
            for company in companies:
                transformed_data[(sbu, product)][f"{company}_Sales_{year}"] = entry[
                    f"{company.lower()}_share"
                ]
        ...
        for (sbu, product), values in transformed_data.items():
            ...
            years = sorted(
                [y.split("-")[1] for y in values.keys() if "Total_Sales" in y]
            )
            ...
            for i in range(len(years)):
                ...
                values.get(f"Total_Sales_{prev_fiscal_year}", 0)
            ...
            for (sbu, product), values in transformed_data.items():
                ...
                for i in range(len(years)):
                    ...
                    for company in ["HPCL", "BPCL", "IOC"]:
                        sales = values.get(f"{company}_Sales_{fiscal_year}", 0)
                        prev_sales = values.get(
                            f"{company}_Sales_{prev_fiscal_year}", 0
                        )
        ...
        if "sbu_level" in resp_level:
            for item in li:
                transformed_data.append(
                    {
                        f"{prefix_map[item['fiscal_year']]}_market_share": item[
                            "market_share"
                        ],
                        **{
                            f"{prefix_map[item['fiscal_year']]}_{company.lower()}_share": item[
                                f"{company.lower()}_share"
                            ]
                            for company in unique_companies
                        },
    ...
    for item in transformed_data:
        if len(group_items) == 1:
            ...
            for grp_item in group_items[1:]:
                ...
                merged_data[f"{sbu}_{item[base_key]}"][
                    grp_item
                ] = sbu  # Ensure group_item is retained
    if drill_state:
        return generate_stacked_data(
            pd.DataFrame(list(merged_data.values())), resp_format, "month_name"
        )
    ...
    if resp_format == "company_level" and time_grain != "Monthly":
        ...
        history_shares = {
            str(i): input_dict.get(f"history_{company}_share", 0.0)
            for i, company in enumerate(companies)
        }
        actual_shares = {
            str(i): input_dict.get(f"actual_{company}_share", 0.0)
            for i, company in enumerate(companies)
        }
        ...
        return {
            "message": "Industry Cummulative company_level Data",
            "data": transformed_data,
            "status": True,
        }
    if "month_name" in df.columns:
        df["month_name"] = pd.Categorical(
            df["month_name"], categories=[m.upper() for m in m60.months], ordered=True
        )
        df = df.sort_values("month_name").reset_index(drop=True)
    if (
        resp_format == "company_level"
        and time_grain == "Monthly"
        and '"inc"' in [x["key"] for x in filters]
    ):
        getCumulative = False
        ...
        """
        required_companies = [
            x["value"] for x in filters if x["key"].strip('"') == "company_name"
        ][0].split(",")
        base_companies = ["HPCL", "BPCL", "IOCL"]
        selected_columns = [
            col
            for col in df.columns
            if any(comp.lower() in col for comp in base_companies)
        ]
        ...
        [
            f"actual_{company.lower()}_share"
            for company in required_companies
            if f"actual_{company.lower()}_share" in df.columns
        ]
        [
            f"history_{company.lower()}_share"
            for company in required_companies
            if f"history_{company.lower()}_share" in df.columns
        ]
        if len(required_companies) == 6:
            ...
            df["actual_psu_share"] = df[
                [f"actual_{company.lower()}_share" for company in required_companies]
            ].sum(axis=1)
            df["history_psu_share"] = df[
                [f"history_{company.lower()}_share" for company in required_companies]
            ].sum(axis=1)
            new_df = df[
                selected_columns
                + ["month_name", "actual_psu_share", "history_psu_share"]
            ]
        if len(required_companies) > 6:
            ...
            actual_opsu_columns = [
                ...
                for company in required_companies
                if f"actual_{company.lower()}_share" in df.columns
                and company in OMC["OtherPSU"]
            ]
            history_opsu_columns = [
                f"history_{company.lower()}_share"
                for company in required_companies
                if f"history_{company.lower()}_share" in df.columns
                and company in OMC["OtherPSU"]
        ...
        if len(required_companies) <= 3:
            ...
            line_axis = [
                x.split("_")[1]
                for x in li
                if "market" not in x and "actual" not in x and "name" not in x
            ]
            return {
                "message": "Industry_Performance_LineGraph",
                "status": True,
                "data": {
                    key: value.to_dict()
        ...
        return {
            ...
            "data": {
                key: value.to_dict()
                for key, value in new_df.to_dict(orient="series").items()
            },
            "axis": line_axis,
        }
    if (
        resp_format == "company_level"
        and (resp_level == "sbu_level" or resp_level == "product_level")
        and resp_format_org == "company_level_heatmap"
    ):
        ...
        cols = [
            ...
            for col in df.columns
            if com_name in col
            or "month_name" in col
            or "sbu_name" in col
            or "productname" in col
        ]
        df = df[cols]
        for col in df.columns:
        ...
        for entry in list1:
            ...
            actual = (
                entry[f"actual_{com_name}_share"]
                if entry[f"actual_{com_name}_share"] is not None
                else 0.0
            )
            history = (
                entry[f"history_{com_name}_share"]
                if entry[f"history_{com_name}_share"] is not None
                else 0.0
            )

            if req_name not in output and "sbu_level" in resp_level:
                output[req_name] = {
                    "sbu_name": req_name.capitalize()
                }  # Convert back to title case for output
            ...
            output[req_name][f"{month}_actual"] = (
                output[req_name].get(f"{month}_actual", 0.0) + actual
            )
            output[req_name][f"{month}_history"] = (
                output[req_name].get(f"{month}_history", 0.0) + history
            )
        return {
            "message": "Industry Cummulative company_level Data",
    ...
    if (
        resp_format == "company_level"
        and (resp_level == "sbu_level" or resp_level == "product_level")
        and resp_format == "company_level"
    ):
        months = df["month_name"].unique().tolist()
        company = [
            x["value"].strip('"') for x in filters if x["key"] == '"company_name"'
        ][0].lower()
        ...
        for req_name in req_names:
            req_data = {"name": req_name, "Data": []}

            for month in months:
                month_data = {"name": month.capitalize(), "actual": 0, "history": 0}
        ...
        if len(data) == 0:
            return {
                "message": "Industry_Performance_SBU_Level_Graphs",
                "status": False,
                "data": list2,
                "company": unique_companies,
            }
    ...
    if '"table_graph"' in [x["key"] for x in filters]:
        ...
        companies = [
            col.replace("actual_", "").replace("_share", "")
            for col in df.columns
        ...
        for company in companies:
            ...
            if total_history == 0:
                market_share = (
                    100 if total_actual > 0 else None
                )  # If actual > 0 & history = 0 -> 100%, else None
            elif total_actual == 0:
                market_share = -100  # If actual = 0 & history > 0 -> -100%
            else:
                market_share = round(
                    ((total_actual - total_history) / total_history) * 100, 1
                )
        ...
        for company in companies:
            ...
            df[growth_col] = np.where(
                df[history_col] == 0,
                np.where(df[actual_col] > 0, 100, np.nan),
                np.where(
                    df[actual_col] == 0,
                    -100,
                    round(
                        ((df[actual_col] - df[history_col]) / df[history_col]) * 100, 1
                    ),
                ),
            )
            # df[growth_col] = round(((df[actual_col] - df[history_col]) / df[history_col]) * 100,1)
            df.dropna(
                subset=[col for col in df.columns if "growth" in col],
                how="all",
                inplace=True,
            )
        ...
        company_columns = [
            col
            for col in df.columns
            if col not in ["actual_market_share", "month_name"]
        ]
        df[company_columns].sum()
        df["actual_market_share"].sum()
    ...
    if '"table_month"' in [x["key"] for x in filters]:
        ...
        if req_month:
            ...
            company_columns = [
                col.replace("actual_", "").replace("_share", "")
                for col in df.columns
                if col.startswith("actual_") and col != "actual_market_share"
            ]
            ...
            for company in company_columns:
                ...
                if not getCumulative:
                    ...
                    cumulative_historical_market_volume = (
                        df[
                            [
                                col
                                for col in df.columns
                    ...
                    cumulative_market_volume = (
                        df[df["month_name"].str.lower().isin(months_list)][
                            [
                                col
                                for col in df.columns
                                if col.startswith("actual_")
                ...
                market_share_change = (
                    ((actual_volume - historical_volume) / historical_volume * 100)
                ...
                if not getCumulative:
                    ...
                    cumulative_actual = df[
                        df["month_name"].str.lower().isin(months_list)
                    ][f"actual_{company}_share"].sum()
                    cumulative_historical = df[
                        df["month_name"].str.lower().isin(months_list)
                    ][f"history_{company}_share"].sum()
                # cumulative_market_share_change = cumulative_historical - cumulative_actual
                cumulative_market_share_change = (
                    (
                        (cumulative_actual - cumulative_historical)
                        / cumulative_historical
                        * 100
                    )
                    if cumulative_historical
                    else 0
                )
                output.append(
                    {
                        ...
                        "monthly": {
                            ...
                            "marketShare": {
                                "actual": (
                                    round((actual_volume / market_volume) * 100, 2)
                                    if market_volume > 0
                                    else 0
                                ),
                                "historical": (
                                    round(
                                        (historical_volume / historical_market_volume)
                                        * 100,
                                        2,
                                    )
                                    if historical_market_volume > 0
                                    else 0
                                ),
                        ...
                        "cumulative": {
                            ...
                            "marketShare": {
                                "actual": (
                                    round(
                                        (cumulative_actual / cumulative_market_volume)
                                        * 100,
                                        2,
                                    )
                                ...
                                "historical": (
                                    ...
                                    if cumulative_historical_market_volume > 0
                                    else None
                                ),
            ...
            for each in output:
                for each_ele in each:
                    if each[each_ele] is None:
                        each[each_ele] = ""
            return {
                "message": "Industry_Performance__latest_TableData",
                "status": True,
                "data": output,
            }
    ...
    companies = sorted(
        set(
            ...
            if "_" in key
            and key != "cumulative"
            and "actual" not in key
            and "month_name" not in key
        )
    )
...
def generate_stacked_data(df, resp_format="", month_column=""):
    ...
    numeric_cols = [
        col for col in columns if col.startswith("history") or col.startswith("actual")
    ]
    if month_column and month_column in df.columns:
        df[month_column] = pd.Categorical(
            df[month_column],
            categories=[month.upper() for month in m60.months],
            ordered=True,
        )
    ...
    if other_columns:
        ...
        if not resp_format:
            ...
            df_pivot.columns = [
                f"{month}_{metric}" if metric in numeric_cols else metric
            ...
            unique_months = sorted(
                df[month_column].unique(), key=lambda x: m60.months.index(x)
            )
            series_data = []
            ...
            return [
                {
                    "month_name": month,
                    **{
                        key: group[key].tolist() for key in numeric_cols + other_columns
                    },
                }
...
async def industry_performance(
    filters, cross_filters, drill_state="", time_grain="", resp_format="", resp_level=""
):
    print("going to top ")
    ...
    if resp_format == "historical_years":
        ...
        results = await get_fiscal_sales(
            filters=filters, cross_filters=cross_filters, resp_format=resp_format
        )
        ...
        return {"status": True, "message": "Success", "data": results["data"]}

    resp_format_lower = resp_format.lower()
    if resp_format_lower == "file_download":
        # Get the data first
        status, results, _ = await get_zones_and_regions(
            filters, cross_filters, drill_state, time_grain, resp_format_lower
        )
    ...
    if resp_format_lower in ("top_performers", "bottom_performers"):
        status, results, file_path = await get_zones_and_regions(
            filters, cross_filters, drill_state, time_grain, resp_format_lower
        )
        if not status:
    ...
    for condition in filters:
        if (
            condition["key"].strip('"') == "C"
            or condition["key"].strip('"') == "cumulative"
        ):
            cumulative = True
            break
    # omc_compare = ["BPCL", "HPCL"]
    omc_compare = list(
        set([company for sublist in OMC.values() for company in sublist])
    )
    if "," in resp_level:
        resp_level = resp_level.split(",")
    # Fetching all group by filters, return should be a list always
    group_by_filter = generate_group_by_conditions(
        filters, cross_filters, cumulative, drill_state, time_grain, resp_level
    )
    org_filters = filters
    ...
    filters, fiscal_year_pre, fiscal_year_last, months = get_date_filters(
        filters,
        months_list=[
            "APR",
            "MAY",
    ...
    if fiscal_year_pre and not fiscal_year_last:
        filters.append(
            {"key": "fiscal_year", "cond": "equals", "value": fiscal_year_pre}
        )
    elif not fiscal_year_pre and fiscal_year_last:
        filters.append(
            {"key": "fiscal_year", "cond": "equals", "value": fiscal_year_last}
        )
        ...
        filters.append(
            {
                "key": "fiscal_year",
                "cond": "one-off",
                "value": [fiscal_year_pre, fiscal_year_last],
            }
        )
    ...
    cross_filters = [
        ...
        if rec["key"].strip('"') not in ["C", "cumulative"]
        and not (rec["key"] == "month_name" and not rec["value"].strip('"'))
    ]
    ...
    for cond in cross_filters:
        if (
            cond["key"].strip('"') == "month_name"
            or cond["key"].strip('"') == "table_month"
        ) and "," in cond["value"]:
            cond["cond"] = "in"
                ...
                filters.append(
                    {
                        "key": "coname",
                        "cond": "one-off",
                        "value": [x.upper() for x in each_filter["value"].split(",")],
                    }
                )
    print("changed here")
    print("filters", filters)
    ...
    resp_data = await m60.collect_data(
        [req_keys],
...
def generate_growth_query(
    ...
    company_cases = ",\n".join(
        [
            ...
            for company in companies
            if company in [growth_company, drop_company]
        ]
    )
    company_cases += ",\n"
    company_cases += ",\n".join(
    ...
    overall_condition = (
        "("
        + " + ".join([f"{company.lower()}_current" for company in ["market_share"]])
        + ")"
    )
    overall_last_condition = (
        "("
        + " + ".join([f"{company.lower()}_last" for company in ["market_share"]])
        + ")"
    )
...
async def industry_performance_compare(
    filters, cross_filters, drill_state="", time_grain="", resp_format=""
):
    companies = []
    omcs = []
    ...
    filters = [
        cond_filter
        for cond_filter in filters
        if cond_filter["key"].strip('"') not in OMC
        and cond_filter["key"].strip('"') not in all_companies
    ]
    ...
    omc_companies = list(
        set(
            [
                company
                for cond_filter in omcs
                for company in OMC[cond_filter["key"].strip('"')]
            ]
        )
    )
    ...
    filters.append(
        {
            ...
            "value": list(
                set(
                    omc_companies
                    + [cond_filter["key"].strip('"') for cond_filter in companies]
                )
            ),
        }
    )
    ...
    df[
        [
            f"{drop_company.lower()}_current",
            f"{drop_company.lower()}_last",
            f"{growth_company.lower()}_current",
            f"{growth_company.lower()}_last",
            "market_share_last",
...
async def get_category_wise_cumulative_data(filters):
    ...
    req_keys = [
        f"""ROUND(SUM("netweight_tmt")/1000::numeric,0) AS "sales" """,
        "coname",
        "company_name",
        "fiscal_year",
    ]
    resp_data = await m60.collect_data(
        req_keys, "industry_performance", where_conditions, "", "", group_by, ""
    )
    ...
    for year in fiscal_years:
        ...
        filtered_df_psu = filtered_df[
            filtered_df["coname"].isin(OMC["MPSU"] + OMC["OtherPSU"])
        ]
        filtered_df_psu["company_name"] = "PSU"
        filtered_df_pvt = filtered_df[
            filtered_df["coname"].isin(OMC["PSU"] + OMC["PVT"])
        ]
        ...
        for co in details:
            ...
            if co == "MPSU":
                        ...
                        "subData": [
                            {
                                ...
                                "value": v,
                                "percentage": (
                                    round((v / total) * 100, 2) if total != 0 else 0
                                ),
                            }
                        ],
                    }
                    for c, v in details[co].items()
                ]
            elif co == "PSU":
                mpsu = [
                    {
                ...
                other_psu = [
                    ...
                    for c, v in details[co].items()
                    if c in OMC["OtherPSU"]
                ]
                mpsu.append(
                    {
                        "category": "Other PSU",
                        "value": sum([r["value"] for r in other_psu]),
                        "percentage": (
                            round(
                                (sum([r["value"] for r in other_psu]) / total) * 100, 2
                            )
                            if total != 0
                            else 0
                        ),
                ...
                mpsu.append(
                    {
                        "category": "PVT",
                        "value": sum([r["value"] for r in pvt]),
                        "percentage": (
                            round((sum([r["value"] for r in pvt]) / total) * 100, 2)
                            if total != 0
                            else 0
                        ),
                        "subData": pvt,
    ...
    for cogroup, details in result_dict[fiscal_years[-1]].items():
        growth_dict[cogroup] = {}
        cat_prev = {
            rec["category"]: rec for rec in result_dict[fiscal_years[0]][cogroup]
        }
        print("cat_prev", cat_prev)
        cat_pres = {
            rec["category"]: rec for rec in result_dict[fiscal_years[1]][cogroup]
        }
        ...
        for company, data in cat_pres.items():
            if len(data["subData"]) <= 1:
                ...
                growth_dict[cogroup][company] = round(
                    data["percentage"] - prev_percentage, 2
                )
                ...
                cat_prev_sub = {
                    rec["category"]: rec for rec in cat_prev[company]["subData"]
                }
                ...
                for co, dt in cat_pres_sub.items():
                    ...
                    growth_dict[cogroup][co] = round(
                        dt["percentage"] - prev_dt["percentage"], 2
                    )
...
async def generate_omc_compare_data(filters, drill_state):
    ...
    months = []
    ind_sbu_cumulative = [
        x["value"] for x in filters if "ind_sbu_cumulative" in x["key"].lower()
    ]

    # Fetching all selected Company groups
    selected_co = [
        x["value"].strip() for x in filters if x["key"].strip('"') == "cogroup"
    ]
    ...
    required_companies = list(
        set(
            [
                [rec.strip() for rec in x["value"].split(",")]
                for x in filters
                if x["key"].strip('"') == "coname"
            ][0]
        )
    )
    ...
    filters = [
        x
        for x in filters
        if x["key"].strip('"') not in ["cogroup", "coname", "ind_sbu_cumulative"]
        and x["value"]
    ]
    ...
    if True in ind_sbu_cumulative or "true" in ind_sbu_cumulative:
        ...
        if len(req_month.split(",")) >= 2:
            ...
            months = months_list[
                months_list.index(start_month) : months_list.index(end_month) + 1
            ]
        else:
    ...
    filters = [
        cond_filter
        for cond_filter in filters
        if cond_filter["key"].strip('"') not in ["fiscal_year", "month_name"]
    ]
    ...
    filters.append(
        {"key": "coname", "cond": "one-off", "value": market_share_companies}
    )
    year_filters = [
        filter for filter in org_filters if filter["key"].strip('"') == "fiscal_year"
    ]
    ...
    if year_filters[0]["value"].strip('"') != fiscal_year_pre:
        ...
        if (
            abs(
                int(year_filters[0]["value"].split("-")[0])
                - int(fiscal_year_pre.split("-")[0])
            )
            != 2
        ):
            print("coming in if")
            ...
            fiscal_year_last = (
                str(int(fiscal_year_last.split("-")[0]) - 1)
                + "-"
                + str(int(fiscal_year_last.split("-")[-1]) - 1)
            )
            filters.append(
                {
                    "key": '"fiscal_year"',
        ...
        filters.append(
            {"key": '"fiscal_year"', "cond": "in", "value": ["2025-2026", "2026-2027"]}
        )
    ...
    for entry in resp_data:
        ...
        sales = (
            0
            if sales is None or (isinstance(sales, str) and sales.strip() == "")
            else sales
        )
    ...
    structured_data = [
        {f"{drill_state if drill_state else 'cumulative'}": key, **values}
    ...
    for entry in structured_data:
        ...
        if total_sales != 0:
            for company in entry["Sales"]:
                entry["Market Share"][company] = round(
                    (entry["Sales"].get(company, 0) / total_sales) * 100, 2
                )
        else:
            for company in entry["Sales"]:
        ...
        if total_history_sales != 0:
            for company in entry["History"]:
                entry["Market Share History"][company] = round(
                    (entry["History"].get(company, 0) / total_history_sales) * 100, 2
                )
        else:
            for company in entry["History"]:
        ...
        for company in entry["Sales"]:
            if company in entry["History"] and entry["History"].get(company, 0) != 0:
                entry["Growth"][company] = round(
                    (
                        (
                            entry["Sales"].get(company, 0)
                ...
                company in entry["History"]
                and entry["History"].get(company, 0) == 0
                and entry["Sales"].get(company, 0) != 0
            ):
                entry["Growth"][
                    company
                ] = 100  # or some other value that makes sense for your use case
            elif (
                company in entry["History"]
                and entry["History"].get(company, 0) == 0
                and entry["Sales"].get(company, 0) == 0
                ...
                entry["Growth"][
                    company
                ] = 0  # or some other value that makes sense for your use case
            else:
                entry["Growth"][
                    company
                ] = None  # or some other value that makes sense for your use case
        ...
        for word in words:
            if word in question:
                extracted[key].append(
                    word
                )  # Assign the found keyword (or later a matched value)
    return extracted

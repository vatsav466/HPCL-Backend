...
# ── Original Helpers (from pasted_content_2.txt) ──────────────────────────────
...
async def send_tas_notification(template_name, to_recipients, subject, cc_recipients=None, bcc_recipients=None, notification_data=None):
    ...
    if not os.path.exists(template_path):
        template_path = os.path.join(os.path.dirname(hpcl_ceg_model.__file__), '..', 'orchestrator', 'reporting_services', 'templates', template_name)
    ...
    await ins.publish_message(
        subject=subject, recipients=to_recipients, cc_recipients=cc_recipients or [],
        bcc_recipients=bcc_recipients or [], html_content=True, body=final_data, force_send=True
    )
...
def find_gaps(records: list[dict], gap_threshold_min: int, start_ms: int, end_ms: int) -> list[dict]:
    gaps, threshold_ms = [], gap_threshold_min * 60 * 1000
    
    if not records:
        # If no data at all since start_ms
        return [{"duration_min": round((end_ms - start_ms) / 60000, 1)}]

    # 1. Check Initial Gap: From Midnight (start_ms) to the FIRST data point of the day
    # records are DESC, so records[-1] is the earliest record today
    earliest_today_ts = records[-1]["ts"]
    initial_gap_ms = earliest_today_ts - start_ms
    if initial_gap_ms > threshold_ms:
        gaps.append({"duration_min": round(initial_gap_ms / 60000, 1)})

    # 2. Check gaps BETWEEN records
    for i in range(len(records) - 1):
        diff_ms = records[i]["ts"] - records[i + 1]["ts"]
        if diff_ms > threshold_ms:
            gaps.append({"duration_min": round(diff_ms / 60000, 1)})
            
    # 3. Check Trailing Gap: From the LATEST data point to NOW (end_ms)
    # records are DESC, so records[0] is the most recent record
    latest_today_ts = records[0]["ts"]
    trailing_gap_ms = end_ms - latest_today_ts
    if trailing_gap_ms > threshold_ms:
        gaps.append({"duration_min": round(trailing_gap_ms / 60000, 1)})
...
async def publish_integrated_daily_report(email_type, audience):
    ...
    start_ms = int(now_ist.replace(hour=0, minute=0, second=0, microsecond=0).timestamp() * 1000)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for loc_name, dev in sorted(location_map.items()):
            ...
            # FIXED: Pass start_ms and end_ms to find_gaps to ensure Midnight check
            gaps = find_gaps(records, GAP_THRESHOLD_MIN, start_ms, end_ms)
            
            total_min = sum(g["duration_min"] for g in gaps)
            
            # Note: The logic for "if not records" is now handled inside find_gaps
            
            sev_name, sev_class = get_severity_info(total_min)
            loc_summary.append({
                "name": loc_name, 
                "count": len(gaps), 
                "downtime": f"{int(total_min//60)}h {int(total_min%60)}m", 
                "severity": sev_name, 
                "severity_class": sev_class
            })

    print("[3/4] Preparing Notification Data...")
    status_data = {
        ...
        'loc_summary': loc_summary, 'gap_threshold': GAP_THRESHOLD_MIN
    }

    print("[4/4] Sending Integrated Email...")
    recipients = await get_email_users_by_type(email_type=email_type, audience=audience)
    await send_tas_notification(
        template_name="seg6.html", to_recipients=recipients["to"],
        subject="Novex Daily Report: TAS Alerts & Loss of Communication Summary",
        cc_recipients=recipients["cc"], bcc_recipients=recipients["bcc"],

...
def group_tk_devices_by_location(devices: list[dict]) -> dict[str, dict]:
    ...
    for dev in devices:
        ...
        is_exception = bool(
            exception_prefix
            and local_name.strip().upper().startswith(exception_prefix.upper())
        )
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for loc_name, dev in sorted(location_map.items()):
            records = await fetch_telemetry(
                session, jwt, dev["id"]["id"], TELEMETRY_KEY, start_ms, end_ms
            )
            ...
            loc_summary.append(
                {
                    ...
                    "count": len(gaps),
                    "downtime": (
                        f"{int(total_min//60)}h {int(total_min%60)}m"
                        if total_min > 0
                        else "0h 0m"
                    ),
                    "severity": sev_name,

...
async def nozzles_sales_top_performance():
    ...
    nozzle_sales_top_query = f"""
                SELECT
                    ...
                    ROUND(
                        SUM(sales_volume) FILTER (
                            WHERE product_grp IN ('MS', 'POWER 99', 'POWER 95', 'POWER 100')
                            AND transaction_date >= 
                                CASE
                                    WHEN CURRENT_DATE = date_trunc('month', CURRENT_DATE)::date
                                    THEN date_trunc('month', CURRENT_DATE - INTERVAL '1 month')
                    ...
                    ROUND(
                        SUM(sales_volume) FILTER (
                            ...
                            AND transaction_date >= 
                                CASE
                                    WHEN CURRENT_DATE = date_trunc('month', CURRENT_DATE)::date
                                    THEN date_trunc('month', CURRENT_DATE - INTERVAL '2 month')
                    ...
                    END AS current_month_label,
                    CASE
                        WHEN CURRENT_DATE = date_trunc('month', CURRENT_DATE)::date THEN
                            TO_CHAR(date_trunc('month', CURRENT_DATE - INTERVAL '2 month'), 'Mon''YY')
                        ELSE
                            TO_CHAR(date_trunc('month', CURRENT_DATE - INTERVAL '1 month'), 'Mon''YY')
                    END AS last_month_label
    ...
    Charts_Connection_Vault_RoutingParams.action = 'execute_query'
    function = await charts_connection_vault_routing(Charts_Connection_Vault_RoutingParams)
    nozzle_sales_top = await function(query=nozzle_sales_top_query)
    nozzle_sales_top_df = pl.DataFrame(nozzle_sales_top)
    print("nozzle_sales_top_df = pd.DataFrame(nozzle_sales_top_bottom) ---->\n", nozzle_sales_top_df.head(5))
    print("count of records in nozzle_sales_top_df ---->\n", nozzle_sales_top_df.shape[0])

    nozzle_sales_top_df.write_csv("/tmp/nozzle_sales_top_performance.csv")

    labels = nozzle_sales_top_df.select([
        "current_month_label",
        "last_month_label"
    ]).unique().to_dicts()[0]
    print("labels ---->\n", labels)
    ...
    top_3_sales_areas = (nozzle_sales_top_df
        .group_by(["zone", "region", "sales_area"]).agg(
            ...
            last_month_power_kl_sum = pl.col("last_month_power_kl").sum(),
            last_month_ms_total_kl_sum = pl.col("last_month_ms_total_kl").sum(),
            last_month_conversion_pct = (pl.col("last_month_power_kl").sum() / pl.col("last_month_ms_total_kl").sum()) * 100
        )
    )
    ...
    return {
        ...
        "nozzle_present_month": labels["current_month_label"],
        "nozzle_previous_month": labels["last_month_label"]
    }

...
def fetching_incidnets_from_server():
    ...
    for name, config in CAMUNDA_DRYOUT_INSTANCES.items():
        server_instances.append(
            {"server_type": "DRYOUT", "host": config["host"], "port": config["port"]}
        )
...
def fetch_incidents():
    ...
    for instance in camunda_instances:
        ...
        try:
            ...
            print(
                f"Error in getting incidents information from {instance['host']}:{instance['port']} → {e}"
            )
...
def generate_excel(all_incidents):
    temp = tempfile.NamedTemporaryFile(
        prefix="incident_result_", suffix=".xlsx", delete=False
    )
...
def generate_csv(all_incidents):
    temp = tempfile.NamedTemporaryFile(
        prefix="incident_result_", suffix=".csv", delete=False
    )
    CSV_PATH = temp.name
    temp.close()
...
def incident_summary(incidents):
    ...
    for (server_type, host, port), count in counter.items():
        summary.append(
            {"server_type": server_type, "host": host, "port": port, "count": count}
        )
...
async def send_email(
    template_name, to_recipients, subject, cc_recipients, final_data, attachments=None
):
...
async def main():
    ...
    email_data = {
        "generated_time": datetime.now(ZoneInfo("Asia/Kolkata")).strftime(
            "%d-%m-%Y %I:%M %p"
        ),
        "summary": summary,
    }

...
def get_zones_by_performance(actual, target, by_sbu=False, req_key="Zone_Name"):
    ...
    if not by_sbu:
        resp_actual = {
            f"{rec['SBU_Name']}_{rec[req_key]}": float(rec["ACTUAL_TMT_SALES"])
            for rec in actual
        }
        resp_target = {
            f"{rec['SBU_Name']}_{rec[req_key]}": float(rec["TARGET_TMT_SALES"])
            for rec in target
        }
        # Calculate percentage achieved
        percentage_achieved = {}
        for key in resp_target:
            actual_value = resp_actual.get(
                key, 0
            )  # Get actual value, default to 0 if missing
            target_value = resp_target[key]
            if target_value == 0:
                percentage_achieved[key] = (
                    0 if not actual_value else 100
                )  # Avoid division by zero
            else:
                percentage_achieved[key] = round(
                    float(((actual_value - target_value) / target_value) * 100), 1
                )
        sorted_zones = sorted(
            percentage_achieved.items(), key=lambda x: x[1], reverse=True
        )
    ...
    for sbu in resp_target:
        ...
        for zone in resp_target[sbu]:
            actual_value = resp_actual.get(sbu, {}).get(
                zone, 0
            )  # Get actual value, default to 0 if missing
            target_value = resp_target.get(sbu, {}).get(
                zone, 0
            )  # Get actual value, default to 0 if missing
            if target_value == 0:
                percentage_achieved[zone] = (
                    0 if not actual_value else 100
                )  # Avoid division by zero
            else:
                percentage_achieved[zone] = round(
                    float(((actual_value - target_value) / target_value) * 100), 1
                )
        sorted_zones = sorted(
            percentage_achieved.items(), key=lambda x: x[1], reverse=True
...
async def get_m60_sales_data():
    ...
    yesterday_last_year = helpers.get_time_stamp_by_delta(
        dt,
        days=1,
        years=1,
        ascending=False,
    ...
    sbu_level_zones = get_zones_by_performance(
        resp_actual, resp_target, by_sbu=True, req_key="Zone_Name"
    )
    ...
    resp_target = await function(query=target)
    resp_actual = await function(query=actual)
    sbu_level_regions = get_zones_by_performance(
        resp_actual, resp_target, by_sbu=True, req_key="Region_Name"
    )
...
async def fetch_sales_data():
    ...
    # Filter for YTD
    filters = {
        "filters": [actual, history, cumulative, ytd, target],
        "cross_filters": [],
        "drill_state": "",
        "time_grain": "",
    }
    ...
    # exit()
    sales_data["current_sales"] = round_off(
        resp["data"]["data"]["ACTUAL_TMT_SALES"][0], ""
    )

    sales_data["history_sales"] = round_off(
        resp["data"]["data"]["ACTUAL_HISTORY_TMT_SALES"][0], ""
    )
    sales_data["pro_rate_sales_target"] = round_off(
        resp["data"]["data"]["TARGET_TMT_SALES"][0], ""
    )
    ...
    for sbu in ["Retail", "LPG", "Lubes"]:
        sales_data[f"{sbu}_growing_zones"] = len(
            [rec for rec in sbu_level_zones[sbu] if rec[1] > 0]
        )
        sales_data[f"{sbu}_declining_zones"] = len(
            [rec for rec in sbu_level_zones[sbu] if rec[1] < 0]
        )
    ...
    filters = {
        "filters": [
            ...
            cumulative,
            {
                "key": '"DATE"',
                "cond": "equals",
                "value": f"{yesterday_date},{yesterday_date}",
            },
        ],
        "cross_filters": [],
        "drill_state": "",
    ...
    sales_data["yesterday_growth_loss_percentage"] = get_growth_percentage(
        yesterday_act, yesterday_hist
    )

    sales_data["total_growth_loss"] = round(
        float(sales_data["current_sales"]) - float(sales_data["history_sales"])
    )
    sales_data["total_growth_loss_percentage"] = get_growth_percentage(
        float(sales_data["current_sales"]), float(sales_data["history_sales"])
    )
    ...
    for sbu_name, map_key in sbu_mapping.items():
        ...
        sbu_sales_data["present_month_growth"] = get_growth_percentage(
            present_month_act, present_month_hist
        )
        sbu_sales_data["ytpm_historical"] = sbu_sales_data["ytpm_current"] = (
            sbu_sales_data["ytpm_growth"]
        ) = "NA"
        ...
        filters = {
            "filters": [
                ...
                cumulative,
                {
                    "key": '"DATE"',
                    "cond": "equals",
                    "value": f"{prev_month_start},{prev_month_end}",
                },
            ],
            "cross_filters": [],
            "drill_state": "",
        ...
        if present_month != 4:
            filters = {
                "filters": [actual, history, ytpm, cumulative],
                "cross_filters": [],
                "drill_state": "",
            }
            if ytpm:
                ...
                filters["filters"].append(
                    {"key": '"fiscal_year"', "cond": "equals", "value": dyn_fis_year}
                )
...
async def get_alert_data(alert_section):
    ...
    for alert in alerts["data"]:
        if alert["severity"] == "Critical":
            data.update(
                {
                    f"{alert_section.lower()}_critical_{alert['bu'].lower()}": alert[
                        "count"
                    ]
                }
            )
        if alert["severity"] == "High":
            data.update(
                {f"{alert_section.lower()}_high_{alert['bu'].lower()}": alert["count"]}
            )

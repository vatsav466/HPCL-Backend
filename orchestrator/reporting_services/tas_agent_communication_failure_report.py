...
async def get_email_users_by_type(email_type, audience):
    ...
    for user in all_users.get("data", []):
        if (
            user.get("audience") == audience
            and (user.get("email_type") or "").lower() == email_type.lower()
        ):
...
async def send_report_email(
    ...
    subject = (
        f"OPC Telemetry Loss of Communication Report — " f"{start_dt.strftime('%B %Y')}"
    )
...
def group_tk_devices_by_location(devices):
    ...
    for dev in devices:
        ...
        is_exception = bool(
            exception_prefix and local_name.upper().startswith(exception_prefix.upper())
        )
...
def generate_word_report(report, location_map, start_dt, end_dt):
    ...
    info.add_run(
        f"Period: {start_dt.strftime('%b %d')} – {end_dt.strftime('%b %d, %Y')}\n"
    )
    ...
    sev_data = [
        ...
        (
            "Low",
            "< 2h",
            "Minor interruptions. Likely transient connectivity issues.",
            "bf9000",
        ),
        ...
        (
            "High",
            "10h – 20h",
            "Significant data loss. Immediate action recommended.",
            "ea9999",
        ),
        (
            "Critical",
            "> 20h",
            "Severe prolonged outage. Escalation required.",
            "ff0000",
        ),
    ]
    for i, (lvl, thr, desc, color) in enumerate(sev_data, 1):
        ...
        if lvl in ["None", "Low", "Medium", "Critical"]:
            sev_table.cell(i, 0).paragraphs[0].runs[0].font.color.rgb = RGBColor(
                255, 255, 255
            )
    ...
    for location in sorted(location_map.keys()):
        ...
        row_cells[2].text = (
            f"{int(total_min // 60)}h {int(total_min % 60)}m" if total_min > 0 else "—"
        )
    ...
    for location, rows in sorted(by_location.items()):
        ...
        for r in sorted(rows, key=lambda x: x["gap_start"]):
            ...
            row_cells[2].text = (
                f"{int(r['duration_min'] // 60)}h {int(r['duration_min'] % 60)}m"
            )

    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "reports")
    os.makedirs(output_dir, exist_ok=True)
    filename = os.path.join(
        output_dir, f"OPC_Connectivity_Outage_Report_{start_dt.strftime('%B%Y')}.docx"
    )
    doc.save(filename)
    return filename
...
async def main(email_type: str = "loc_report", audience: str = "TAS"):
    ...
    current_month_start = now_ist.replace(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for location, device in sorted(location_map.items()):
            ...
            records = await fetch_telemetry(
                session, jwt, device_id, TELEMETRY_KEY, start_ms, end_ms
            )

...
class SimpleQueryGenerator:
    def __init__(self, prompt_to_query, word_to_idx, idx_to_word, embedding_matrix):
        ...
        for category in self.categories:
            ...
            if category_queries:
                self.category_templates[category] = random.sample(
                    category_queries, min(5, len(category_queries))
                )
    ...
    def get_metadata(self):
        return {
            ...
            "num_templates": sum(
                len(templates) for templates in self.category_templates.values()
            ),
...
def generate_sql_query(prompt):
    query_generator = SimpleQueryGenerator(
        prompt_to_query, word_to_idx, idx_to_word, embedding_matrix
    )

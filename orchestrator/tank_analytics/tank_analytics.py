...
def process_rows(rows: list[dict]) -> list[dict]:
    ...
    return [
        {
            k: (
                tank_queries.product_name_mapping.get(v, v)
                if k == "product"
...
async def generate_filters_cond(filters):
    ...
    for filter in filters:
        ...
        if key == "date_time":
            date = filter.value
            cond = (
                f"hltd.{key}::DATE BETWEEN DATE '{date.replace(",", "' AND DATE '")}'"
            )
            conds[key] = cond
        elif key == "sap_id":
            sap_id = filter.value
...
async def stock_dispatch_receipt(filter_clause, connection_function):
    ...
    queries = [
        tank_queries.queries.get(key, "").format(condition=filter_clause)
    ...
    return {key: pl.DataFrame(process_rows(rows)) for key, rows in zip(keys, results)}
...
async def tank_ullage(filter_clause, connection_function):
    ...
    ullage_data = (
        ...
        )  # drop rows with no tank_dia_details match
        .group_by("product")
        .agg(
            pl.col("ullage").sum(),
            pl.col("capacity").sum(),
...
async def action_tankwise_sustainability(filter_clause, connection_function):
    ...
    """
    query = tank_queries.queries.get("tankwise_stock_sustainability").format(
        filter_clause[0], filter_clause[0], filter_clause[1], filter_clause[0]
    )
    ...
    tankwise_data = (
        pl.DataFrame(process_rows(tankwise_resp))
        .with_columns(
            (
                (
                    pl.when(pl.col("available_stock_kl") < 0)
                    .then(0)
                    .otherwise(pl.col("available_stock_kl"))
    ...
    return {
        "status": True,
        "data": sanitize_for_json(tankwise_data.fill_nan(None).to_dicts()),
...
async def action_stock_sustainability(filter_clause, connection_function):
    ...
    """
    stock_clause = (
        f"date_time::DATE = DATE '{date.today().strftime("%Y-%m-%d")}' {filter_clause}"
    )

    # get net stock
    tank_data = await stock_dispatch_receipt(
        filter_clause=stock_clause, connection_function=connection_function
    )
    net_stock = await calculate_net_stock(tank_data)

    # get dispatch average
    dispatch_average_query = tank_queries.queries.get("dispatch_average", "").format(
        filter_clause
    )
    ...
    if isinstance(stock_sustainability, float) and (
        math.isnan(stock_sustainability) or math.isinf(stock_sustainability)
    ):
        stock_sustainability = 0

    return {
        "status": True,
        "data": stock_sustainability,
        "message": "successfully retrieved data",
    }
...
async def action_product_wise_trends(filter_clause, connection_function):
    ...
    prod_avg_dispatch = pl.DataFrame(process_rows(prod_avg_resp)).drop_nulls()

    # join all dataframes
    res = (
        stock_data.join(dispatch_data, on="product", how="full", coalesce=True)
    ...
    res = res.with_columns(
        (
            (
                pl.when(pl.col("available_stock") < 0)
                .then(0)
                .otherwise(pl.col("available_stock"))
                + pl.col("tank_receipt")
    ...
    ullage_data = await tank_ullage(
        filter_clause=filter_clause, connection_function=connection_function
    )
    res = res.join(ullage_data, on="product", how="left")

    return {
        "status": True,
        "data": sanitize_for_json(res.fill_nan(None).to_dicts()),
...
async def action_daily_trends(filter_clause, connection_function):
    ...
    Returns:
        api response dictionary with product wise trends data
    """
    tank_dispatch = tank_queries.queries.get("dispatch", "").format(
        condition=filter_clause
    )
    tank_receipt = tank_queries.queries.get("receipt", "").format(
        condition=filter_clause
    )
    bcu_dispatch = tank_queries.queries.get("bcu_total_dispatch", "").format(
        condition=filter_clause
    )
    ...
    for prod_data in bcu_resp:
        ...
        prod_data["product"] = tank_queries.product_mapping[sap_id][
            "host_day_end_details"
        ][prod]

    dispatch_df = pl.DataFrame(tank_dis_resp).rename({"sum": "tank_dispatch"})

    bcu_df = pl.DataFrame(bcu_resp).rename({"sum": "bcu_dispatch"})

    reciept_df = pl.DataFrame(tank_rec_resp).rename({"sum": "tank_receipt"})
    ...
    resp = (
        dispatch_df.join(
            bcu_df, on=["product", "sap_id", "zone"], how="full", coalesce=True
        )
...
async def action_daywise_trends(filter_clause, connection_function):
    ...
    """
    products_query = tank_queries.queries.get("daywise_trends", "").format(
        filter_clause
    )
    print("products query -->", products_query)
    ullage_query = tank_queries.queries.get("ullage_daywise_trends", "").format(
        filter_clause
    )
    print("ullage query -->", ullage_query)
    ...
    for daywise_data in products_res:
        daywise_data["product"] = tank_queries.product_name_mapping.get(
            daywise_data["product"], daywise_data["product"]
        )
    products_res = [
        {k: float(v) if isinstance(v, Decimal) else v for k, v in row.items()}
    ...
    products_data = (
        products_data.group_by(["product", "date_time"])
        .agg(pl.col("dispatch").sum(), pl.col("reciept").sum())
        .drop_nulls()
    )

    ullage_data = pl.DataFrame(ullage_res)
    ullage_data = (
        ullage_data.group_by(["product", "date_time"])
        .agg(
    ...
    res = products_data.join(
        ullage_data, on=["product", "date_time"], how="left"
    ).drop_nulls()
    return {
        "status": True,
        "data": sanitize_for_json(res.fill_nan(None).to_dicts()),
...
async def tank_analytics(filters, action, drill_state, cross_filters, payload):
    ...
    try:
        ...
        sap_id_filter = (
            f"AND {filter_conditions['sap_id']}"
            if filter_conditions.get("sap_id", "")
            else ""
        )
        zone_filter = (
            f"AND {filter_conditions['zone']}"
            if filter_conditions.get("zone", "")
            else ""
        )
        filter_clause = " AND ".join(
            [value for key, value in filter_conditions.items()]
        )
        ...
        if action == "stock_sustainability":
            return await action_stock_sustainability(
                sap_id_filter + zone_filter, function
            )

        if action == "product_wise_trends":
            return await action_product_wise_trends(
                sap_id_filter + zone_filter, function
            )
        ...
        if action in ("total_capacity", "total_ullage"):
            ullage = await tank_ullage(
                filter_clause=sap_id_filter + zone_filter, connection_function=function
            )
        ...
        if action == "tankwise_sustainability":
            ...
            return await action_tankwise_sustainability(
                [filter_clause, average_filter_clause], function
            )

        if action == "net_stock":
            tank_data = await stock_dispatch_receipt(
                filter_clause=filter_clause, connection_function=function
            )

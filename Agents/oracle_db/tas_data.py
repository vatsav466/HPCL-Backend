...
async def host_unauthorized(file_name):
    ...
    df["date"] = df["TIMESTAMP"].apply(
        lambda x: datetime.datetime.strptime(x, "%Y-%m-%d %H:%M:%S").strftime(
            "%Y-%m-%d"
        )
    )
...
async def host_local_loadedtts(file_name):
    ...
    df["sap_id"] = "1128"
    if (
        "TRANSACTION_END_TIME" in df.columns
        and not df["TRANSACTION_END_TIME"].isna().all()
    ):
...
async def host_bay_reassignment(file_name):
    ...
    df["sap_id"] = "1128"
    if (
        "BAY_REASSIGNMENT_TIME" in df.columns
        and not df["BAY_REASSIGNMENT_TIME"].isna().all()
    ):
...
def add_random_created_at(
    ...
    for key, records in data_dict.items():
        ...
        for record in records:
            ...
            created_at = today - datetime.timedelta(

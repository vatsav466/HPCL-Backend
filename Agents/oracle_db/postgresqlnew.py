...
class Postgresql:
        ...
        try:
            ...
            if data.empty:
                return (
                    False,
                    "No Data Available",
                    "No records found for manual FAN printed",
                )
    ...
    async def create_cancel_tt_report(self, data):
        ...
        try:
            ...
            query = (
                f"""select id from alerts where interlock_name = 'Cancel TT Reported' and vehicle_number = '{data["vehicle_number"]}' """
    ...
    async def process_host_manual_fan_printed(self, data, table_db_name, model):
        ...
        try:
            ...
            if int(current_hour) == 19 and resp.get("data", []):
                ...
                interlock_name = config["interlock_name"].get(
                    "host_manual_fan_printed", "Manual FAN Printed"
                )
                ...
                if is_close_alert:
                    close_success, close_msg = await self.close_created_alert(
                        alert_data=alert_data
                    )
                    print(
                        f"Manual FAN alert closure result: {close_success}, Message: {close_msg}"
                    )
    ...
    async def process_host_unauthorised_flow(self, data, table_db_name, model):
        ...
        try:
            ...
            total_net = sum(
                float(record.get("nettotalizer", 0))
            ...
            bcu_numbers = sorted(
                set(record.get("bcu_number", "") for record in unauthorised_records)
            )
            ...
            if success:
                ...
                if is_close_alert:
                    ...
                    print(
                        f"Unauthorized flow alert closure result: {close_success}, Message: {close_msg}"
                    )
    ...
    async def create_table(
        ...
        try:
            ...
            status, msg = await model.bulk_update(
                data, upsert=True, upsert_skip_keys=["alert_created"]
            )
            if not status:
            ...
            if table_db_name == "host_manual_fan_printed":
                status, message = await self.process_host_manual_fan_printed(
                    data, table_db_name, model
                )
                return {"status": "Table created and processed", "message": message}

            elif table_db_name == "host_unauthorised_flow":
                status, message = await self.process_host_unauthorised_flow(
                    data, table_db_name, model
                )
                return {"status": "Table created and processed", "message": message}
        ...
        try:
            ...
            for record in resp.get("data", []):
                try:
                    ...
                    if interlock_name == "Cancel TT Reported":
                        is_close_alert = True
                        success, msg, alert_data = await self.create_cancel_tt_report(
                            alert_data
                        )
                    else:
                        success, msg = await alert_factory.AlertFactory.create_alert(
                            alert_data
                        )

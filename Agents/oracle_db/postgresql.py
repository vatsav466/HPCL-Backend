...
class Postgresql:
    ...
    async def cal_host_manual_fan_printed(self, data):
        ...
        if total_fan_count > 0:  # Avoid division by zero
            ...
            return (
                False,
                "Division by zero",
                "Total count is zero but manual count exists",
            )
        ...
        if manual_percentage > 5:
            ...
            return (
                ...
                f"Manual percentage: {manual_percentage:.2f}% exceeds threshold of 5%",
            )
        else:
            return
    ...
    async def create_cancel_tt_report(self, data):
        ...
        query = (
            f"""select id from alerts where interlock_name = 'Cancel TT Reported' and vehicle_number = '{data["vehicle_number"]}' and tt_load_number = '{data["tt_load_number"]}' """
        ...
        if resp.get("data", []):
            ...
            input_data = {"action_type": "Cancelled", "action_msg": action_msg}
    ...
    async def create_bay_reasignment_report(self, data, compartment_number=None):
        ...
        to_date = urdhva_base.utilities.get_present_time(True).strftime("%Y-%m-%d")
        query = (
            f"""select id from alerts where interlock_name = 'Bay reassignment' and vehicle_number = '{data["vehicle_number"]}' and tt_load_number = '{data["tt_load_number"]}' """
            f"""and created_at::DATE = '{to_date}'"""
        )
        resp = await hpcl_ceg_model.Alerts.get_aggr_data(query)
        if resp.get("data", []):
            ...
            input_data = {"action_type": "BayReAssigned", "action_msg": action_msg}
            await alert_manager.AlertAction().update_alert_history(
                input_data=input_data, alert_data=alert_data
            )
            return True, "Success", data
    ...
    async def create_sick_tt_report(self, data):
        ...
        if resp.get("data", []):
            print(
                f"Sick TT alert already exists for Truck: {vehicle_number}, Load: {tt_load_number}"
            )
    ...
    async def close_created_alert(self, alert_data):
        ...
        close_success, close_msg = await alert_factory.AlertFactory.close_alert(
            close_data
        )
    ...
    async def create_table(
        self, schema_name, table_name, sample_records, primary_key=[], unique_key=[]
    ):
        """
        ...
        for record in sample_records:
            ...
            if (
                "cum_start_mass_mt" in record
                and record["cum_start_mass_mt"] is not None
            ):
            ...
            for key, value in record.items():
                ...
                if "cancelled_date" in record and isinstance(
                    record["cancelled_date"], str
                ):
                ...
                if "transaction_end_time" in record and isinstance(
                    record["transaction_end_time"], str
                ):
                    record["transaction_end_time"] = pd.to_datetime(
                        record["transaction_end_time"]
                    )

                if "bay_reassignment_time" in record and isinstance(
                    record["bay_reassignment_time"], str
                ):
                    record["bay_reassignment_time"] = pd.to_datetime(
                        record["bay_reassignment_time"]
                    )

                if "trans_start_time" in record and isinstance(
                    record["trans_start_time"], str
                ):
                    record["trans_start_time"] = pd.to_datetime(
                        record["trans_start_time"]
                    )

                if "trans_end_time" in record and isinstance(
                    record["trans_end_time"], str
                ):
                    record["trans_end_time"] = pd.to_datetime(record["trans_end_time"])

                if "current_k_factor" in record:
                    try:
                        record["current_k_factor"] = (
                            round(float(record["current_k_factor"]), 2)
                            if record["current_k_factor"]
                            else None
                        )
                ...
                if "current_meter_factor" in record:
                    try:
                        record["current_meter_factor"] = (
                            round(float(record["current_meter_factor"]), 2)
                            if record["current_meter_factor"]
                            else None
                        )
                    except (ValueError, TypeError):
        ...
        for sap_id, sap_id_data in data_by_sap_id.items():
            ...
            if table_db_name == "host_day_end_details":
                ...
                processed_data = [
                    {k: v for k, v in record.items() if k not in columns_to_skip}
            ...
            if table_db_name == "host_manual_fan_printed":
                ...
                current_date = urdhva_base.utilities.get_present_time().strftime(
                    "%Y-%m-%d"
                )
                ...
                """
                latest_record_resp = await urdhva_base.BasePostgresModel.get_aggr_data(
                    check_query
                )
                ...
                if latest_record_resp.get("data") and latest_record_resp.get("data")[0]:
                    latest_db_count = latest_record_resp.get("data")[0].get(
                        "manual_fan_count"
                    )
                ...
                for record in sap_id_data:
                    ...
                    if current_manual_count > 0:
                        if (
                            current_manual_count != latest_db_count
                            and current_manual_count != latest_processed_count
                        ):
                ...
                if is_eod and not filtered_data:  # No records inserted yet
                    ...
                    """
                    zero_check_resp = await urdhva_base.BasePostgresModel.get_aggr_data(
                        zero_check_query
                    )

                    record_count = 0
                    if (
                        zero_check_resp.get("data")
                        and len(zero_check_resp.get("data")) > 0
                    ):
                        record_count = int(
                            zero_check_resp.get("data")[0].get("count", 0)
                        )
                    ...
                    if record_count == 0 and latest_db_count != 0:
                        zero_records = [
                            x for x in sap_id_data if x.get("manual_fan_count", 0) == 0
                        ]
                        if zero_records:
                            zero_records.sort(
                                key=lambda x: x.get("date_time", ""), reverse=True
                            )
                            filtered_data.append(zero_records[0])

                # After processing, update processed_data with our filtered results
                processed_data = filtered_data
            status, msg = await model.bulk_update(
                processed_data, upsert=True, upsert_skip_keys=["alert_created"]
            )
            ...
            if table_db_name == "host_manual_fan_printed":
                ...
                if resp.get("data", []):
                    for record in resp.get("data", []):
                        ...
                        if result is None:
                            ...
                            if interlock_name != "" and interlock_name is not None:
                                ...
                                if is_close_alert:
                                    await self.close_created_alert(
                    ...
                    return {
                        "status": "Table updated and alerts processed for non-zero records"
                    }
                else:
            ...
            if table_db_name == "host_unauthorised_flow":
                ...
                for record in unauthorised_records:
                    float(record.get("start_totalizer", 0))
                    float(record.get("end_totalizer", 0))
                    ...
                    is_close_alert, interlock_name = await self.cal_unauthorized_flow(
                        net_totalizer
                    )
                ...
                for record in resp.get("data", []):
                    try:
                        ...
                        if interlock_name != "" and interlock_name is not None:
                            ...
                            if interlock_name == "Cancel TT Reported":
                                is_close_alert = True
                                success, msg, alert_data = (
                                    await self.create_cancel_tt_report(alert_data)
                                )
                            elif interlock_name == "Bay reassignment":
                                is_close_alert = True
                                success, msg, alert_data = (
                                    await self.create_bay_reasignment_report(
                                        alert_data,
                                        compartment_number=record.get(
                                            "compartment_number", ""
                                        ),
                                    )
                                )

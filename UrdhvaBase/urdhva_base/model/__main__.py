...
parser.add_argument(
    ...
    "--db",
    default="mongo",
    choices=["mongo", "elastic", "postgres"],
...
mm.model_param_defs.add("gen", "The code generation target programming language.")
mm.model_param_defs.add("file", "input file used for code generation.")
mm.model_param_defs.add("db", "Database to be used for ")
        ...
        "policies": [
            {
                ...
                "config": {"roles": '[{"id":"superAdmin","required":false}]'},
            },
            {
            ...
            },
            {
                ...
                "logic": "POSITIVE",
                "decisionStrategy": "UNANIMOUS",
                "config": {"roles": '[{"id":"readOnly","required":false}]'},
            },
            {
            ...
            },
            {
                ...
                "logic": "POSITIVE",
                "decisionStrategy": "UNANIMOUS",
                "config": {"roles": '[{"id":"admin","required":false}]'},
            },
            {
    ...
    for model in m.models:
        if not model.is_internal:
            ...
            for action in model.actions:
                authz["scopes"].append(
                    {"name": action.name.lower(), "displayName": action.name.lower()}
                )
                resource_spec["scopes"].append({"name": action.name.lower()})
                resource_spec_admin["scopes"].append({"name": action.name.lower()})
                if action.name.lower().startswith(
                    "get"
                ) or action.name.lower().startswith("search"):
